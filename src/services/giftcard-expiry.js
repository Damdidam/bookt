...
async function processExpiredGiftCards() {
  const result = await query(
    `UPDATE gift_cards SET status = 'expired', updated_at = NOW()
     ...
     RETURNING id, code, business_id, amount_cents, balance_cents, recipient_email, recipient_name`
  );
  ...
  for (const gc of result.rows) {
    try {
      ...
      const bizResult = await query(
        `SELECT name, theme FROM businesses WHERE id = $1`,
        [gc.business_id]
      );
      if (!bizResult.rows[0]) continue;
      const { name: biz_name, theme: biz_theme } = bizResult.rows[0];
      ...
      const bodyHTML = `
        ...
        <p style="font-size:14px;color:#3D3832">N'h\u00e9sitez pas \u00e0 nous contacter pour toute question.</p>`;
