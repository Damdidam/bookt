  ...
  try {
    ...
    if (isSplitGroup && Array.isArray(slotPractitioners) && slotPractitioners.length > 0) {
      ...
      for (let gi = 0; gi < groupMembers.length; gi++) {
        ...
        await client.query(
          `UPDATE bookings SET start_at = $1, end_at = $2, ${gi === 0 ? 'reschedule_count = reschedule_count + 1, ' : ''}reminder_24h_sent_at = NULL, reminder_2h_sent_at = NULL, confirmation_expires_at = CASE WHEN status = 'pending' THEN NULL ELSE confirmation_expires_at END, status = CASE WHEN status = 'modified_pending' THEN 'confirmed' ELSE status END, updated_at = NOW()
           WHERE id = $3`,
    ...
    // SKIP for quote_only — merchant set a specific amount manually via "Exiger un acompte"
    {
      ...
      if (bk.deposit_status === 'pending' && !_isDepQuoteOnly) {
        ...
        const _depPct = parseInt(_depSettings.deposit_percent) || 0;
    ...
    // BUG-RESCHEDULE-INVOICE fix: delegate to syncDraftInvoicesForBookings which ALSO
    // detects sent/paid invoices whose price no longer matches booked_price and queues
    // an email_dispute_alert so the pro can issue a credit note (legal BE requirement).
    // Previous inline code only handled drafts — staff reschedule already uses this helper,
    // public flow should have parity (else client can change price of a sent invoice silently).
    {
      const allIds = (bk.group_id && groupMembers.length > 0) ? groupMembers.map(m => m.id) : [bk.id];
      const { syncDraftInvoicesForBookings } = require('../staff/bookings-helpers');
      await syncDraftInvoicesForBookings(client, allIds);
    }
