...
router.post('/:slug/gift-card/checkout', depositLimiter, async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `SELECT id, name, slug, plan, settings, theme, stripe_connect_id, stripe_connect_status FROM businesses WHERE slug = $1 AND is_active = true LIMIT 1`,
      [req.params.slug]
    );
    if (rows.length === 0) return res.status(404).json({ error: 'Salon introuvable' });
    const biz = rows[0];
    ...
    // STRIPE-04 fix (scan 23 avril) : gate stripe_connect_status='active' pour éviter
    // 500 inexplicable si compte désactivé/restricted. Parity avec deposit.js:41.
    if (biz.stripe_connect_id && biz.stripe_connect_status !== 'active') {
      return res.status(503).json({ error: 'Le paiement en ligne n\'est pas encore configuré par ce commerce. Contactez-le directement.' });
    }
    const s = biz.settings || {};
    ...
    if (remaining <= 0) {
      ...
      try {
        ...
        if (fullBk.rows[0]?.client_email) {
          ...
          const _emailBooking = { start_at: row.start_at, end_at: groupEndAt || row.end_at, deposit_amount_cents: row.deposit_amount_cents, gc_paid_cents: gcPaidForEmail, client_name: row.client_name, client_email: row.client_email, client_phone: row.client_phone, service_name: row.service_name, service_category: row.service_category, practitioner_name: row.practitioner_name, public_token: row.public_token, promotion_label: row.promotion_label, promotion_discount_cents: row.promotion_discount_cents, promotion_discount_pct: row.promotion_discount_pct, service_price_cents: row.service_price_cents, booked_price_cents: row.booked_price_cents, discount_pct: row.discount_pct, duration_min: row.duration_min };
          await sendDepositPaidEmail({
            booking: _emailBooking,
            business: { id: bk.business_id, name: row.biz_name, slug: row.biz_slug, email: row.biz_email, phone: row.biz_phone, address: row.biz_address, theme: row.biz_theme, settings: row.biz_settings },
            groupServices
          });
          sendDepositPaidProEmail({
            booking: _emailBooking,
            business: { id: bk.business_id, name: row.biz_name, email: row.biz_email, theme: row.biz_theme }
...
router.post('/:slug/pass/checkout', depositLimiter, async (req, res, next) => {
  try {
    ...
    // STRIPE-04 fix (scan 23 avril) : gate stripe_connect_status='active' (parity deposit.js:41)
    if (biz.stripe_connect_id && biz.stripe_connect_status !== 'active') {
      return res.status(503).json({ error: 'Le paiement en ligne n\'est pas encore configuré par ce commerce. Contactez-le directement.' });
    }
