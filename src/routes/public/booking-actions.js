    ...
    (async () => {
      try {
        ...
        if (fullBk.rows[0]?.client_email) {
          ...
          // Pattern SELECT COPIÉ sur deposit-expiry.js:336-345 (crons ground truth):
          // bookings n'a PAS de colonnes service_price_cents/duration_min — elles
          // sont COALESCE depuis sv/s via LEFT JOIN service_variants + services.
          if (bk.group_id) {
            try {
...
router.post('/booking/:token/reject', async (req, res, next) => {
  try {
    ...
    // X3C-04 fix: symétrie avec /cancel + /cancel-booking — capture net_refund_cents (reset=0 si
    // retention). Reject fait full refund Stripe (pas de fees déduits), donc en success rejectNetRefundCents
    // reste null → fallback email-cancel.js sur gross (= deposit_amount - gcPaid) = montant correct.
    let rejectNetRefundCents = null;
    try {
      await txClient.query('BEGIN');
      result = await txClient.query(
      ...
      try {
        ...
        if (fullBk.rows[0]?.client_email) {
          ...
          await sendCancellationEmail({
            ...
            business: { id: rejBk.business_id, name: row.biz_name, email: row.biz_email, phone: row.biz_phone, address: row.biz_address, theme: row.biz_theme, slug: row.biz_slug, settings: row.biz_settings },
            groupServices
          });
        }
    ...
    (async () => {
      try {
        ...
        if (fullBk.rows[0]?.client_email) {
          ...
          // B#4 fix (2e route publique) : siblings multi-clients pour /cancel-booking
          if (bk.group_id) {
            try {
              const _sibsMc2 = await query(
                `SELECT b.id, b.start_at, b.end_at,
                        COALESCE(sv.price_cents, s.price_cents, 0) AS service_price_cents,
                        COALESCE(sv.duration_min, s.duration_min, 0) AS duration_min,
                        b.booked_price_cents, b.discount_pct,
