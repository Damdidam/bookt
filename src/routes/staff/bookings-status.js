            ...
            try {
              ...
              for (const _sibS of _sibsMcS.rows) {
                try {
                  ...
                  // BL-10 clone fix : si sibling a un PI distinct du primary, ne pas forwarder
                  // netRefundCentsForEmail (calcul fait sur primary uniquement) → null → template recalcule.
                  const _sibNetRefundS = (_sibS.deposit_payment_intent_id && _sibS.deposit_payment_intent_id === d.deposit_payment_intent_id)
                    ? txResult.netRefundCentsForEmail
                    : null;
                  sendCancellationEmail({
                    booking: { start_at: _sibS.start_at, end_at: _sibS.end_at, client_name: _sibS.client_name, client_email: _sibS.client_email, service_name: _sibS.service_name, service_category: _sibS.service_category, custom_label: _sibS.custom_label, comment_client: _sibS.comment_client, practitioner_name: _sibS.practitioner_name, deposit_required: _sibS.deposit_required, deposit_status: _sibS.deposit_status, deposit_amount_cents: _sibS.deposit_amount_cents, deposit_paid_at: _sibS.deposit_paid_at, deposit_payment_intent_id: _sibS.deposit_payment_intent_id, gc_paid_cents: _sibGcS, gc_refunded_cents: _sibGcRefS.rows[0]?.amt || 0, pass_refunded: _sibPassRefS.rows.length > 0, net_refund_cents: _sibNetRefundS, deposit_retention_reason: txResult.depRetentionReason, service_price_cents: _sibS.service_price_cents, booked_price_cents: _sibS.booked_price_cents, discount_pct: _sibS.discount_pct, duration_min: _sibS.duration_min, promotion_label: _sibS.promotion_label, promotion_discount_cents: _sibS.promotion_discount_cents, promotion_discount_pct: _sibS.promotion_discount_pct, cancel_reason: _sibS.cancel_reason || 'Annulé par le salon' },
                    business: { id: bid, name: d.biz_name, slug: d.slug, email: d.biz_email, phone: d.biz_phone, address: d.address, theme: d.theme, settings: d.biz_settings },
                    groupServices
          ...
          try {
            ...
            for (const _sibD of _sibsMcD.rows) {
              try {
                ...
                // BL-10 clone fix : si sibling a un PI distinct du primary, ne pas forwarder
                // netRefundCentsManual (calculé sur PI primary uniquement) → null → template recalcule.
                const _sibNetRefundD = (_sibD.deposit_payment_intent_id && _sibD.deposit_payment_intent_id === d.deposit_payment_intent_id)
                  ? txResult.netRefundCentsManual
                  : null;
                _sendCancelD({
                  booking: { start_at: _sibD.start_at, end_at: _sibD.end_at, client_name: _sibD.client_name, client_email: _sibD.client_email, service_name: _sibD.service_name, service_category: _sibD.service_category, custom_label: _sibD.custom_label, comment_client: _sibD.comment_client, practitioner_name: _sibD.practitioner_name, deposit_required: _sibD.deposit_required, deposit_status: _sibD.deposit_status, deposit_amount_cents: _sibD.deposit_amount_cents, deposit_paid_at: _sibD.deposit_paid_at, deposit_payment_intent_id: _sibD.deposit_payment_intent_id, gc_paid_cents: _sibGcD, gc_refunded_cents: _sibGcRefD.rows[0]?.amt || 0, pass_refunded: _sibPassRefD.rows.length > 0, net_refund_cents: _sibNetRefundD, deposit_retention_reason: txResult.depRetentionReasonManual, service_price_cents: _sibD.service_price_cents, booked_price_cents: _sibD.booked_price_cents, discount_pct: _sibD.discount_pct, duration_min: _sibD.duration_min, promotion_label: _sibD.promotion_label, promotion_discount_cents: _sibD.promotion_discount_cents, promotion_discount_pct: _sibD.promotion_discount_pct, cancel_reason: _sibD.cancel_reason || 'Acompte remboursé — RDV annulé' },
                  business: { id: bid, name: d.biz_name, slug: d.slug, email: d.biz_email, phone: d.biz_phone, address: d.address, settings: d.settings, theme: d.theme },
                  groupServices
