...
router.patch('/', requireOwner, blockIfImpersonated, async (req, res, next) => {
  try {
    ...
    // Defense-in-depth : activer deposit/giftcard/passes requires Stripe Connect actif
    // (audit batch 15 P1 + batch 20 P2). Sans ce guard, direct API call peut activer
    // ces features en DB sans Stripe → checkouts casses cote client.
    const _stripeGated = settings_deposit_enabled === true
