...
function renderGrid() {
  ...
  const weekNav = `<div style="display:flex;align-items:center;gap:10px">
    <button class="btn-outline btn-sm" onclick="fsWeekNav(-1)" aria-label="Semaine précédente"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px"><polyline points="15 18 9 12 15 6"/></svg></button>
    <span style="font-size:.85rem;font-weight:600;min-width:200px;text-align:center">${wsLabel} — ${weLabel}</span>
    <button class="btn-outline btn-sm" onclick="fsWeekNav(1)" aria-label="Semaine suivante"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px"><polyline points="9 18 15 12 9 6"/></svg></button>
  </div>`;
