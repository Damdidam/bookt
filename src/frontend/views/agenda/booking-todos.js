...
function fcRenderTodos() {
  ...
  if (!t.length) {
    el.innerHTML = '<div class="m-empty"><div class="m-empty-icon"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg></div>Aucune t\u00e2che</div>';
    return;
  }
  el.innerHTML = t.map(x => `<div class="todo-item ${x.is_done ? 'done' : ''}"><div class="todo-check" onclick="fcToggleTodo('${safeId(x.id)}',${!x.is_done})">${x.is_done ? '<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>' : ''}</div><span class="todo-text">${esc(x.content)}</span><button class="todo-delete" onclick="fcDeleteTodo('${safeId(x.id)}')"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>`).join('');
}
