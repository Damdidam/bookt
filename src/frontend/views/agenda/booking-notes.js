...
function fcRenderNotes() {
  ...
  if (!n.length) {
    el.innerHTML = '<div class="m-empty"><div class="m-empty-icon">' + IC.fileText + '</div>Aucune note</div>';
    return;
  }
  el.innerHTML = n.map(x => `<div class="note-card ${x.is_pinned ? 'pinned' : ''}"><div class="note-content">${esc(x.content)}</div><div class="note-meta">${esc(x.author_email || 'Vous')} \u00b7 ${new Date(x.created_at).toLocaleDateString('fr-BE', { timeZone: 'Europe/Brussels' })}${x.is_pinned ? ' \u00b7 ' + IC.pin : ''}</div><button class="note-delete" onclick="fcDeleteNote('${safeId(x.id)}')">${IC.x}</button></div>`).join('');
}
