...
async function loadCalls(){
  ...
  try{
    ...
    if(usage && usage.quota > 0){
      ...
      const barCol=isOver?'var(--red)':isWarn?'var(--amber)':'var(--primary)';
      h+=`<div class="card" style="margin-bottom:16px;${isOver?'border:2px solid var(--red);background:var(--red-bg)':''}">`;
      ...
      h+=`<div style="position:absolute;left:${Math.min(callPct,100)}%;top:0;height:100%;width:${Math.min(smsPct,100-Math.min(callPct,100))}%;background:var(--green);transition:width .5s" title="SMS: ${usage.usage?.sms||0}"></div>`;
      ...
      h+=`<span><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:var(--green);margin-right:4px"></span>SMS : ${usage.usage?.sms||0}</span>`;
      if(usage.usage?.voicemails)h+=`<span><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:var(--purple);margin-right:4px"></span>Vocaux : ${usage.usage.voicemails}</span>`;
      h+=`</div>`;
      if(usage.billing?.overage>0){
        h+=`<div style="margin-top:10px;padding:10px 14px;background:var(--red-bg);border:1px solid var(--red-bg);border-radius:8px;font-size:.82rem;color:var(--red)">`;
        ...
        h+=`<div style="margin-top:10px;padding:10px 14px;background:var(--amber-bg);border:1px solid var(--amber-bg);border-radius:8px;font-size:.82rem;color:var(--amber-dark)">`;
    ...
    h+=`<div class="kpi"><div class="kpi-val" style="color:var(--green)">${st.vip||0}</div><div class="kpi-label"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> VIP pass\u00e9s</div></div>`;
...
function renderCallLogs(logs){
  ...
  if(logs.length===0){
    ...
    logs.forEach(l=>{
      ...
      const dateStr=dt.toLocaleDateString('fr-BE',{day:'numeric',month:'short',timeZone:'Europe/Brussels'})+' '+dt.toLocaleTimeString('fr-BE',{hour:'2-digit',minute:'2-digit'});
...
function renderCallVoicemails(vms){
  ...
  if(vms.length===0){
    ...
    vms.forEach(vm=>{
      ...
      const dateStr=dt.toLocaleDateString('fr-BE',{day:'numeric',month:'short',year:'numeric',timeZone:'Europe/Brussels'})+' \u00e0 '+dt.toLocaleTimeString('fr-BE',{hour:'2-digit',minute:'2-digit'});
...
function renderCallUsage(usage,logs){
  ...
  h+=`<div style="padding:14px;background:#f0fdf4;border-radius:10px;text-align:center"><div style="font-size:1.4rem;font-weight:800;color:var(--green)">${usage.usage?.sms||0}</div><div style="font-size:.78rem;color:var(--text-3)">SMS</div></div>`;
  h+=`<div style="padding:14px;background:var(--purple-bg);border-radius:10px;text-align:center"><div style="font-size:1.4rem;font-weight:800;color:var(--purple)">${usage.usage?.voicemails||0}</div><div style="font-size:.78rem;color:var(--text-3)">Vocaux</div></div>`;
  h+=`<div style="padding:14px;background:${(usage.billing?.overage||0)>0?'var(--red-bg)':'#f8fafc'};border-radius:10px;text-align:center"><div style="font-size:1.4rem;font-weight:800;color:${(usage.billing?.overage||0)>0?'var(--red)':'var(--text)'}">${usage.usage?.total||0} / ${usage.quota}</div><div style="font-size:.78rem;color:var(--text-3)">Total / Forfait</div></div>`;
  h+=`</div>`;
  ...
  h+=`<div>Hors forfait : <strong style="color:${(usage.billing?.overage||0)>0?'var(--red)':'var(--green)'}">${usage.billing?.overage||0} unit\u00e9s (${((usage.billing?.overage_total_cents||0)/100).toFixed(2)}\u20ac)</strong></div>`;
  h+=`</div></div>`;
  ...
  (logs||[]).slice(0,20).forEach(l=>{
    const dt=new Date(l.created_at).toLocaleDateString('fr-BE',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit',timeZone:'Europe/Brussels'});
...
function renderCallConfig(cs){
  ...
  const modeLabels={off:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--red)"><circle cx="12" cy="12" r="4" fill="currentColor"/></svg> D\u00e9sactiv\u00e9',soft:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--amber)"><circle cx="12" cy="12" r="4" fill="currentColor"/></svg> Soft',strict:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--green)"><circle cx="12" cy="12" r="4" fill="currentColor"/></svg> Strict',schedule_based:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> Horaires',vacation:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg> Vacances'};
  const modeColors={off:'var(--red-bg);color:var(--red)',soft:'var(--amber-bg);color:var(--amber-dark)',strict:'#f0fdf4;color:var(--green)',schedule_based:'#eff6ff;color:#2563eb',vacation:'var(--amber-bg);color:var(--amber-dark)'};
  ...
  const modes=[
    {key:'off',icon:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--red)"><circle cx="12" cy="12" r="4" fill="currentColor"/></svg>',name:'D\u00e9sactiv\u00e9',desc:'Les appels passent directement sur votre t\u00e9l\u00e9phone.'},
    {key:'soft',icon:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--amber)"><circle cx="12" cy="12" r="4" fill="currentColor"/></svg>',name:'Soft',desc:'Message vocal + SMS. L\'appel est ensuite transf\u00e9r\u00e9 sur votre t\u00e9l\u00e9phone.'},
    {key:'strict',icon:'<svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--green)"><circle cx="12" cy="12" r="4" fill="currentColor"/></svg>',name:'Strict',desc:'Message vocal + SMS + raccroch\u00e9. Vous n\'\u00eates jamais d\u00e9rang\u00e9.'},
...
function renderCallBlacklist(bl){
  ...
  if(bl.length===0){
    ...
    bl.forEach(b=>{
      const date=new Date(b.created_at).toLocaleDateString('fr-BE',{day:'numeric',month:'short',timeZone:'Europe/Brussels'});
...
async function saveBlacklistEntry(){
  ...
  try{
    ...
    document.getElementById('blModal')._dirtyGuard?.markClean();
    closeModal('blModal');
...
function editWhitelistEntry(id,phone,label,active){
  ...
  let m=`<div class="m-overlay open" id="wlEditModal"><div class="m-dialog m-sm"><div class="m-header-simple"><h3>Modifier num\u00e9ro VIP</h3><button class="m-close" onclick="closeModal('wlEditModal')">${X_SVG}</button></div><div class="m-body">`;
...
async function saveWhitelistEntry(id){
  try{
    ...
    document.getElementById('wlEditModal')._dirtyGuard?.markClean();
    closeModal('wlEditModal');
