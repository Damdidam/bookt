import{a as s,G as u,d as b,s as w}from"./dashboard-CKD_CV46.js";import{i as k,s as $}from"./plan-gate-Badg7tzn.js";const r=u.toast.bind(u);function d(t){return String(t||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const p={google:{label:"Google Calendar",color:"#4285F4",icon:"M21.35 11.1h-9.18v2.73h5.51c-.54 2.56-2.73 3.97-5.51 3.97a6.13 6.13 0 1 1 0-12.26c1.51 0 2.89.55 3.96 1.45l2.04-2.04A9.27 9.27 0 0 0 12.17 2 10 10 0 1 0 22 12.17c0-.57-.07-1.13-.18-1.66l-.47.59Z"},outlook:{label:"Outlook Calendar",color:"#0078D4",icon:"M21 5H3v14h18V5Zm-2 2v3h-3V7h3ZM5 7h3v3H5V7Zm0 5h3v3H5v-3Zm5 5v-3h4v3h-4Zm4-5h-4v-3h4v3Zm2 5v-3h3v3h-3Zm3-5h-3v-3h3v3Z"},ical:{label:"Lien iCal",color:"#6B6560",icon:"M8 2v4m8-4v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Z"}},C={push:"Genda → Calendrier",pull:"Calendrier → Genda",both:"Bidirectionnel"};async function g(){if(!k()){$(document.getElementById("contentArea"),"Calendrier externe");return}const t=document.getElementById("contentArea");t.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const[o,n]=await Promise.all([fetch("/api/calendar/connections",{headers:{Authorization:"Bearer "+s.getToken()}}),fetch("/api/practitioners",{headers:{Authorization:"Bearer "+s.getToken()}})]),l=(await o.json()).connections||[],a=(await n.json()).practitioners||[];let i='<div style="max-width:640px;margin:0 auto">';if(i+=`<div class="card" style="margin-bottom:16px">
      ...
      </div>`;else{i+='<div class="card"><h3 style="font-size:.88rem;font-weight:700;margin-bottom:12px">Calendriers connectés</h3>';for(const e of l){const c=p[e.provider]||p.ical,h=a.find(f=>f.id===e.practitioner_id)?.display_name||"Tous",v=e.status==="active"?"var(--green)":"var(--red)",x=e.status==="active"?"Connecté":e.status,m=e.last_sync_at?new Date(e.last_sync_at).toLocaleString("fr-BE",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"}):"Jamais";i+=`<div style="padding:14px;border:1px solid var(--border-light);border-radius:12px;margin-bottom:10px">
    ...
    </div>`,r("Lien iCal généré","success")}catch(t){r(t.message,"error")}}async function B(t,o){try{const n=await fetch(`/api/calendar/connections/${t}`,{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:"Bearer "+s.getToken()},body:JSON.stringify({sync_direction:o})});if(!n.ok)throw new Error((await n.json()).error);r("Direction mise à jour","success")}catch(n){r(n.message,"error")}}async function M(t){try{r("Synchronisation...","info");const o=await fetch(`/api/calendar/connections/${t}/sync`,{method:"POST",headers:{Authorization:"Bearer "+s.getToken()}}),n=await o.json();if(!o.ok)throw new Error(n.error);r(`Sync OK — ${n.pulled||0} importé(s), ${n.pushed||0} exporté(s)`,"success"),g()}catch(o){r(o.message,"error")}}async function T(t,o){if(await w(`Déconnecter ${o} ?`))try{const n=await fetch(`/api/calendar/connections/${t}`,{method:"DELETE",headers:{Authorization:"Bearer "+s.getToken()}});if(!n.ok)throw new Error((await n.json()).error);r("Calendrier déconnecté","success"),g()}catch(n){r(n.message,"error")}}b({loadCalSync:g,calSyncConnect:z,calSyncGenIcal:S,calSyncUpdateDir:B,calSyncNow:M,calSyncDisconnect:T});export{g as loadCalSync};
