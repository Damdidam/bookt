import{a as m,e as a,d as f,G as d,n as h}from"./dashboard-CeZSoZue.js";import{I as b}from"./icons-DATqSsmT.js";let x=[],c=null;const w={min_amount:e=>`Panier min ${e||"?"} €`,specific_service:e=>`Service : ${e||"?"}`,first_visit:()=>"Nouveau client",date_range:(e,t)=>`Du ${e||"?"} au ${t||"?"}`,none:()=>"Sans condition"},_={free_service:e=>`Service offert : ${e||"?"}`,discount_pct:e=>`Réduction ${e||"?"}%`,discount_fixed:e=>`Réduction ${e||"?"} €`,info_only:()=>"Info seulement"};function $(e){const t=w[e.condition_type];return t?e.condition_type==="date_range"?t(e.condition_date_start,e.condition_date_end):t(e.condition_value):e.condition_type||"—"}function E(e){const t=_[e.reward_type];return t?t(e.reward_value):e.reward_type||"—"}async function v(){const e=document.getElementById("contentArea");e.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=await m.get("/api/promotions"),o=t.promotions||t||[];x=o,k(e,o)}catch(t){e.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${a(t.message)}</div>`}}function k(e,t){const o=t.length,s=t.filter(i=>i.is_active).length;let r="";r+=`<div class="stats" style="grid-template-columns:repeat(2,1fr)">
    ...
    <div class="stat-card"><div class="label">Promotions actives</div><div class="val" style="color:var(--green)">${s}</div><div class="sub">en cours</div></div>
  </div>`,r+=`<div class="card" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 16px">
    <div style="flex:1"></div>
    <span style="font-size:.78rem;color:var(--text-3)">${s}/${o} actives</span>
    <button onclick="openPromoModal()" class="btn-primary"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nouvelle promotion</button>
      ...
      </tr>`}),r+="</tbody></table></div>"),e.innerHTML=r}async function z(){if(c)return c;try{const e=await m.get("/api/services");c=(e.services||e||[]).filter(t=>t.is_active!==!1)}catch(e){console.error("[promotions] fetch services failed",e),c=[]}return c}async function C(e){const t=document.getElementById("promoModal");t&&t.remove();let o=null;if(e&&(o=x.find(u=>u.id===e),!o)){d.toast("Promotion introuvable","error");return}const r=(await z()).map(u=>`<option value="${a(u.id)}">${a(u.name)}</option>`).join(""),i=o?.condition_type||"none",n=o?.reward_type||"info_only",l=o?.display_style||"banner",p=document.createElement("div");p.className="m-overlay open",p.id="promoModal",p.innerHTML=`<div class="m-dialog m-md">
    ...
    <div class="m-body">
      ...
      <div style="margin-bottom:14px">
        ...
        <div style="display:flex;gap:10px">
          <label style="flex:1;cursor:pointer;padding:10px;border:2px solid ${l==="cards"?"var(--primary)":"var(--border)"};border-radius:var(--radius-xs);text-align:center;font-size:.82rem;background:${l==="cards"?"var(--primary-light)":"var(--surface)"}">
            <input type="radio" name="promoDisplay" value="cards" ${l==="cards"?"checked":""} style="display:none" onchange="promoDisplayChanged()"> Carte
          </label>
    ...
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="savePromo('${e||""}')">${o?"Enregistrer":"Créer"}</button>
  ...
  </div>`,document.body.appendChild(p),h(document.getElementById("promoModal"),{noBackdropClose:!0})}function y(e,t,o){const s=t?.condition_value||"";switch(e){case"min_amount":return`<label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Montant minimum (€)</label>
        <input type="number" id="promoConditionValue" min="0" step="0.01" value="${a(s)}" placeholder="50" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box">`;case"specific_service":return`<label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Service concerné</label>
        ...
        <div style="flex:1"><label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Date de début</label>
          <input type="date" id="promoConditionDateStart" value="${a(t?.condition_date_start||"")}" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box"></div>
        <div style="flex:1"><label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Date de fin</label>
          <input type="date" id="promoConditionDateEnd" value="${a(t?.condition_date_end||"")}" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box"></div>
      </div>`;default:return""}}function g(e,t,o){const s=t?.reward_value||"";switch(e){case"free_service":return`<label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Service offert</label>
        ...
        <input type="number" id="promoRewardValue" min="1" max="100" value="${a(s)}" placeholder="20" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box">`;case"discount_fixed":return`<label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Montant de réduction (€)</label>
        <input type="number" id="promoRewardValue" min="0" step="0.01" value="${a(s)}" placeholder="10" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box">`;default:return""}}function I(){const e=document.getElementById("promoConditionType").value,t=document.getElementById("promoConditionFields"),s=(c||[]).map(r=>`<option value="${a(r.id)}">${a(r.name)}</option>`).join("");t.innerHTML=y(e,null,s)}function B(){const e=document.getElementById("promoRewardType").value,t=document.getElementById("promoRewardFields"),s=(c||[]).map(r=>`<option value="${a(r.id)}">${a(r.name)}</option>`).join("");t.innerHTML=g(e,null,s)}function M(){document.querySelectorAll('input[name="promoDisplay"]').forEach(t=>{const o=t.closest("label");t.checked?(o.style.borderColor="var(--primary)",o.style.background="var(--primary-light)"):(o.style.borderColor="var(--border)",o.style.background="var(--surface)")})}async function P(e){const t=document.getElementById("promoTitle").value.trim();if(!t){d.toast("Le titre est requis","error");return}const o=document.getElementById("promoDescription").value.trim(),s=document.getElementById("promoConditionType").value,r=document.getElementById("promoRewardType").value,i=document.querySelector('input[name="promoDisplay"]:checked')?.value||"banner",n={title:t,description:o,condition_type:s,reward_type:r,display_style:i};if(s==="min_amount"){var l=parseFloat(document.getElementById("promoConditionValue")?.value||0);n.condition_min_cents=Math.round(l*100)}else s==="specific_service"?n.condition_service_id=document.getElementById("promoConditionValue")?.value||null:s==="date_range"&&(n.condition_start_date=document.getElementById("promoConditionDateStart")?.value||null,n.condition_end_date=document.getElementById("promoConditionDateEnd")?.value||null);if(r==="free_service")n.reward_service_id=document.getElementById("promoRewardValue")?.value||null;else if(r==="discount_pct")n.reward_value=parseInt(document.getElementById("promoRewardValue")?.value||0);else if(r==="discount_fixed"){var p=parseFloat(document.getElementById("promoRewardValue")?.value||0);n.reward_value=Math.round(p*100)}try{e?(await m.patch(`/api/promotions/${e}`,n),d.toast("Promotion mise à jour","success")):(await m.post("/api/promotions",n),d.toast("Promotion créée","success")),document.getElementById("promoModal").remove(),v()}catch(u){d.toast(u.message||"Erreur lors de l'enregistrement","error")}}async function S(e,t){try{await m.patch(`/api/promotions/${e}`,{is_active:t}),d.toast(t?"Promotion activée":"Promotion désactivée","success"),v()}catch(o){d.toast(o.message||"Erreur","error")}}async function T(e,t){if(confirm(`Supprimer définitivement la promotion "${t}" ? Cette action est irréversible.`))try{await m.delete(`/api/promotions/${e}`),d.toast("Promotion supprimée","success"),v()}catch(o){d.toast(o.message||"Erreur lors de la suppression","error")}}f({loadPromotions:v,openPromoModal:C,savePromo:P,togglePromo:S,deletePromo:T,promoConditionChanged:I,promoRewardChanged:B,promoDisplayChanged:M});export{v as loadPromotions};
