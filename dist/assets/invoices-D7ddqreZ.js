import{s as $,a as g,G as u,m as E,l as k,d as S}from"./dashboard-C_JJSNJO.js";import{i as q,s as A}from"./plan-gate-Badg7tzn.js";import{trapFocus as C,releaseFocus as D}from"./focus-trap-C-UMhpsq.js";import{f}from"./format-DoZVPAsZ.js";let p="all",x="all",h=[];function v(i){return String(i||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function _(){if(!q()){A(document.getElementById("contentArea"),"Facturation");return}const i=document.getElementById("contentArea");i.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;p!=="all"&&t.set("status",p),x!=="all"&&t.set("type",x);const e=await g.get(`/api/invoices?${t}`),o=e.invoices||[],r=e.stats||{};let a="";a+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
      ...
      <select onchange="invoiceFilter=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        ...
        <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        </tr></thead><tbody>`,o.forEach(n=>{const l={draft:"var(--text-4)",sent:"var(--gold)",paid:"var(--green)",overdue:"var(--red)",cancelled:"var(--text-4)"},d={draft:"Brouillon",sent:"Envoyée",paid:"Payée",overdue:"En retard",cancelled:"Annulée"},c={invoice:"Facture",quote:"Devis",credit_note:"Note crédit"},y={invoice:"var(--primary)",quote:"var(--gold)",credit_note:"var(--text-3)"},m=l[n.status]||"var(--text-4)",I=new Date(n.issue_date).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"});a+=`<tr style="border-bottom:1px solid var(--border-light)">
          <td style="padding:10px 14px;font-weight:600">${v(n.invoice_number)}</td>
          <td style="padding:10px"><span style="font-size:.7rem;padding:2px 8px;border-radius:10px;background:${y[n.type]}15;color:${y[n.type]};font-weight:600">${c[n.type]||n.type}</span></td>
          ...
          <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${m}12;color:${m};font-weight:600">${d[n.status]||n.status}</span></td>
          <td style="padding:10px;text-align:right">
            <button onclick="downloadInvoicePDF('${n.id}')" title="Télécharger PDF" aria-label="Télécharger PDF" style="background:none;border:none;cursor:pointer;font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg></button>
            ${n.status==="draft"?`<button onclick="changeInvoiceStatus('${n.id}','sent')" title="Marquer envoyée" aria-label="Marquer envoyée" style="background:none;border:none;cursor:pointer;font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></button>`:""}
            ${n.status==="sent"||n.status==="overdue"?`<button onclick="changeInvoiceStatus('${n.id}','paid')" title="Marquer payée" aria-label="Marquer payée" style="background:none;border:none;cursor:pointer;font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></button>`:""}
            ${n.type==="invoice"&&["sent","paid","overdue"].includes(n.status)?`<button onclick="issueCreditNote('${n.id}','${v(n.invoice_number||"")}')" title="Émettre une note de crédit (conformité BE)" aria-label="Émettre une note de crédit (conformité BE)" style="background:none;border:none;cursor:pointer;font-size:1rem;color:var(--gold)"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="15" x2="15" y2="15"/></svg></button>`:""}
            ${n.status==="draft"?`<button onclick="deleteInvoice('${n.id}')" title="Supprimer" aria-label="Supprimer" style="background:none;border:none;cursor:pointer;font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></button>`:""}
          </td>
      ...
      </div>`),i.innerHTML=a}catch(t){i.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${v(t.message)}</div>`}}async function F(i="invoice",t={}){let e=[];try{const s=await g.get("/api/clients");e=s.clients||s||[]}catch(s){console.warn("Impossible de charger les clients pour la facture:",s.message)}const o=i==="quote",r=o?"Nouveau devis":"Nouvelle facture",a=document.createElement("div");if(a.className="m-overlay open",a.id="invModal",a.innerHTML=`<div class="m-dialog m-md">
    ...
    <div style="font-size:1rem;font-weight:700;color:var(--text);margin-top:4px">Total TTC : ${f(a)}</div>`}async function L(i){const t=document.getElementById("invClient")?.value;if(!t){u.toast("Sélectionnez un client","error");return}const e=document.getElementById("invLines"),o=[];if(e.querySelectorAll(":scope > div").forEach(r=>{const a=r.querySelector(".inv-desc")?.value?.trim(),s=parseFloat(r.querySelector(".inv-qty")?.value||1),n=parseFloat(r.querySelector(".inv-price")?.value||0),l=r.getAttribute("data-booking-id")||void 0;a&&n!==0&&o.push({description:a,quantity:s,unit_price_cents:Math.round(n*100),booking_id:l})}),o.length===0){u.toast("Ajoutez au moins une ligne","error");return}try{const r={client_id:t,type:i||"invoice",items:o,vat_rate:parseFloat(document.getElementById("invVat")?.value||21),due_days:parseInt(document.getElementById("invDueDays")?.value||30),client_bce:document.getElementById("invClientBce")?.value?.trim()||void 0,notes:document.getElementById("invNotes")?.value?.trim()||void 0},a=await g.post("/api/invoices",r);document.getElementById("invModal")._dirtyGuard?.markClean(),k("invModal"),u.toast(i==="quote"?"Devis créé !":"Facture créée !","success"),_()}catch(r){u.toast(r.message||"Erreur","error")}}async function N(i,t){if(await $("Changer le statut",{sent:"Marquer comme envoyée ?",paid:"Marquer comme payée ?"}[t]||`Changer le statut en "${t}" ?`,"Confirmer"))try{await g.patch(`/api/invoices/${i}/status`,{status:t}),u.toast("Statut mis à jour","success"),_()}catch(r){u.toast(r.message||"Erreur","error")}}async function j(i){if(await $("Supprimer le brouillon","Supprimer ce brouillon ?","Supprimer","danger"))try{await g.delete(`/api/invoices/${i}`),u.toast("Brouillon supprimé","success"),_()}catch(e){u.toast(e.message||"Erreur","error")}}function P(i){const t=localStorage.getItem("genda_token");window.open(`/api/invoices/${i}/pdf?token=${t}`,"_blank","noopener,noreferrer")}async function X(i,t){const e=document.createElement("div");e.className="m-overlay open",e.id="creditNoteModal",e.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="submitCreditNote('${i}')">Émettre la note de crédit</button>
    </div>
  </div>`,document.body.appendChild(e),E(e,{noBackdropClose:!0}),C(e,()=>k("creditNoteModal"))}async function V(i){try{const t=document.getElementById("cnReason")?.value?.trim()||null,e=document.getElementById("cnMarkCancelled")?.checked||!1,o=await g.post(`/api/invoices/${i}/credit-note`,{reason:t,mark_original_cancelled:e});D(),k("creditNoteModal"),u.toast(`Note de crédit ${o.credit_note?.invoice_number||""} émise`,"success"),_()}catch(t){u.toast(t.message||"Erreur lors de l'émission","error")}}async function H(i){if(await $("Créer une facture pour ce rendez-vous ?"))try{const t=await g.post("/api/invoices",{booking_id:i,type:"invoice"});u.toast("Facture créée ! Retrouvez-la dans Facturation.","success"),document.querySelectorAll(".ni").forEach(e=>e.classList.remove("active")),document.querySelector('[data-section="invoices"]')?.classList.add("active"),document.getElementById("pageTitle").textContent="Facturation",_()}catch(t){u.toast(t.message||"Erreur","error")}}async function M(){const i=document.getElementById("invClient")?.value,t=document.getElementById("invUnbilledSection"),e=document.getElementById("invUnbilledList");if(!i||!t||!e){t&&(t.style.display="none");return}try{h=(await g.get(`/api/invoices/unbilled?client_id=${i}`)).bookings||[]}catch{h=[]}if(h.length===0){t.style.display="none";return}t.style.display="",e.innerHTML=h.map((o,r)=>{const a=new Date(o.start_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}),s=o.booked_price_cents??o.variant_price_cents??o.service_price_cents??0,n=o.service_name+(o.variant_name?" — "+o.variant_name:"")+" ("+(o.practitioner_name||"?")+") — "+a,l=o.pass_covered?' <span style="font-size:.68rem;color:var(--green);font-weight:600;padding:1px 6px;border-radius:4px;background:var(--green-bg)">Pass</span>':"",d=o.promotion_discount_cents>0?' <span style="font-size:.68rem;color:var(--green);font-weight:600">promo</span>':"",c=o.pass_covered?'<span style="color:var(--green)">inclus</span>':f(s);return`<label style="display:flex;align-items:center;gap:8px;padding:8px 12px;font-size:.82rem;cursor:pointer;border-bottom:1px solid var(--border-light);background:${r%2===0?"var(--white)":"var(--surface)"}">
    ...
    </label>`}).join("")}function z(i,t){const e=h[i];if(!e)return;const o=document.getElementById("invLines");if(o){if(t){const r=new Date(e.start_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}),a=e.service_name+(e.variant_name?" — "+e.variant_name:"")+" ("+(e.practitioner_name||"")+") — "+r,s=e.booked_price_cents??e.variant_price_cents??e.service_price_cents??0;if(e.pass_covered&&e.pass_info){b(e.id,a+" (inclus abonnement)",1,0);const n=e.pass_info,l="1 crédit consommé — Pass "+n.name+" ("+n.sessions_remaining+"/"+n.sessions_total+" restants)";[...o.querySelectorAll(".inv-desc")].some(c=>c.value===l)||b(e.id,l,1,0)}else if(b(e.id,a,1,s/100),e.promotion_discount_cents>0&&e.promotion_label){const n="Réduction : "+e.promotion_label+(e.promotion_discount_pct?" (-"+e.promotion_discount_pct+"%)":"");b(e.id,n,1,-(e.promotion_discount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id){const n=parseInt(e.gc_paid_cents)||0,l=parseInt(e.deposit_amount_cents)||0;if(e.deposit_payment_intent_id.startsWith("gc_")&&n<=0){const d=e.deposit_payment_intent_id.replace("gc_",""),c=d==="absorbed"?"Acompte payé (reste absorbé par carte cadeau)":"Acompte payé (Carte cadeau "+d+")";[...o.querySelectorAll(".inv-desc")].some(m=>m.value===c)||b(e.id,c,1,-(l/100))}else if(n>0){const d="Acompte payé (Carte cadeau)";[...o.querySelectorAll(".inv-desc")].some(m=>m.value===d)||b(e.id,d,1,-(n/100));const y=l-n;if(y>0){const m="Acompte payé (Stripe)";[...o.querySelectorAll(".inv-desc")].some(T=>T.value===m)||b(e.id,m,1,-(y/100))}}else if(e.deposit_payment_intent_id.startsWith("pi_")||e.deposit_payment_intent_id.startsWith("cs_")){const d="Acompte payé (Stripe)";[...o.querySelectorAll(".inv-desc")].some(y=>y.value===d)||b(e.id,d,1,-(l/100))}}}else o.querySelectorAll(`[data-booking-id="${e.id}"]`).forEach(r=>r.remove());w()}}function b(i,t,e,o){const r=document.getElementById("invLines");if(!r)return;const a=document.createElement("div");a.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",a.setAttribute("data-booking-id",i),a.innerHTML=`
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" aria-label="Retirer la ligne" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,r.appendChild(a)}Object.defineProperty(window,"invoiceFilter",{get(){return p},set(i){p=i}});Object.defineProperty(window,"invoiceType",{get(){return x},set(i){x=i}});S({loadInvoices:_,openInvoiceModal:F,addInvoiceLine:B,updateInvTotals:w,saveInvoice:L,changeInvoiceStatus:N,deleteInvoice:j,downloadInvoicePDF:P,createInvoiceFromBooking:H,invClientChanged:M,invToggleUnbilled:z,issueCreditNote:X,submitCreditNote:V});export{B as addInvoiceLine,N as changeInvoiceStatus,H as createInvoiceFromBooking,j as deleteInvoice,P as downloadInvoicePDF,_ as loadInvoices,F as openInvoiceModal,L as saveInvoice,w as updateInvTotals};
