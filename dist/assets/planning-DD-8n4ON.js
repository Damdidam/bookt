import{a as T,e as y,o as ve,d as ge,l as j,G as S,m as le,s as te}from"./dashboard-CgsIFcrm.js";import{trapFocus as re}from"./focus-trap-C-UMhpsq.js";import{I as _}from"./icons-C4WsiP-A.js";const fe=["Dim","Lun","Mar","Mer","Jeu","Ven","Sam"],ce=["Janvier","Février","Mars","Avril","Mai","Juin","Juillet","Août","Septembre","Octobre","Novembre","Décembre"],K={conge:"C",maladie:"M",formation:"F",autre:"A"},N={conge:"Congé",maladie:"Maladie",formation:"Formation",autre:"Indispo"},F={full:"Journée",am:"Matin",pm:"Après-midi"},ye='fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"',P=e=>`<svg class="gi" viewBox="0 0 24 24" ${ye}>${e}</svg>`,c={sun:_.sun,calendar:_.calendar,plus:_.plus,close:_.x,alertTriangle:_.alertTriangle,checkCircle:_.checkCircle,trash:_.trash,mail:_.mail,clock:_.clock,download:_.download,send:_.send,thermometer:P('<path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0z"/>'),graduationCap:P('<path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c0 2 4 3 6 3s6-1 6-3v-5"/>'),pauseCircle:P('<circle cx="12" cy="12" r="10"/><line x1="10" y1="15" x2="10" y2="9"/><line x1="14" y1="15" x2="14" y2="9"/>'),sunrise:P('<path d="M17 18a5 5 0 0 0-10 0"/><line x1="12" y1="2" x2="12" y2="9"/><line x1="4.22" y1="10.22" x2="5.64" y2="11.64"/><line x1="1" y1="18" x2="3" y2="18"/><line x1="21" y1="18" x2="23" y2="18"/><line x1="18.36" y1="11.64" x2="19.78" y2="10.22"/><line x1="23" y1="22" x2="1" y2="22"/><polyline points="8 6 12 2 16 6"/>'),sunset:P('<path d="M17 18a5 5 0 0 0-10 0"/><line x1="12" y1="9" x2="12" y2="2"/><line x1="4.22" y1="10.22" x2="5.64" y2="11.64"/><line x1="1" y1="18" x2="3" y2="18"/><line x1="21" y1="18" x2="23" y2="18"/><line x1="18.36" y1="11.64" x2="19.78" y2="10.22"/><line x1="23" y1="22" x2="1" y2="22"/><polyline points="16 6 12 10 8 6"/>'),activity:P('<polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>'),flag:P('<path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/>')},he={conge:c.sun,maladie:c.thermometer,formation:c.graduationCap,autre:c.pauseCircle},de={conge:{bg:"var(--blue-bg)",border:"var(--blue-light)",text:"var(--blue)",grad:"linear-gradient(135deg,var(--blue),#1D4ED8)"},maladie:{bg:"var(--red-bg)",border:"var(--red-bg)",text:"var(--red)",grad:"linear-gradient(135deg,var(--red),#B91C1C)"},formation:{bg:"var(--purple-bg)",border:"var(--purple-light)",text:"var(--purple)",grad:"linear-gradient(135deg,var(--purple),#6D28D9)"},autre:{bg:"var(--surface)",border:"var(--border)",text:"var(--text-2)",grad:"linear-gradient(135deg,var(--text-3),var(--text-2))"}};let x,b,M=[],W=[],L={},X={},pe={},Q=[],ne=new Set;function be(){const e=new Date;x=e.getFullYear(),b=e.getMonth()}function q(){return`${x}-${String(b+1).padStart(2,"0")}`}function $e(e){return(e.getDay()+6)%7}function U(e,t){const n=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}-${String(t.getDate()).padStart(2,"0")}`;if(ne.has(n))return!1;const a=pe[e];return!a||a.length===0?!0:a.includes($e(t))}function ie(e){const t=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}-${String(e.getDate()).padStart(2,"0")}`;return ne.has(t)?!1:M.some(n=>U(n.id,e))}function se(e){const t=Q.find(n=>n.date===e);return t?t.name:null}function Se(e,t){const n=(typeof e.date_from=="string"?e.date_from:new Date(e.date_from).toISOString()).slice(0,10),a=(typeof e.date_to=="string"?e.date_to:new Date(e.date_to).toISOString()).slice(0,10),i=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}-${String(t.getDate()).padStart(2,"0")}`;return n===a||i===n?e.period||"full":i===a&&e.period_end||"full"}async function Ze(){be();const e=document.getElementById("contentArea");e.innerHTML='<div class="loading"><div class="spinner"></div></div>',await z()}async function z(){const e=document.getElementById("contentArea");try{const[t,n,a]=await Promise.all([fetch("/api/practitioners",{headers:{Authorization:"Bearer "+T.getToken()}}),fetch("/api/planning/absences?month="+q(),{headers:{Authorization:"Bearer "+T.getToken()}}),fetch("/api/planning/stats?month="+q(),{headers:{Authorization:"Bearer "+T.getToken()}})]),i=await t.json(),o=await n.json(),d=await a.json();M=(i.practitioners||[]).filter(s=>s.is_active),W=o.absences||[],X=d,pe=o.workingDays||{},Q=o.holidays||[],ne=new Set(Q.map(s=>s.date)),Ee(),e.innerHTML=xe()}catch(t){console.error("Planning load error:",t),e.innerHTML=`<div class="empty" style="color:var(--red)">Erreur de chargement: ${y(t.message)}</div>`}}function Ee(){L={},W.forEach(e=>{L[e.practitioner_id]||(L[e.practitioner_id]={});const t=new Date(e.date_from),n=new Date(e.date_to);for(let a=new Date(t);a<=n;a.setDate(a.getDate()+1))if(a.getFullYear()===x&&a.getMonth()===b){const i=a.getDate();L[e.practitioner_id][i]||(L[e.practitioner_id][i]=[]);const o=Se(e,a);L[e.practitioner_id][i].push({type:e.type,id:e.id,period:o,note:e.note})}})}function xe(){const e=new Date(x,b+1,0).getDate(),t=new Date,a=t.getFullYear()===x&&t.getMonth()===b?t.getDate():-1,i=ve.practitioner||"Praticien";if(M.length===0)return`<div class="empty" style="text-align:center;padding:60px 20px">
    ...
    </td>`;for(let v=1;v<=e;v++){const h=new Date(x,b,v),B=v===a,A=`${x}-${String(b+1).padStart(2,"0")}-${String(v).padStart(2,"0")}`,I=se(A),Y=U(l.id,h);let O="plan-day-cell";B&&(O+=" plan-today"),I?O+=" plan-holiday":Y||(O+=" plan-off-day");let k="";const E=u[v]||[];if(I)k=`<span class="plan-holiday-marker" title="${y(I)}">${c.flag}</span>`;else if(!Y)k='<span class="plan-avail-marker">—</span>';else if(E.length===0)k="";else if(E.length===1&&E[0].period==="full"){const $=E[0],f=K[$.type]||"A";k=`<div class="plan-abs-block ${$.type}" title="${y(N[$.type])}${$.note?": "+y($.note):""}" onclick="planAbsClick(event,'${l.id}','${A}','${$.id}')">${f}</div>`}else{const $=E.find(D=>D.period==="am"||D.period==="full"),f=E.find(D=>D.period==="pm"||D.period==="full"&&D!==$);k='<div class="plan-split">',$?k+=`<div class="plan-split-am"><div class="plan-abs-block ${$.type}" style="margin:1px 2px;font-size:.55rem" title="Matin: ${y(N[$.type])}" onclick="planAbsClick(event,'${l.id}','${A}','${$.id}')">${K[$.type]}</div></div>`:k+='<div class="plan-split-am"></div>',f?k+=`<div class="plan-split-pm"><div class="plan-abs-block ${f.type}" style="margin:1px 2px;font-size:.55rem" title="Après-midi: ${y(N[f.type])}" onclick="planAbsClick(event,'${l.id}','${A}','${f.id}')">${K[f.type]}</div></div>`:k+='<div class="plan-split-pm"></div>',k+="</div>"}o+=`<td class="${O}" onclick="planOpenModal('${l.id}','${A}')">${k}</td>`}o+="</tr>"}),o+='<tr class="plan-summary-row"><td style="text-align:left;padding-left:16px;font-size:.72rem">Effectif</td>';for(let l=1;l<=e;l++){const p=new Date(x,b,l);if(!ie(p))o+="<td>—</td>";else{let u=0;M.forEach(v=>{if(!U(v.id,p))return;((L[v.id]||{})[l]||[]).some(A=>A.period==="full")||u++});const r=M.filter(v=>U(v.id,p)).length,m=u>=Math.ceil(r*.7)?"count-good":u>=Math.ceil(r*.4)?"count-warn":"count-bad";o+=`<td class="${m}">${u}</td>`}}return o+="</tr></tbody></table></div></div>",o}function C(e){return e===0?"0":Number.isInteger(e)?String(e):e.toFixed(1).replace(".0","")}function Ae(){b--,b<0&&(b=11,x--),z()}function ke(){b++,b>11&&(b=0,x++),z()}function Te(){const e=new Date;x=e.getFullYear(),b=e.getMonth(),z()}async function _e(){try{const e=await fetch("/api/planning/export?month="+q()+"&format=csv",{headers:{Authorization:"Bearer "+T.getToken()}});if(!e.ok)throw new Error("Erreur export");const t=await e.blob(),n=URL.createObjectURL(t),a=document.createElement("a");a.href=n,a.download=e.headers.get("Content-Disposition")?.match(/filename="?([^"]+)"?/)?.[1]||`planning-${q()}.csv`,document.body.appendChild(a),a.click(),a.remove(),URL.revokeObjectURL(n)}catch{S.toast("Erreur lors de l'export","error")}}function we(){j("planSendOverlay");const e=M.map(n=>`<option value="${n.id}">${y(n.display_name)}${n.email?"":" (pas d'email)"}</option>`).join(""),t=document.createElement("div");t.id="planSendOverlay",t.className="m-overlay open",t.innerHTML=`
      ...
      <div class="m-header">
        ...
        <div class="m-header-content">
          <div class="m-client-hero">
            <div class="m-avatar" style="background:${y(Y)}">${I}</div>
    ...
    </div>`,document.body.appendChild(E),le(E,{noBackdropClose:!0}),re(E,()=>j(E.id)),Z(),(e||s)&&setTimeout(R,100)}function ae(){j("planAbsOverlay"),w=null}function ue(){const e=document.getElementById("planAbsFrom")?.value,t=document.getElementById("planAbsTo")?.value,n=document.getElementById("planEndBlock"),a=document.getElementById("planDateError"),i=document.getElementById("planAbsSaveBtn"),o=e&&t&&e!==t;if(n&&(n.style.display=o?"":"none"),!o&&e){const d=document.getElementById("planAbsTo");d&&(d.value=e)}e&&t&&t<e?(a&&a.classList.add("show"),i&&(i.disabled=!0)):(a&&a.classList.remove("show"),i&&(i.disabled=!1)),Z(),R()}function Be(e){document.querySelectorAll("#planAbsOverlay .m-tab").forEach(t=>t.classList.toggle("active",t.dataset.tab===e)),document.querySelectorAll("#planAbsOverlay .m-panel").forEach(t=>t.classList.remove("active")),document.getElementById(e==="details"?"planPanelDetails":"planPanelLog")?.classList.add("active"),e==="log"&&w&&me(w)}function Ce(e){document.querySelectorAll("#planTypePills .plan-type-pill").forEach(n=>n.className="plan-type-pill"),e.className="plan-type-pill active-"+e.dataset.type;const t=document.getElementById("planModalHeaderBg");t&&(t.style.background=de[e.dataset.type].grad),Z()}function De(e,t){const n=document.getElementById(e==="start"?"planSegStart":"planSegEnd");if(!n)return;n.querySelectorAll(".plan-seg-pill").forEach(o=>o.classList.remove("active")),t.classList.add("active");const a=document.getElementById("planAbsFrom")?.value,i=document.getElementById("planAbsTo")?.value;if(e==="start"&&(!i||a===i)){const o=document.getElementById("planSegEnd");if(o){o.querySelectorAll(".plan-seg-pill").forEach(s=>s.classList.remove("active"));const d=o.querySelector(`.plan-seg-pill[data-seg="${t.dataset.seg}"]`);d&&d.classList.add("active")}}Z(),R()}function Le(e){const t=new Date().toLocaleDateString("en-CA",{timeZone:"Europe/Brussels"}),n=new Date(Date.now()+864e5).toLocaleDateString("en-CA",{timeZone:"Europe/Brussels"}),a=document.getElementById("planAbsFrom"),i=document.getElementById("planAbsTo");!a||!i||(e==="half"?(a.value=t,i.value=t,H("planSegStart","am"),H("planSegEnd","am")):e==="full"?(a.value=t,i.value=t,H("planSegStart","full"),H("planSegEnd","full")):e==="multi"&&(a.value=t,i.value=n,H("planSegStart","full"),H("planSegEnd","full")),ue())}function H(e,t){const n=document.getElementById(e);if(!n)return;n.querySelectorAll(".plan-seg-pill").forEach(i=>i.classList.remove("active"));const a=n.querySelector(`.plan-seg-pill[data-seg="${t}"]`);a&&a.classList.add("active")}function Ie(){const e=document.getElementById("planAbsPrac")?.value,t=M.find(i=>i.id===e);if(!t||w)return;const n=document.getElementById("planModalTitle");n&&(n.textContent=t.display_name);const a=document.querySelector("#planAbsOverlay .m-avatar");a&&(a.style.background=t.color||"var(--blue)",a.textContent=(t.display_name||"??").split(" ").map(i=>i[0]).join("").toUpperCase().slice(0,2))}function Z(){const e=document.getElementById("planModalSubtitle");if(!e)return;const t=document.getElementById("planAbsFrom")?.value,n=document.getElementById("planAbsTo")?.value;if(!t){e.textContent="Planifiez une absence";return}const{period:a,period_end:i}=J(),o=l=>{const p=new Date(l+"T12:00:00");return`${p.getDate()} ${ce[p.getMonth()].toLowerCase()}`},d=!n||t===n,s=l=>l==="am"?"matin":l==="pm"?"après-midi":"journée";if(d){const l=a==="full"?"1 jour":"½ jour";e.textContent=`Le ${o(t)} · ${s(a)} · ${l}`}else{const l=new Date(t+"T12:00:00"),p=new Date(n+"T12:00:00");let g=0;const u=new Date(l);for(;u<=p;)u.getDay()!==0&&g++,u.setDate(u.getDate()+1);(a==="am"||a==="pm")&&(g-=.5),(i==="am"||i==="pm")&&(g-=.5);const r=g%1===0?`${g} jour${g>1?"s":""}`:`${g.toFixed(1).replace(".",",")} jours`,m=a!=="full"?` (${s(a)})`:"",v=i!=="full"?` (${s(i)})`:"";e.textContent=`Du ${o(t)}${m} → ${o(n)}${v} · ${r}`}}function Pe(){return document.querySelector('#planTypePills .plan-type-pill[class*="active-"]')?.dataset.type||"conge"}function J(){const e=document.querySelector("#planSegStart .plan-seg-pill.active"),t=document.querySelector("#planSegEnd .plan-seg-pill.active"),n=e?.dataset.seg||"full",a=t?.dataset.seg||"full",i=document.getElementById("planAbsFrom")?.value,o=document.getElementById("planAbsTo")?.value;return!o||i===o?{period:n,period_end:n}:{period:n,period_end:a}}function Oe(e,t,n,a){e.stopPropagation();const i=W.find(u=>String(u.id)===String(a));if(!i){ee(null,n,a);return}const o=(typeof i.date_from=="string"?i.date_from:new Date(i.date_from).toISOString()).slice(0,10),d=(typeof i.date_to=="string"?i.date_to:new Date(i.date_to).toISOString()).slice(0,10);if(o===d){ee(null,n,a);return}document.getElementById("planCtxMenu")?.remove();const s=document.createElement("div");s.id="planCtxMenu",s.className="plan-ctx-menu";let l=e.clientX,p=e.clientY;s.innerHTML=`
