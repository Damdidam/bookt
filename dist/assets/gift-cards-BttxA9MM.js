import{a as p,d as D,G as a,s as I,l as m,m as w}from"./dashboard-DXKU8dyQ.js";import{i as R,s as G}from"./plan-gate-Badg7tzn.js";import{f as c}from"./format-DoZVPAsZ.js";let g="all",b="",x=[],$=null,E=0,y={};function l(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}let h=!0;async function f(){if(!R()){G(document.getElementById("contentArea"),"Cartes cadeau");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;g!=="all"&&e.set("status",g),b.trim()&&e.set("search",b.trim());const t=await p.get(`/api/gift-cards?${e}`),r=t.gift_cards||[];x=r,h=t.feature_enabled!==!1;const n=t.stats||{};S(o,r,n)}catch(e){o.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function S(o,e,t){e=e||x,t=t||{};let r="";h||(r+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
    ...
    <div class="stat-card"><div class="label">Total vendu</div><div class="val" style="color:var(--primary)">${c(parseInt(t.total_sold_cents||0))}</div><div class="sub">${t.total||0} cartes</div></div>
    <div class="stat-card"><div class="label">Solde restant</div><div class="val" style="color:var(--gold)">${c(parseInt(t.total_balance_cents||0))}</div><div class="sub">non utilisé</div></div>
  ...
  </div>`;const d=[{v:"all",l:"Toutes"},{v:"active",l:"Actives"},{v:"used",l:"Utilisées"},{v:"expired",l:"Expirées"},{v:"cancelled",l:"Annulées"}].map(s=>{const v=g===s.v;return`<button onclick="gcFilter='${s.v}';loadGiftCards()" class="btn-sm${v?" active":""}">${s.l}</button>`}).join("");if(r+=`<div class="card" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 16px">
    ${d}
    <div style="flex:1"></div>
    <input type="text" placeholder="Rechercher par code ou nom..." value="${l(b)}" onkeydown="if(event.key==='Enter'){gcSearch=this.value;loadGiftCards()}" onblur="gcSearch=this.value;loadGiftCards()" style="padding:6px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem;min-width:200px">
    ${h?'<button onclick="openCreateGiftCardModal()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer une carte</button>':'<button disabled title="Activez la fonctionnalité dans les Paramètres" aria-label="Activez la fonctionnalité dans les Paramètres" class="btn-primary btn-sm" style="opacity:.5;cursor:not-allowed"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer une carte</button>'}
      ...
      </tr></thead><tbody>`;const s={active:"var(--green)",used:"var(--text-3)",expired:"var(--gold)",cancelled:"var(--red)"},v={active:"Active",used:"Utilisée",expired:"Expirée",cancelled:"Annulée"};e.forEach(i=>{const C=s[i.status]||"var(--text-4)",M=i.created_at?new Date(i.created_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}):"—",_=i.expires_at?new Date(i.expires_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}):"—",A=i.recipient_name||"—",k=i.recipient_email||"",B=i.status==="active",z=parseInt(i.balance_cents||0);let u="";B?(u+=`<button onclick="openDebitGiftCard('${i.id}')" title="Débiter" aria-label="Débiter" style="background:none;border:none;cursor:pointer;color:var(--primary);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><line x1="5" y1="12" x2="19" y2="12"/></svg></button>`,u+=`<button onclick="refundGiftCard('${i.id}')" title="Rembourser" aria-label="Rembourser" style="background:none;border:none;cursor:pointer;color:var(--blue);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg></button>`,u+=`<button onclick="cancelGiftCard('${i.id}')" title="Annuler" aria-label="Annuler" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg></button>`):i.status==="used"?u+=`<button onclick="refundGiftCard('${i.id}')" title="Rembourser" aria-label="Rembourser" style="background:none;border:none;cursor:pointer;color:var(--blue);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg></button>`:u='<span style="color:var(--text-4)">—</span>',r+=`<tr style="border-bottom:1px solid var(--border-light)">
        <td style="padding:10px 14px"><span style="font-family:monospace;font-weight:600;font-size:.8rem;letter-spacing:.5px">${l(i.code)}</span></td>
        <td style="padding:10px;text-align:right;font-weight:600">${c(i.amount_cents)}</td>
        <td style="padding:10px;text-align:right;font-weight:600;color:${z>0?"var(--green)":"var(--text-4)"}">${c(z)}</td>
        <td style="padding:10px"><div style="font-weight:500">${l(A)}</div>${k?`<div style="font-size:.7rem;color:var(--text-4)">${l(k)}</div>`:""}</td>
        <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${C}12;color:${C};font-weight:600">${v[i.status]||i.status}</span></td>
        <td style="padding:10px;font-size:.78rem;color:var(--text-3)">${M}</td>
        <td style="padding:10px;font-size:.78rem;color:var(--text-3)">${_}</td>
        <td style="padding:10px;text-align:center;white-space:nowrap">${u}</td>
      </tr>`}),r+="</tbody></table></div>"}o.innerHTML=r}function F(){m("gcCreateModal");const e=[25,50,75,100,150,200].map(r=>`<button type="button" onclick="selectGcAmount(${r*100})" class="gc-amount-pill" data-cents="${r*100}" style="padding:8px 16px;border:1px solid var(--border);border-radius:var(--radius-xs);background:var(--surface);color:var(--text-1);font-size:.85rem;font-weight:600;cursor:pointer">${r} €</button>`).join(""),t=document.createElement("div");t.className="m-overlay open",t.id="gcCreateModal",t.innerHTML=`<div class="m-dialog m-md">
  ...
  </div>`,document.body.appendChild(t),w(t,{noBackdropClose:!0})}function j(o){document.getElementById("gcSelectedAmount").value=o,document.querySelectorAll(".gc-amount-pill").forEach(t=>{parseInt(t.dataset.cents)===o?(t.style.background="var(--primary)",t.style.color="#fff",t.style.borderColor="var(--primary)"):(t.style.background="var(--surface)",t.style.color="var(--text-1)",t.style.borderColor="var(--border)")});const e=document.getElementById("gcCustomAmount");e&&!e.matches(":focus")&&(e.value="")}function L(o){clearTimeout($);const e=document.getElementById("gcClientAcDrop");if(e){if(o.length<3){e.style.display="none";return}$=setTimeout(async()=>{const t=++E;try{const r=await fetch(`/api/clients?search=${encodeURIComponent(o)}&limit=8`,{headers:{Authorization:"Bearer "+p.getToken()}});if(t!==E)return;const d=(await r.json()).clients||[];if(d.length===0){e.innerHTML=`<div style="padding:12px;text-align:center;font-size:.8rem;color:var(--text-4)">Aucun client trouvé pour "${l(o)}" — laissez le champ tel quel pour un nouveau destinataire</div>`,e.style.display="block";return}y={},d.forEach(s=>{y[s.id]=s}),e.innerHTML=d.map(s=>{const v=[s.phone,s.email].filter(Boolean).join(" · ");return`<div class="ac-item" onmousedown="event.preventDefault();gcPickClient('${s.id}')"><div class="ac-name">${l(s.full_name)}</div><div class="ac-meta">${l(v)||"—"}</div></div>`}).join(""),e.style.display="block"}catch{e.style.display="none"}},250)}}function T(o){const e=y[o];if(!e)return;const t=document.getElementById("gcRecipientName"),r=document.getElementById("gcRecipientEmail");t&&(t.value=e.full_name||""),r&&(r.value=e.email||"");const n=document.getElementById("gcClientAcDrop");n&&(n.style.display="none")}async function N(){const o=parseInt(document.getElementById("gcSelectedAmount").value),e=document.getElementById("gcRecipientName").value.trim(),t=document.getElementById("gcRecipientEmail").value.trim(),r=document.getElementById("gcMessage").value.trim();if(!o||o<=0){a.toast("Veuillez sélectionner un montant","error");return}if(!e){a.toast("Veuillez saisir le nom du destinataire","error");return}try{await p.post("/api/gift-cards",{amount_cents:o,recipient_name:e,recipient_email:t||void 0,message:r||void 0}),m("gcCreateModal"),a.toast("Carte cadeau créée avec succès","success"),f()}catch(n){a.toast(n.message||"Erreur lors de la création","error")}}function P(o){const e=x.find(r=>r.id===o);if(!e){a.toast("Carte introuvable","error");return}m("gcDebitModal");const t=document.createElement("div");t.className="m-overlay open",t.id="gcDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-body">
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
        ...
        <div style="font-size:1.2rem;font-weight:700;color:var(--green)">${c(e.balance_cents)}</div>
      </div>
      <div style="margin-bottom:14px">
  ...
  </div>`,document.body.appendChild(t),w(t,{noBackdropClose:!0})}async function q(o){const e=document.querySelector("#gcDebitModal .m-btn-primary");if(e?.disabled)return;const t=parseFloat(document.getElementById("gcDebitAmount").value),r=document.getElementById("gcDebitNote").value.trim();if(!t||t<=0){a.toast("Veuillez saisir un montant valide","error");return}e&&(e.disabled=!0,e.textContent="Traitement...");try{await p.post(`/api/gift-cards/${o}/debit`,{amount_cents:Math.round(t*100),note:r||void 0}),m("gcDebitModal"),a.toast("Carte débitée avec succès","success"),f()}catch(n){e&&(e.disabled=!1,e.textContent="Débiter"),a.toast(n.message||"Erreur lors du débit","error")}}async function H(o){const e=x.find(d=>d.id===o);if(!e){a.toast("Carte introuvable","error");return}m("gcRefundModal");const t=parseInt(e.amount_cents||0)-parseInt(e.balance_cents||0);if(t<=0){a.toast("Aucun montant à rembourser","error");return}const r=document.createElement("div");r.className="m-overlay open",r.id="gcRefundModal";const n=e.status==="used";r.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-body">
      ${n?'<div style="background:#FEF3E2;border-left:3px solid #F59E0B;padding:10px 12px;margin-bottom:12px;border-radius:6px;font-size:.82rem;color:#92700C"><strong>Attention :</strong> cette carte a été entièrement utilisée. Vérifiez que le remboursement est légitime avant de continuer.</div>':""}
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
        <div style="display:flex;justify-content:space-between;font-size:.82rem">
          <span style="color:var(--text-3)">Montant initial</span><span style="font-weight:600">${c(e.amount_cents)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-top:4px">
          <span style="color:var(--text-3)">Solde actuel</span><span style="font-weight:600;color:var(--green)">${c(e.balance_cents)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-top:4px">
          <span style="color:var(--text-3)">Montant utilisé</span><span style="font-weight:600">${c(t)}</span>
        </div>
      </div>
      <div style="margin-bottom:14px">
  ...
  </div>`,document.body.appendChild(r),w(r,{noBackdropClose:!0})}async function V(o){const e=document.querySelector("#gcRefundModal .m-btn-primary");if(e?.disabled)return;const t=parseFloat(document.getElementById("gcRefundAmount").value),r=document.getElementById("gcRefundNote").value.trim();if(!t||t<=0){a.toast("Veuillez saisir un montant valide","error");return}e&&(e.disabled=!0,e.textContent="Traitement...");try{const n=await p.post(`/api/gift-cards/${o}/refund`,{amount_cents:Math.round(t*100),note:r||void 0});if(m("gcRefundModal"),n&&n.stripe_refund_cents!=null){const d=(n.stripe_refund_cents/100).toFixed(2).replace(".",",")+" €";a.toast(`Remboursement effectué (${d} Stripe${n.stripe_fees_cents>0?" — frais "+(n.stripe_fees_cents/100).toFixed(2).replace(".",",")+" €":""})`,"success")}else a.toast("Remboursement effectué","success");f()}catch(n){e&&(e.disabled=!1,e.textContent="Rembourser"),a.toast(n.message||"Erreur lors du remboursement","error")}}async function U(o){const e=x.find(r=>r.id===o);if(!e){a.toast("Carte introuvable","error");return}if(await I("Annuler la carte",`Annuler la carte ${e.code} ? Cette action est irréversible.`,"Annuler la carte","danger"))try{await p.patch(`/api/gift-cards/${o}`,{status:"cancelled"}),a.toast("Carte annulée","success"),f()}catch(r){a.toast(r.message||"Erreur lors de l'annulation","error")}}Object.defineProperty(window,"gcFilter",{get(){return g},set(o){g=o},configurable:!0});Object.defineProperty(window,"gcSearch",{get(){return b},set(o){b=o},configurable:!0});D({loadGiftCards:f,openCreateGiftCardModal:F,selectGcAmount:j,submitCreateGiftCard:N,openDebitGiftCard:P,submitDebitGiftCard:q,refundGiftCard:H,submitRefundGiftCard:V,cancelGiftCard:U,gcClientLiveSearch:L,gcPickClient:T});export{f as loadGiftCards,S as renderGiftCards};
