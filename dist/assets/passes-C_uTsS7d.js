import{a as d,d as S,s as z,G as o,k as m,l as h}from"./dashboard-CuqvpENP.js";import{i as B,s as _}from"./plan-gate-Badg7tzn.js";let u="all",v="",g=[];function n(s){return String(s||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function A(s){return((s||0)/100).toFixed(2).replace(".",",")+" €"}let f=!0;async function x(){if(!B()){_(document.getElementById("contentArea"),"Abonnements");return}const s=document.getElementById("contentArea");s.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;u!=="all"&&t.set("status",u),v.trim()&&t.set("search",v.trim());const r=await d.get(`/api/passes?${t}`),e=r.passes||[];g=e,f=r.feature_enabled!==!1;const i=r.stats||{};I(s,e,i)}catch(t){s.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${n(t.message)}</div>`}}function I(s,t,r){t=t||g,r=r||{};let e="";f||(e+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
    ...
    </div>`),e+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    <div class="stat-card"><div class="label">Total vendus</div><div class="val" style="color:var(--primary)">${r.total_sessions_sold||0} séances</div><div class="sub">${r.total_count||0} passes</div></div>
    <div class="stat-card"><div class="label">Séances restantes</div><div class="val" style="color:var(--gold)">${r.total_sessions_remaining||0}</div><div class="sub">non utilisées</div></div>
    <div class="stat-card" style="cursor:pointer" onclick="passFilter='active';loadPasses()"><div class="label">Passes actifs</div><div class="val" style="color:var(--green)">${r.active_count||0}</div><div class="sub">en circulation</div></div>
    <div class="stat-card" style="cursor:pointer" onclick="passFilter='used';loadPasses()"><div class="label">Passes utilisés</div><div class="val" style="color:var(--text-3)">${r.used_count||0}</div><div class="sub">séances épuisées</div></div>
    ...
    ${f?'<button onclick="openCreatePass()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>':'<button disabled title="Activez la fonctionnalité dans les Paramètres" class="btn-primary btn-sm" style="opacity:.5;cursor:not-allowed"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>'}
      ...
      </tr></thead><tbody>`;const l={active:"var(--green)",used:"var(--text-3)",expired:"var(--gold)",cancelled:"var(--red)"},c={active:"Actif",used:"Utilisé",expired:"Expiré",cancelled:"Annulé"};t.forEach(a=>{const y=l[a.status]||"var(--text-4)",$=a.expires_at?new Date(a.expires_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}):"—",P=a.buyer_name||"—",w=a.buyer_email||"",E=a.service_name||"—",C=a.status==="active",k=parseInt(a.sessions_remaining||0),M=parseInt(a.sessions_total||0);let p="";C?(p+=`<button onclick="openDebitPass('${a.id}')" title="Débiter 1 séance" style="background:none;border:none;cursor:pointer;color:var(--primary);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><line x1="5" y1="12" x2="19" y2="12"/></svg></button>`,p+=`<button onclick="refundPass('${a.id}')" title="Rembourser 1 séance" style="background:none;border:none;cursor:pointer;color:var(--blue);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg></button>`,p+=`<button onclick="cancelPass('${a.id}')" title="Annuler" style="background:none;border:none;cursor:pointer;color:var(--gold);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg></button>`):a.status==="used"&&(p+=`<button onclick="refundPass('${a.id}')" title="Rembourser 1 séance" style="background:none;border:none;cursor:pointer;color:var(--blue);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg></button>`),p+=`<button onclick="deletePass('${a.id}','${n(a.code)}')" title="Supprimer" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:.78rem;padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>`,e+=`<tr style="border-bottom:1px solid var(--border-light)">
        <td style="padding:10px 14px"><span style="font-family:monospace;font-weight:600;font-size:.8rem;letter-spacing:.5px">${n(a.code)}</span></td>
        <td style="padding:10px"><div style="font-weight:500">${n(P)}</div>${w?`<div style="font-size:.7rem;color:var(--text-4)">${n(w)}</div>`:""}</td>
        <td style="padding:10px;color:var(--text-2)">${n(E)}</td>
        <td style="padding:10px;text-align:center;font-weight:600"><span style="color:${k>0?"var(--green)":"var(--text-4)"}">${k}</span><span style="color:var(--text-4);font-weight:400">/${M}</span></td>
        <td style="padding:10px;font-size:.78rem;color:var(--text-3)">${$}</td>
        <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${y}12;color:${y};font-weight:600">${c[a.status]||a.status}</span></td>
        <td style="padding:10px;text-align:center;white-space:nowrap">${p}</td>
      </tr>`}),e+="</tbody></table></div>"}s.innerHTML=e}async function R(){m("passCreateModal");let s=[];try{const e=await d.get("/api/services");s=(e.services||e||[]).filter(i=>i.is_active!==!1)}catch(e){console.error("[passes] fetch services failed",e),o.toast("Impossible de charger les prestations: "+e.message,"error")}const t=s.map(e=>`<option value="${n(e.id)}">${n(e.name)}${e.price_cents!=null?" — "+A(e.price_cents):""}</option>`).join(""),r=document.createElement("div");r.className="m-overlay open",r.id="passCreateModal",r.innerHTML=`<div class="m-dialog m-md">
  ...
  </div>`,document.body.appendChild(r),h(r,{noBackdropClose:!0})}function j(){const s=document.getElementById("passServiceSelect"),t=document.getElementById("passManualNameRow");s.value?t.style.display="none":t.style.display=""}async function D(){const s=document.getElementById("passServiceSelect").value.trim(),t=document.getElementById("passNameManual").value.trim(),r=parseInt(document.getElementById("passSessionsTotal").value),e=parseFloat(document.getElementById("passAmountEur").value),i=document.getElementById("passBuyerName").value.trim(),b=document.getElementById("passBuyerEmail").value.trim(),l=document.getElementById("passExpiresAt").value,c=document.getElementById("passServiceSelect"),a=s?c.options[c.selectedIndex].text.split(" — ")[0]:t;if(!a){o.toast("Veuillez choisir une prestation ou saisir un nom","error");return}if(!r||r<1){o.toast("Veuillez saisir un nombre de séances valide","error");return}if(!e||e<=0){o.toast("Veuillez saisir un prix valide","error");return}if(!i){o.toast("Veuillez saisir le nom du client","error");return}try{await d.post("/api/passes",{service_id:s||void 0,name:a,sessions_total:r,price_cents:Math.round(e*100),buyer_name:i,buyer_email:b||void 0,expires_at:l||void 0}),m("passCreateModal"),o.toast("Pass créé avec succès","success"),x()}catch(y){o.toast(y.message||"Erreur lors de la création","error")}}function N(s){const t=g.find(i=>i.id===s);if(!t){o.toast("Pass introuvable","error");return}m("passDebitModal");const r=parseInt(t.sessions_remaining||0),e=document.createElement("div");e.className="m-overlay open",e.id="passDebitModal",e.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-body">
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
        ...
        <div style="font-size:1.2rem;font-weight:700;color:var(--green)">${r} / ${parseInt(t.sessions_total||0)}</div>
    ...
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="debitPass('${t.id}')" ${r<=0?"disabled":""}>Débiter 1 séance</button>
    </div>
  </div>`,document.body.appendChild(e),h(e,{noBackdropClose:!0})}async function F(s){try{await d.post(`/api/passes/${s}/debit`),m("passDebitModal"),o.toast("Séance débitée avec succès","success"),x()}catch(t){o.toast(t.message||"Erreur lors du débit","error")}}async function L(s){const t=g.find(b=>b.id===s);if(!t){o.toast("Pass introuvable","error");return}m("passRefundModal");const r=parseInt(t.sessions_remaining||0),e=parseInt(t.sessions_total||0),i=document.createElement("div");i.className="m-overlay open",i.id="passRefundModal",i.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-body">
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
        ...
        <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-top:4px">
          <span style="color:var(--text-3)">Séances utilisées</span><span style="font-weight:600">${e-r}</span>
        </div>
      </div>
  ...
  </div>`,document.body.appendChild(i),h(i,{noBackdropClose:!0})}async function T(s){try{await d.post(`/api/passes/${s}/refund`),m("passRefundModal"),o.toast("Séance remboursée avec succès","success"),x()}catch(t){o.toast(t.message||"Erreur lors du remboursement","error")}}async function H(s){const t=g.find(e=>e.id===s);if(!t){o.toast("Pass introuvable","error");return}if(await z("Annuler le pass",`Annuler le pass ${t.code} ? Cette action est irréversible.`,"Annuler le pass","danger"))try{await d.patch(`/api/passes/${s}`,{status:"cancelled"}),o.toast("Pass annulé","success"),x()}catch(e){o.toast(e.message||"Erreur lors de l'annulation","error")}}async function V(s,t){if(await z("Supprimer le pass",`Supprimer définitivement le pass ${t} ? Cette action est irréversible.`,"Supprimer","danger"))try{await d.delete(`/api/passes/${s}`),o.toast("Pass supprimé","success"),x()}catch(e){o.toast(e.message||"Erreur lors de la suppression","error")}}function q(s){u=s}function U(s){v=s}Object.defineProperty(window,"passFilter",{get(){return u},set(s){u=s},configurable:!0});Object.defineProperty(window,"passSearch",{get(){return v},set(s){v=s},configurable:!0});S({loadPasses:x,openCreatePass:R,submitCreatePass:D,passServiceChanged:j,openDebitPass:N,debitPass:F,refundPass:L,submitRefundPass:T,cancelPass:H,deletePass:V,setPassFilter:q,passSearchInput:U});export{x as loadPasses,I as renderPasses};
