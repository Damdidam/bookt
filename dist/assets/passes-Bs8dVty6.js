import{a as m,d as A,s as S,G as o,l as d,m as w}from"./dashboard-i0EUwuoi.js";import{i as F,s as I}from"./plan-gate-Badg7tzn.js";import{releaseFocus as j,trapFocus as k}from"./focus-trap-C-UMhpsq.js";import{f as D}from"./format-DoZVPAsZ.js";import{r as T}from"./pagination-DxEIci4p.js";let p="all",u="",x=0;const z=50;let h={total_count:0,limit:z,offset:0},g=[];function l(s){return String(s||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}let $=!0;async function v(){if(!F()){I(document.getElementById("contentArea"),"Abonnements");return}const s=document.getElementById("contentArea");s.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;p!=="all"&&e.set("status",p),u.trim()&&e.set("search",u.trim()),e.set("limit",String(z)),e.set("offset",String(x));const a=await m.get(`/api/passes?${e}`),t=a.passes||[];g=t,$=a.feature_enabled!==!1,h=a.pagination||{total_count:t.length,limit:z,offset:x};const n=a.stats||{};N(s,t,n)}catch(e){s.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function N(s,e,a){e=e||g,a=a||{};let t="";$||(t+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
    ...
    ${$?'<button onclick="openCreatePass()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>':'<button disabled title="Activez la fonctionnalité dans les Paramètres" aria-label="Activez la fonctionnalité dans les Paramètres" class="btn-primary btn-sm" style="opacity:.5;cursor:not-allowed"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>'}
      ...
      </tr>`}),t+="</tbody></table></div>",t+=T({total:h.total_count,limit:h.limit,offset:h.offset,onPage:"passesGoToPage",label:"pass"})}s.innerHTML=t}async function L(){d("passCreateModal");let s=[];try{const t=await m.get("/api/services");s=(t.services||t||[]).filter(n=>n.is_active!==!1)}catch(t){console.error("[passes] fetch services failed",t),o.toast("Impossible de charger les prestations: "+t.message,"error")}const e=s.map(t=>`<option value="${l(t.id)}">${l(t.name)}${t.price_cents!=null?" — "+D(t.price_cents):""}</option>`).join(""),a=document.createElement("div");a.className="m-overlay open",a.id="passCreateModal",a.innerHTML=`<div class="m-dialog m-md">
  ...
  </div>`,document.body.appendChild(a),w(a,{noBackdropClose:!0}),k(a,()=>d(a.id))}function q(){const s=document.getElementById("passServiceSelect"),e=document.getElementById("passManualNameRow");s.value?e.style.display="none":e.style.display=""}async function H(){const s=document.getElementById("passServiceSelect").value.trim(),e=document.getElementById("passNameManual").value.trim(),a=parseInt(document.getElementById("passSessionsTotal").value),t=parseFloat(document.getElementById("passAmountEur").value),n=document.getElementById("passBuyerName").value.trim(),f=document.getElementById("passBuyerEmail").value.trim(),c=document.getElementById("passExpiresAt").value,i=document.getElementById("passServiceSelect"),r=s?i.options[i.selectedIndex].text.split(" — ")[0]:e;if(!r){o.toast("Veuillez choisir une prestation ou saisir un nom","error");return}if(!a||a<1){o.toast("Veuillez saisir un nombre de séances valide","error");return}if(!t||t<=0){o.toast("Veuillez saisir un prix valide","error");return}if(!n){o.toast("Veuillez saisir le nom du client","error");return}try{await m.post("/api/passes",{service_id:s||void 0,name:r,sessions_total:a,price_cents:Math.round(t*100),buyer_name:n,buyer_email:f||void 0,expires_at:c||void 0}),d("passCreateModal"),o.toast("Pass créé avec succès","success"),v()}catch(y){o.toast(y.message||"Erreur lors de la création","error")}}function V(s){const e=g.find(n=>n.id===s);if(!e){o.toast("Pass introuvable","error");return}d("passDebitModal");const a=parseInt(e.sessions_remaining||0),t=document.createElement("div");t.className="m-overlay open",t.id="passDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(t),w(t,{noBackdropClose:!0}),k(t,()=>d(t.id))}async function G(s){const e=document.querySelector("#passDebitModal .m-btn-primary");if(!e?.disabled){e&&(e.disabled=!0,e.textContent="Traitement...");try{await m.post(`/api/passes/${s}/debit`),d("passDebitModal"),o.toast("Séance débitée avec succès","success"),v()}catch(a){e&&(e.disabled=!1,e.textContent="Débiter 1 séance"),o.toast(a.message||"Erreur lors du débit","error")}}}async function O(s){const e=g.find(f=>f.id===s);if(!e){o.toast("Pass introuvable","error");return}d("passRefundModal");const a=parseInt(e.sessions_remaining||0),t=parseInt(e.sessions_total||0),n=document.createElement("div");n.className="m-overlay open",n.id="passRefundModal",n.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(n),w(n,{noBackdropClose:!0}),k(n,()=>d(n.id))}async function U(s){const e=document.querySelector("#passRefundModal .m-btn-primary");if(!e?.disabled){e&&(e.disabled=!0,e.textContent="Traitement...");try{await m.post(`/api/passes/${s}/refund`),d("passRefundModal"),o.toast("Séance remboursée avec succès","success"),v()}catch(a){e&&(e.disabled=!1,e.textContent="Rembourser 1 séance"),o.toast(a.message||"Erreur lors du remboursement","error")}}}async function Z(s){const e=g.find(r=>r.id===s);if(!e){o.toast("Pass introuvable","error");return}const a=parseInt(e.sessions_remaining||0),t=parseInt(e.sessions_total||0),n=t>0?Math.round((e.price_cents||0)/t):0,c=(a*n/100).toFixed(2).replace(".",",")+" €",i=document.createElement("div");i.className="m-overlay open",i.id="passFullRefundModal",i.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(i),w(i,{noBackdropClose:!0}),k(i,()=>d("passFullRefundModal"))}async function J(s){const e=document.querySelector("#passFullRefundModal .m-btn-danger");if(!e?.disabled){e&&(e.disabled=!0,e.textContent="Traitement...");try{const a=document.getElementById("fullRefundReason")?.value?.trim()||null,t=await m.post(`/api/passes/${s}/refund-full`,{reason:a});if(j(),d("passFullRefundModal"),t.stripe_refund_cents!=null){const n=(t.stripe_refund_cents/100).toFixed(2).replace(".",",")+" €";o.toast(`Pass remboursé (${n} Stripe${t.stripe_fees_cents>0?" — frais "+(t.stripe_fees_cents/100).toFixed(2).replace(".",",")+" €":""})`,"success")}else o.toast("Pass annulé (pas de paiement Stripe à rembourser)","success");v()}catch(a){e&&(e.disabled=!1,e.textContent="Rembourser et annuler"),o.toast(a.message||"Erreur lors du remboursement total","error")}}}async function K(s){const e=g.find(t=>t.id===s);if(!e){o.toast("Pass introuvable","error");return}if(await S("Annuler le pass",`Annuler le pass ${e.code} ? Cette action est irréversible.`,"Annuler le pass","danger"))try{await m.patch(`/api/passes/${s}`,{status:"cancelled"}),o.toast("Pass annulé","success"),v()}catch(t){o.toast(t.message||"Erreur lors de l'annulation","error")}}async function Q(s,e){if(await S("Supprimer le pass",`Supprimer définitivement le pass ${e} ? Cette action est irréversible.`,"Supprimer","danger"))try{await m.delete(`/api/passes/${s}`),o.toast("Pass supprimé","success"),v()}catch(t){o.toast(t.message||"Erreur lors de la suppression","error")}}function W(s){p!==s&&(x=0),p=s}function X(s){u!==s&&(x=0),u=s}Object.defineProperty(window,"passFilter",{get(){return p},set(s){p!==s&&(x=0),p=s},configurable:!0});Object.defineProperty(window,"passSearch",{get(){return u},set(s){u!==s&&(x=0),u=s},configurable:!0});function Y(s){x=Math.max(0,parseInt(s)||0),v()}A({loadPasses:v,openCreatePass:L,submitCreatePass:H,passServiceChanged:q,openDebitPass:V,debitPass:G,refundPass:O,submitRefundPass:U,fullRefundPass:Z,submitFullRefundPass:J,cancelPass:K,deletePass:Q,setPassFilter:W,passSearchInput:X,passesGoToPage:Y});export{v as loadPasses,N as renderPasses};
