import{a as v,d as E,G as i,s as A,k as c,l as f}from"./dashboard-bIYtn9Go.js";import{i as B,s as G}from"./plan-gate-Badg7tzn.js";let u="all",m="",x=[];function s(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function n(o){return((o||0)/100).toFixed(2).replace(".",",")+" €"}async function g(){if(!B()){G(document.getElementById("contentArea"),"Cartes cadeau");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;u!=="all"&&e.set("status",u),m.trim()&&e.set("search",m.trim());const t=await v.get(`/api/gift-cards?${e}`),r=t.gift_cards||[];x=r;const l=t.stats||{};_(o,r,l)}catch(e){o.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${s(e.message)}</div>`}}function _(o,e,t){e=e||x,t=t||{};let r="";r+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
  ...
  </div>`,document.body.appendChild(t),f(t,{noBackdropClose:!0})}function I(o){document.getElementById("gcSelectedAmount").value=o,document.querySelectorAll(".gc-amount-pill").forEach(t=>{parseInt(t.dataset.cents)===o?(t.style.background="var(--primary)",t.style.color="#fff",t.style.borderColor="var(--primary)"):(t.style.background="var(--surface)",t.style.color="var(--text-1)",t.style.borderColor="var(--border)")});const e=document.getElementById("gcCustomAmount");e&&!e.matches(":focus")&&(e.value="")}async function R(){const o=parseInt(document.getElementById("gcSelectedAmount").value),e=document.getElementById("gcRecipientName").value.trim(),t=document.getElementById("gcRecipientEmail").value.trim(),r=document.getElementById("gcMessage").value.trim();if(!o||o<=0){i.toast("Veuillez sélectionner un montant","error");return}if(!e){i.toast("Veuillez saisir le nom du destinataire","error");return}try{await v.post("/api/gift-cards",{amount_cents:o,recipient_name:e,recipient_email:t||void 0,message:r||void 0}),c("gcCreateModal"),i.toast("Carte cadeau créée avec succès","success"),g()}catch(l){i.toast(l.message||"Erreur lors de la création","error")}}function S(o){const e=x.find(r=>r.id===o);if(!e){i.toast("Carte introuvable","error");return}c("gcDebitModal");const t=document.createElement("div");t.className="m-overlay open",t.id="gcDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(t),f(t,{noBackdropClose:!0})}async function j(o){const e=parseFloat(document.getElementById("gcDebitAmount").value),t=document.getElementById("gcDebitNote").value.trim();if(!e||e<=0){i.toast("Veuillez saisir un montant valide","error");return}try{await v.post(`/api/gift-cards/${o}/debit`,{amount_cents:Math.round(e*100),note:t||void 0}),c("gcDebitModal"),i.toast("Carte débitée avec succès","success"),g()}catch(r){i.toast(r.message||"Erreur lors du débit","error")}}async function N(o){const e=x.find(l=>l.id===o);if(!e){i.toast("Carte introuvable","error");return}c("gcRefundModal");const t=parseInt(e.amount_cents||0)-parseInt(e.balance_cents||0);if(t<=0){i.toast("Aucun montant à rembourser","error");return}const r=document.createElement("div");r.className="m-overlay open",r.id="gcRefundModal",r.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(r),f(r,{noBackdropClose:!0})}async function F(o){const e=parseFloat(document.getElementById("gcRefundAmount").value),t=document.getElementById("gcRefundNote").value.trim();if(!e||e<=0){i.toast("Veuillez saisir un montant valide","error");return}try{await v.post(`/api/gift-cards/${o}/refund`,{amount_cents:Math.round(e*100),note:t||void 0}),c("gcRefundModal"),i.toast("Remboursement effectué","success"),g()}catch(r){i.toast(r.message||"Erreur lors du remboursement","error")}}async function L(o){const e=x.find(r=>r.id===o);if(!e){i.toast("Carte introuvable","error");return}if(await A("Annuler la carte",`Annuler la carte ${e.code} ? Cette action est irréversible.`,"Annuler la carte","danger"))try{await v.patch(`/api/gift-cards/${o}`,{status:"cancelled"}),i.toast("Carte annulée","success"),g()}catch(r){i.toast(r.message||"Erreur lors de l'annulation","error")}}Object.defineProperty(window,"gcFilter",{get(){return u},set(o){u=o},configurable:!0});Object.defineProperty(window,"gcSearch",{get(){return m},set(o){m=o},configurable:!0});E({loadGiftCards:g,openCreateGiftCardModal:D,selectGcAmount:I,submitCreateGiftCard:R,openDebitGiftCard:S,submitDebitGiftCard:j,refundGiftCard:N,submitRefundGiftCard:F,cancelGiftCard:L});export{g as loadGiftCards,_ as renderGiftCards};
