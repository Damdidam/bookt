import{s as I,a as y,G as u,m as M,l as w,d as A}from"./dashboard-rfa-74At.js";import{i as D,s as L}from"./plan-gate-Badg7tzn.js";import{trapFocus as z,releaseFocus as F}from"./focus-trap-C-UMhpsq.js";import{f}from"./format-DoZVPAsZ.js";import{r as N}from"./pagination-DxEIci4p.js";let p="all",g="all",k=0;const B=50;let _=[];function v(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function h(){if(!D()){L(document.getElementById("contentArea"),"Facturation");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;p!=="all"&&t.set("status",p),g!=="all"&&t.set("type",g),t.set("limit",String(B)),t.set("offset",String(k));const e=await y.get(`/api/invoices?${t}`),n=e.invoices||[],r=e.stats||{},a=e.pagination||{total_count:n.length,limit:B,offset:k};let s="";s+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
      ...
      <select onchange="invoiceFilter=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        ...
        <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
          ...
          <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${$}12;color:${$};font-weight:600">${d[i.status]||i.status}</span></td>
        ...
        </tr>`}),s+="</tbody></table></div>",s+=N({total:a.total_count,limit:a.limit,offset:a.offset,onPage:"invoicesGoToPage",label:"factures"})),y.getBusiness()?.settings?.iban||(s+=`<div class="card" style="background:var(--gold-bg);border:1px solid #E0D4A8;margin-top:14px;padding:14px 18px">
      ...
      </div>`),o.innerHTML=s}catch(t){o.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${v(t.message)}</div>`}}async function j(o="invoice",t={}){let e=[];try{const s=await y.get("/api/clients");e=s.clients||s||[]}catch(s){console.warn("Impossible de charger les clients pour la facture:",s.message)}const n=o==="quote",r=n?"Nouveau devis":"Nouvelle facture",a=document.createElement("div");if(a.className="m-overlay open",a.id="invModal",a.innerHTML=`<div class="m-dialog m-md">
    ...
    <div style="font-size:1rem;font-weight:700;color:var(--text);margin-top:4px">Total TTC : ${f(a)}</div>`}async function P(o){const t=document.getElementById("invClient")?.value;if(!t){u.toast("Sélectionnez un client","error");return}const e=document.getElementById("invLines"),n=[];if(e.querySelectorAll(":scope > div").forEach(r=>{const a=r.querySelector(".inv-desc")?.value?.trim(),s=parseFloat(r.querySelector(".inv-qty")?.value||1),l=parseFloat(r.querySelector(".inv-price")?.value||0),i=r.getAttribute("data-booking-id")||void 0;a&&l!==0&&n.push({description:a,quantity:s,unit_price_cents:Math.round(l*100),booking_id:i})}),n.length===0){u.toast("Ajoutez au moins une ligne","error");return}try{const r={client_id:t,type:o||"invoice",items:n,vat_rate:parseFloat(document.getElementById("invVat")?.value||21),due_days:parseInt(document.getElementById("invDueDays")?.value||30),client_bce:document.getElementById("invClientBce")?.value?.trim()||void 0,notes:document.getElementById("invNotes")?.value?.trim()||void 0},a=await y.post("/api/invoices",r);document.getElementById("invModal")._dirtyGuard?.markClean(),w("invModal"),u.toast(o==="quote"?"Devis créé !":"Facture créée !","success"),h()}catch(r){u.toast(r.message||"Erreur","error")}}async function R(o,t){if(await I("Changer le statut",{sent:"Marquer comme envoyée ?",paid:"Marquer comme payée ?"}[t]||`Changer le statut en "${t}" ?`,"Confirmer"))try{await y.patch(`/api/invoices/${o}/status`,{status:t}),u.toast("Statut mis à jour","success"),h()}catch(r){u.toast(r.message||"Erreur","error")}}async function V(o){if(await I("Supprimer le brouillon","Supprimer ce brouillon ?","Supprimer","danger"))try{await y.delete(`/api/invoices/${o}`),u.toast("Brouillon supprimé","success"),h()}catch(e){u.toast(e.message||"Erreur","error")}}async function X(o){try{const t=localStorage.getItem("genda_token"),e=await fetch(`/api/invoices/${o}/pdf`,{headers:{Authorization:"Bearer "+t}});if(!e.ok)throw new Error("Erreur téléchargement PDF");const n=await e.blob(),r=URL.createObjectURL(n),a=document.createElement("a");a.href=r,a.download=`invoice-${o}.pdf`,document.body.appendChild(a),a.click(),setTimeout(()=>{document.body.removeChild(a),URL.revokeObjectURL(r)},100)}catch(t){u.toast(t.message||"Erreur téléchargement","error")}}async function H(o,t){const e=document.createElement("div");e.className="m-overlay open",e.id="creditNoteModal",e.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" aria-label="Retirer la ligne" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,r.appendChild(a)}Object.defineProperty(window,"invoiceFilter",{get(){return p},set(o){p!==o&&(k=0),p=o}});Object.defineProperty(window,"invoiceType",{get(){return g},set(o){g!==o&&(k=0),g=o}});function O(o){k=Math.max(0,parseInt(o)||0),h()}A({loadInvoices:h,openInvoiceModal:j,addInvoiceLine:T,updateInvTotals:C,saveInvoice:P,changeInvoiceStatus:R,deleteInvoice:V,downloadInvoicePDF:X,createInvoiceFromBooking:G,invClientChanged:S,invToggleUnbilled:q,issueCreditNote:H,submitCreditNote:U,invoicesGoToPage:O});export{T as addInvoiceLine,R as changeInvoiceStatus,G as createInvoiceFromBooking,V as deleteInvoice,X as downloadInvoicePDF,h as loadInvoices,j as openInvoiceModal,P as saveInvoice,C as updateInvTotals};
