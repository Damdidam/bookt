import{a as p,d as D,G as i,s as I,k as m,l as w}from"./dashboard-B8L_NV6d.js";import{i as G,s as R}from"./plan-gate-Badg7tzn.js";import{f as d}from"./format-DoZVPAsZ.js";let g="all",x="",b=[],$=null,E=0,y={};function l(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}let h=!0;async function f(){if(!G()){R(document.getElementById("contentArea"),"Cartes cadeau");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;g!=="all"&&e.set("status",g),x.trim()&&e.set("search",x.trim());const t=await p.get(`/api/gift-cards?${e}`),r=t.gift_cards||[];b=r,h=t.feature_enabled!==!1;const s=t.stats||{};S(o,r,s)}catch(e){o.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function S(o,e,t){e=e||b,t=t||{};let r="";h||(r+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
  ...
  </div>`,document.body.appendChild(t),w(t,{noBackdropClose:!0})}function j(o){document.getElementById("gcSelectedAmount").value=o,document.querySelectorAll(".gc-amount-pill").forEach(t=>{parseInt(t.dataset.cents)===o?(t.style.background="var(--primary)",t.style.color="#fff",t.style.borderColor="var(--primary)"):(t.style.background="var(--surface)",t.style.color="var(--text-1)",t.style.borderColor="var(--border)")});const e=document.getElementById("gcCustomAmount");e&&!e.matches(":focus")&&(e.value="")}function L(o){clearTimeout($);const e=document.getElementById("gcClientAcDrop");if(e){if(o.length<3){e.style.display="none";return}$=setTimeout(async()=>{const t=++E;try{const r=await fetch(`/api/clients?search=${encodeURIComponent(o)}&limit=8`,{headers:{Authorization:"Bearer "+p.getToken()}});if(t!==E)return;const c=(await r.json()).clients||[];if(c.length===0){e.innerHTML=`<div style="padding:12px;text-align:center;font-size:.8rem;color:var(--text-4)">Aucun client trouvé pour "${l(o)}" — laissez le champ tel quel pour un nouveau destinataire</div>`,e.style.display="block";return}y={},c.forEach(a=>{y[a.id]=a}),e.innerHTML=c.map(a=>{const v=[a.phone,a.email].filter(Boolean).join(" · ");return`<div class="ac-item" onmousedown="event.preventDefault();gcPickClient('${a.id}')"><div class="ac-name">${l(a.full_name)}</div><div class="ac-meta">${l(v)||"—"}</div></div>`}).join(""),e.style.display="block"}catch{e.style.display="none"}},250)}}function T(o){const e=y[o];if(!e)return;const t=document.getElementById("gcRecipientName"),r=document.getElementById("gcRecipientEmail");t&&(t.value=e.full_name||""),r&&(r.value=e.email||"");const s=document.getElementById("gcClientAcDrop");s&&(s.style.display="none")}async function N(){const o=parseInt(document.getElementById("gcSelectedAmount").value),e=document.getElementById("gcRecipientName").value.trim(),t=document.getElementById("gcRecipientEmail").value.trim(),r=document.getElementById("gcMessage").value.trim();if(!o||o<=0){i.toast("Veuillez sélectionner un montant","error");return}if(!e){i.toast("Veuillez saisir le nom du destinataire","error");return}try{await p.post("/api/gift-cards",{amount_cents:o,recipient_name:e,recipient_email:t||void 0,message:r||void 0}),m("gcCreateModal"),i.toast("Carte cadeau créée avec succès","success"),f()}catch(s){i.toast(s.message||"Erreur lors de la création","error")}}function P(o){const e=b.find(r=>r.id===o);if(!e){i.toast("Carte introuvable","error");return}m("gcDebitModal");const t=document.createElement("div");t.className="m-overlay open",t.id="gcDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(t),w(t,{noBackdropClose:!0})}async function H(o){const e=parseFloat(document.getElementById("gcDebitAmount").value),t=document.getElementById("gcDebitNote").value.trim();if(!e||e<=0){i.toast("Veuillez saisir un montant valide","error");return}try{await p.post(`/api/gift-cards/${o}/debit`,{amount_cents:Math.round(e*100),note:t||void 0}),m("gcDebitModal"),i.toast("Carte débitée avec succès","success"),f()}catch(r){i.toast(r.message||"Erreur lors du débit","error")}}async function V(o){const e=b.find(c=>c.id===o);if(!e){i.toast("Carte introuvable","error");return}m("gcRefundModal");const t=parseInt(e.amount_cents||0)-parseInt(e.balance_cents||0);if(t<=0){i.toast("Aucun montant à rembourser","error");return}const r=document.createElement("div");r.className="m-overlay open",r.id="gcRefundModal";const s=e.status==="used";r.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-body">
      ${s?'<div style="background:#FEF3E2;border-left:3px solid #F59E0B;padding:10px 12px;margin-bottom:12px;border-radius:6px;font-size:.82rem;color:#92700C"><strong>Attention :</strong> cette carte a été entièrement utilisée. Vérifiez que le remboursement est légitime avant de continuer.</div>':""}
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
  ...
  </div>`,document.body.appendChild(r),w(r,{noBackdropClose:!0})}async function q(o){const e=parseFloat(document.getElementById("gcRefundAmount").value),t=document.getElementById("gcRefundNote").value.trim();if(!e||e<=0){i.toast("Veuillez saisir un montant valide","error");return}try{await p.post(`/api/gift-cards/${o}/refund`,{amount_cents:Math.round(e*100),note:t||void 0}),m("gcRefundModal"),i.toast("Remboursement effectué","success"),f()}catch(r){i.toast(r.message||"Erreur lors du remboursement","error")}}async function U(o){const e=b.find(r=>r.id===o);if(!e){i.toast("Carte introuvable","error");return}if(await I("Annuler la carte",`Annuler la carte ${e.code} ? Cette action est irréversible.`,"Annuler la carte","danger"))try{await p.patch(`/api/gift-cards/${o}`,{status:"cancelled"}),i.toast("Carte annulée","success"),f()}catch(r){i.toast(r.message||"Erreur lors de l'annulation","error")}}Object.defineProperty(window,"gcFilter",{get(){return g},set(o){g=o},configurable:!0});Object.defineProperty(window,"gcSearch",{get(){return x},set(o){x=o},configurable:!0});D({loadGiftCards:f,openCreateGiftCardModal:F,selectGcAmount:j,submitCreateGiftCard:N,openDebitGiftCard:P,submitDebitGiftCard:H,refundGiftCard:V,submitRefundGiftCard:q,cancelGiftCard:U,gcClientLiveSearch:L,gcPickClient:T});export{f as loadGiftCards,S as renderGiftCards};
