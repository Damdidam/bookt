import{s as k,a as g,G as u,m as E,l as $,d as T}from"./dashboard-DXKU8dyQ.js";import{i as S,s as q}from"./plan-gate-Badg7tzn.js";import{r as A,t as D}from"./focus-trap-Vw_KJRtZ.js";import{f}from"./format-DoZVPAsZ.js";let p="all",x="all",h=[];function v(i){return String(i||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function _(){if(!S()){q(document.getElementById("contentArea"),"Facturation");return}const i=document.getElementById("contentArea");i.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;p!=="all"&&t.set("status",p),x!=="all"&&t.set("type",x);const e=await g.get(`/api/invoices?${t}`),o=e.invoices||[],r=e.stats||{};let s="";s+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    ...
    </div>`,o.length===0?s+='<div class="card"><div class="empty">Aucune facture. Créez votre première facture ou devis !</div></div>':(s+=`<div class="card" style="padding:0;overflow:auto"><table style="width:100%;border-collapse:collapse;font-size:.82rem">
      ...
      </div>`),i.innerHTML=s}catch(t){i.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${v(t.message)}</div>`}}async function F(i="invoice",t={}){let e=[];try{const a=await g.get("/api/clients");e=a.clients||a||[]}catch(a){console.warn("Impossible de charger les clients pour la facture:",a.message)}const o=i==="quote",r=o?"Nouveau devis":"Nouvelle facture",s=document.createElement("div");if(s.className="m-overlay open",s.id="invModal",s.innerHTML=`<div class="m-dialog m-md">
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,r.appendChild(s)}Object.defineProperty(window,"invoiceFilter",{get(){return p},set(i){p=i}});Object.defineProperty(window,"invoiceType",{get(){return x},set(i){x=i}});T({loadInvoices:_,openInvoiceModal:F,addInvoiceLine:C,updateInvTotals:w,saveInvoice:L,changeInvoiceStatus:N,deleteInvoice:j,downloadInvoicePDF:P,createInvoiceFromBooking:H,invClientChanged:B,invToggleUnbilled:z,issueCreditNote:X,submitCreditNote:V});export{C as addInvoiceLine,N as changeInvoiceStatus,H as createInvoiceFromBooking,j as deleteInvoice,P as downloadInvoicePDF,_ as loadInvoices,F as openInvoiceModal,L as saveInvoice,w as updateInvTotals};
