import{s as k,a as x,G as u,l as z,d as T}from"./dashboard-VRxV7MY_.js";import{i as S,s as q}from"./plan-gate-Badg7tzn.js";let p="all",y="all",h=[];function b(i){return String(i||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function f(i){return((i||0)/100).toFixed(2).replace(".",",")+" €"}async function _(){if(!S()){q(document.getElementById("contentArea"),"Facturation");return}const i=document.getElementById("contentArea");i.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;p!=="all"&&t.set("status",p),y!=="all"&&t.set("type",y);const e=await x.get(`/api/invoices?${t}`),o=e.invoices||[],s=e.stats||{};let r="";r+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    ...
    </label>`}).join("")}function B(i,t){const e=h[i];if(!e)return;const o=document.getElementById("invLines");if(o){if(t){const s=new Date(e.start_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}),r=e.service_name+(e.variant_name?" — "+e.variant_name:"")+" ("+(e.practitioner_name||"")+") — "+s,a=e.booked_price_cents??e.variant_price_cents??e.service_price_cents??0;if(e.pass_covered&&e.pass_info){m(e.id,r+" (inclus abonnement)",1,0);const n=e.pass_info,l="1 crédit consommé — Pass "+n.name+" ("+n.sessions_remaining+"/"+n.sessions_total+" restants)";[...o.querySelectorAll(".inv-desc")].some(c=>c.value===l)||m(e.id,l,1,0)}else if(m(e.id,r,1,a/100),e.promotion_discount_cents>0&&e.promotion_label){const n="Réduction : "+e.promotion_label+(e.promotion_discount_pct?" (-"+e.promotion_discount_pct+"%)":"");m(e.id,n,1,-(e.promotion_discount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id){const n=parseInt(e.gc_paid_cents)||0,l=parseInt(e.deposit_amount_cents)||0;if(e.deposit_payment_intent_id.startsWith("gc_")&&n<=0){const c="Acompte payé (Carte cadeau "+e.deposit_payment_intent_id.replace("gc_","")+")";[...o.querySelectorAll(".inv-desc")].some(v=>v.value===c)||m(e.id,c,1,-(l/100))}else if(n>0){const d="Acompte payé (Carte cadeau)";[...o.querySelectorAll(".inv-desc")].some(v=>v.value===d)||m(e.id,d,1,-(n/100));const g=l-n;if(g>0){const v="Acompte payé (Stripe)";[...o.querySelectorAll(".inv-desc")].some(C=>C.value===v)||m(e.id,v,1,-(g/100))}}else if(e.deposit_payment_intent_id.startsWith("pi_")||e.deposit_payment_intent_id.startsWith("cs_")){const d="Acompte payé (Stripe)";[...o.querySelectorAll(".inv-desc")].some(g=>g.value===d)||m(e.id,d,1,-(l/100))}}}else o.querySelectorAll(`[data-booking-id="${e.id}"]`).forEach(s=>s.remove());w()}}function m(i,t,e,o){const s=document.getElementById("invLines");if(!s)return;const r=document.createElement("div");r.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",r.setAttribute("data-booking-id",i),r.innerHTML=`
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,s.appendChild(r)}Object.defineProperty(window,"invoiceFilter",{get(){return p},set(i){p=i}});Object.defineProperty(window,"invoiceType",{get(){return y},set(i){y=i}});T({loadInvoices:_,openInvoiceModal:M,addInvoiceLine:I,updateInvTotals:w,saveInvoice:A,changeInvoiceStatus:D,deleteInvoice:L,downloadInvoicePDF:F,createInvoiceFromBooking:j,invClientChanged:E,invToggleUnbilled:B});export{I as addInvoiceLine,D as changeInvoiceStatus,j as createInvoiceFromBooking,L as deleteInvoice,F as downloadInvoicePDF,_ as loadInvoices,M as openInvoiceModal,A as saveInvoice,w as updateInvTotals};
