import{a as m,d as I,G as a,s as R,l as c,m as w}from"./dashboard-C_JJSNJO.js";import{trapFocus as C}from"./focus-trap-C-UMhpsq.js";import{i as G,s as S}from"./plan-gate-Badg7tzn.js";import{f as u}from"./format-DoZVPAsZ.js";let g="all",b="",x=[],E=null,M=0,y={};function l(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}let h=!0;async function f(){if(!G()){S(document.getElementById("contentArea"),"Cartes cadeau");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;g!=="all"&&e.set("status",g),b.trim()&&e.set("search",b.trim());const t=await m.get(`/api/gift-cards?${e}`),r=t.gift_cards||[];x=r,h=t.feature_enabled!==!1;const i=t.stats||{};F(o,r,i)}catch(e){o.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function F(o,e,t){e=e||x,t=t||{};let r="";h||(r+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
    ...
    ${h?'<button onclick="openCreateGiftCardModal()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer une carte</button>':'<button disabled title="Activez la fonctionnalité dans les Paramètres" aria-label="Activez la fonctionnalité dans les Paramètres" class="btn-primary btn-sm" style="opacity:.5;cursor:not-allowed"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer une carte</button>'}
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px;text-align:right;font-weight:600;color:${$>0?"var(--green)":"var(--text-4)"}">${u($)}</td>
        <td style="padding:10px"><div style="font-weight:500">${l(B)}</div>${z?`<div style="font-size:.7rem;color:var(--text-4)">${l(z)}</div>`:""}</td>
  ...
  </div>`,document.body.appendChild(t),w(t,{noBackdropClose:!0}),C(t,()=>c(t.id))}function L(o){document.getElementById("gcSelectedAmount").value=o,document.querySelectorAll(".gc-amount-pill").forEach(t=>{parseInt(t.dataset.cents)===o?(t.style.background="var(--primary)",t.style.color="#fff",t.style.borderColor="var(--primary)"):(t.style.background="var(--surface)",t.style.color="var(--text-1)",t.style.borderColor="var(--border)")});const e=document.getElementById("gcCustomAmount");e&&!e.matches(":focus")&&(e.value="")}function T(o){clearTimeout(E);const e=document.getElementById("gcClientAcDrop");if(e){if(o.length<3){e.style.display="none";return}E=setTimeout(async()=>{const t=++M;try{const r=await fetch(`/api/clients?search=${encodeURIComponent(o)}&limit=8`,{headers:{Authorization:"Bearer "+m.getToken()}});if(t!==M)return;const d=(await r.json()).clients||[];if(d.length===0){e.innerHTML=`<div style="padding:12px;text-align:center;font-size:.8rem;color:var(--text-4)">Aucun client trouvé pour "${l(o)}" — laissez le champ tel quel pour un nouveau destinataire</div>`,e.style.display="block";return}y={},d.forEach(s=>{y[s.id]=s}),e.innerHTML=d.map(s=>{const v=[s.phone,s.email].filter(Boolean).join(" · ");return`<div class="ac-item" onmousedown="event.preventDefault();gcPickClient('${s.id}')"><div class="ac-name">${l(s.full_name)}</div><div class="ac-meta">${l(v)||"—"}</div></div>`}).join(""),e.style.display="block"}catch{e.style.display="none"}},250)}}function N(o){const e=y[o];if(!e)return;const t=document.getElementById("gcRecipientName"),r=document.getElementById("gcRecipientEmail");t&&(t.value=e.full_name||""),r&&(r.value=e.email||"");const i=document.getElementById("gcClientAcDrop");i&&(i.style.display="none")}async function P(){const o=parseInt(document.getElementById("gcSelectedAmount").value),e=document.getElementById("gcRecipientName").value.trim(),t=document.getElementById("gcRecipientEmail").value.trim(),r=document.getElementById("gcMessage").value.trim();if(!o||o<=0){a.toast("Veuillez sélectionner un montant","error");return}if(!e){a.toast("Veuillez saisir le nom du destinataire","error");return}try{await m.post("/api/gift-cards",{amount_cents:o,recipient_name:e,recipient_email:t||void 0,message:r||void 0}),c("gcCreateModal"),a.toast("Carte cadeau créée avec succès","success"),f()}catch(i){a.toast(i.message||"Erreur lors de la création","error")}}function q(o){const e=x.find(r=>r.id===o);if(!e){a.toast("Carte introuvable","error");return}c("gcDebitModal");const t=document.createElement("div");t.className="m-overlay open",t.id="gcDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(r),w(r,{noBackdropClose:!0}),C(r,()=>c(r.id))}async function U(o){const e=document.querySelector("#gcRefundModal .m-btn-primary");if(e?.disabled)return;const t=parseFloat(document.getElementById("gcRefundAmount").value),r=document.getElementById("gcRefundNote").value.trim();if(!t||t<=0){a.toast("Veuillez saisir un montant valide","error");return}e&&(e.disabled=!0,e.textContent="Traitement...");try{const i=await m.post(`/api/gift-cards/${o}/refund`,{amount_cents:Math.round(t*100),note:r||void 0});if(c("gcRefundModal"),i&&i.stripe_refund_cents!=null){const d=(i.stripe_refund_cents/100).toFixed(2).replace(".",",")+" €";a.toast(`Remboursement effectué (${d} Stripe${i.stripe_fees_cents>0?" — frais "+(i.stripe_fees_cents/100).toFixed(2).replace(".",",")+" €":""})`,"success")}else a.toast("Remboursement effectué","success");f()}catch(i){e&&(e.disabled=!1,e.textContent="Rembourser"),a.toast(i.message||"Erreur lors du remboursement","error")}}async function O(o){const e=x.find(r=>r.id===o);if(!e){a.toast("Carte introuvable","error");return}if(await R("Annuler la carte",`Annuler la carte ${e.code} ? Cette action est irréversible.`,"Annuler la carte","danger"))try{await m.patch(`/api/gift-cards/${o}`,{status:"cancelled"}),a.toast("Carte annulée","success"),f()}catch(r){a.toast(r.message||"Erreur lors de l'annulation","error")}}Object.defineProperty(window,"gcFilter",{get(){return g},set(o){g=o},configurable:!0});Object.defineProperty(window,"gcSearch",{get(){return b},set(o){b=o},configurable:!0});I({loadGiftCards:f,openCreateGiftCardModal:j,selectGcAmount:L,submitCreateGiftCard:P,openDebitGiftCard:q,submitDebitGiftCard:H,refundGiftCard:V,submitRefundGiftCard:U,cancelGiftCard:O,gcClientLiveSearch:T,gcPickClient:N});export{f as loadGiftCards,F as renderGiftCards};
