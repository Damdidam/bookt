import{a as d,d as C,s as k,G as o,k as m,l as f}from"./dashboard-ChlgHSN4.js";import{i as S,s as B}from"./plan-gate-CWiUKJVs.js";let u="all",v="",g=[];function n(s){return String(s||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function _(s){return((s||0)/100).toFixed(2).replace(".",",")+" €"}async function x(){if(!S()){B(document.getElementById("contentArea"),"Abonnements");return}const s=document.getElementById("contentArea");s.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;u!=="all"&&t.set("status",u),v.trim()&&t.set("search",v.trim());const r=await d.get(`/api/passes?${t}`),e=r.passes||[];g=e;const i=r.stats||{};I(s,e,i)}catch(t){s.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${n(t.message)}</div>`}}function I(s,t,r){t=t||g,r=r||{};let e="";e+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    ...
    <button onclick="openCreatePass()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>
      ...
      </tr>`}),e+="</tbody></table></div>"}s.innerHTML=e}async function A(){m("passCreateModal");let s=[];try{const e=await d.get("/api/services");s=(e.services||e||[]).filter(i=>i.is_active!==!1)}catch(e){console.error("[passes] fetch services failed",e),o.toast("Impossible de charger les prestations: "+e.message,"error")}const t=s.map(e=>`<option value="${n(e.id)}">${n(e.name)}${e.price_cents!=null?" — "+_(e.price_cents):""}</option>`).join(""),r=document.createElement("div");r.className="m-overlay open",r.id="passCreateModal",r.innerHTML=`<div class="m-dialog m-md">
  ...
  </div>`,document.body.appendChild(e),f(e,{noBackdropClose:!0})}async function D(s){try{await d.post(`/api/passes/${s}/debit`),m("passDebitModal"),o.toast("Séance débitée avec succès","success"),x()}catch(t){o.toast(t.message||"Erreur lors du débit","error")}}async function T(s){const t=g.find(b=>b.id===s);if(!t){o.toast("Pass introuvable","error");return}m("passRefundModal");const r=parseInt(t.sessions_remaining||0),e=parseInt(t.sessions_total||0),i=document.createElement("div");i.className="m-overlay open",i.id="passRefundModal",i.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(i),f(i,{noBackdropClose:!0})}async function L(s){try{await d.post(`/api/passes/${s}/refund`),m("passRefundModal"),o.toast("Séance remboursée avec succès","success"),x()}catch(t){o.toast(t.message||"Erreur lors du remboursement","error")}}async function F(s){const t=g.find(e=>e.id===s);if(!t){o.toast("Pass introuvable","error");return}if(await k("Annuler le pass",`Annuler le pass ${t.code} ? Cette action est irréversible.`,"Annuler le pass","danger"))try{await d.patch(`/api/passes/${s}`,{status:"cancelled"}),o.toast("Pass annulé","success"),x()}catch(e){o.toast(e.message||"Erreur lors de l'annulation","error")}}async function H(s,t){if(await k("Supprimer le pass",`Supprimer définitivement le pass ${t} ? Cette action est irréversible.`,"Supprimer","danger"))try{await d.delete(`/api/passes/${s}`),o.toast("Pass supprimé","success"),x()}catch(e){o.toast(e.message||"Erreur lors de la suppression","error")}}function V(s){u=s}function U(s){v=s}Object.defineProperty(window,"passFilter",{get(){return u},set(s){u=s},configurable:!0});Object.defineProperty(window,"passSearch",{get(){return v},set(s){v=s},configurable:!0});C({loadPasses:x,openCreatePass:A,submitCreatePass:j,passServiceChanged:R,openDebitPass:N,debitPass:D,refundPass:T,submitRefundPass:L,cancelPass:F,deletePass:H,setPassFilter:V,passSearchInput:U});export{x as loadPasses,I as renderPasses};
