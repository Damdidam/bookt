import{a as p,d as _,G as i,s as I,k as u,l as h}from"./dashboard-BxuDtHfw.js";import{i as D,s as G}from"./plan-gate-Badg7tzn.js";let g="all",x="",b=[],z=null,$=0,y={};function l(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function d(o){return((o||0)/100).toFixed(2).replace(".",",")+" €"}async function f(){if(!D()){G(document.getElementById("contentArea"),"Cartes cadeau");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;g!=="all"&&e.set("status",g),x.trim()&&e.set("search",x.trim());const t=await p.get(`/api/gift-cards?${e}`),r=t.gift_cards||[];b=r;const s=t.stats||{};R(o,r,s)}catch(e){o.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function R(o,e,t){e=e||b,t=t||{};let r="";r+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    ...
    <button onclick="openCreateGiftCardModal()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer une carte</button>
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px;text-align:right;font-weight:600;color:${k>0?"var(--green)":"var(--text-4)"}">${d(k)}</td>
        <td style="padding:10px"><div style="font-weight:500">${l(A)}</div>${C?`<div style="font-size:.7rem;color:var(--text-4)">${l(C)}</div>`:""}</td>
        <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${w}12;color:${w};font-weight:600">${v[n.status]||n.status}</span></td>
  ...
  </div>`,document.body.appendChild(t),h(t,{noBackdropClose:!0})}function j(o){document.getElementById("gcSelectedAmount").value=o,document.querySelectorAll(".gc-amount-pill").forEach(t=>{parseInt(t.dataset.cents)===o?(t.style.background="var(--primary)",t.style.color="#fff",t.style.borderColor="var(--primary)"):(t.style.background="var(--surface)",t.style.color="var(--text-1)",t.style.borderColor="var(--border)")});const e=document.getElementById("gcCustomAmount");e&&!e.matches(":focus")&&(e.value="")}function L(o){clearTimeout(z);const e=document.getElementById("gcClientAcDrop");if(e){if(o.length<3){e.style.display="none";return}z=setTimeout(async()=>{const t=++$;try{const r=await fetch(`/api/clients?search=${encodeURIComponent(o)}&limit=8`,{headers:{Authorization:"Bearer "+p.getToken()}});if(t!==$)return;const m=(await r.json()).clients||[];if(m.length===0){e.innerHTML=`<div style="padding:12px;text-align:center;font-size:.8rem;color:var(--text-4)">Aucun client trouvé pour "${l(o)}" — laissez le champ tel quel pour un nouveau destinataire</div>`,e.style.display="block";return}y={},m.forEach(a=>{y[a.id]=a}),e.innerHTML=m.map(a=>{const v=[a.phone,a.email].filter(Boolean).join(" · ");return`<div class="ac-item" onmousedown="event.preventDefault();gcPickClient('${a.id}')"><div class="ac-name">${l(a.full_name)}</div><div class="ac-meta">${l(v)||"—"}</div></div>`}).join(""),e.style.display="block"}catch{e.style.display="none"}},250)}}function T(o){const e=y[o];if(!e)return;const t=document.getElementById("gcRecipientName"),r=document.getElementById("gcRecipientEmail");t&&(t.value=e.full_name||""),r&&(r.value=e.email||"");const s=document.getElementById("gcClientAcDrop");s&&(s.style.display="none")}async function N(){const o=parseInt(document.getElementById("gcSelectedAmount").value),e=document.getElementById("gcRecipientName").value.trim(),t=document.getElementById("gcRecipientEmail").value.trim(),r=document.getElementById("gcMessage").value.trim();if(!o||o<=0){i.toast("Veuillez sélectionner un montant","error");return}if(!e){i.toast("Veuillez saisir le nom du destinataire","error");return}try{await p.post("/api/gift-cards",{amount_cents:o,recipient_name:e,recipient_email:t||void 0,message:r||void 0}),u("gcCreateModal"),i.toast("Carte cadeau créée avec succès","success"),f()}catch(s){i.toast(s.message||"Erreur lors de la création","error")}}function F(o){const e=b.find(r=>r.id===o);if(!e){i.toast("Carte introuvable","error");return}u("gcDebitModal");const t=document.createElement("div");t.className="m-overlay open",t.id="gcDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(t),h(t,{noBackdropClose:!0})}async function P(o){const e=parseFloat(document.getElementById("gcDebitAmount").value),t=document.getElementById("gcDebitNote").value.trim();if(!e||e<=0){i.toast("Veuillez saisir un montant valide","error");return}try{await p.post(`/api/gift-cards/${o}/debit`,{amount_cents:Math.round(e*100),note:t||void 0}),u("gcDebitModal"),i.toast("Carte débitée avec succès","success"),f()}catch(r){i.toast(r.message||"Erreur lors du débit","error")}}async function H(o){const e=b.find(s=>s.id===o);if(!e){i.toast("Carte introuvable","error");return}u("gcRefundModal");const t=parseInt(e.amount_cents||0)-parseInt(e.balance_cents||0);if(t<=0){i.toast("Aucun montant à rembourser","error");return}const r=document.createElement("div");r.className="m-overlay open",r.id="gcRefundModal",r.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(r),h(r,{noBackdropClose:!0})}async function U(o){const e=parseFloat(document.getElementById("gcRefundAmount").value),t=document.getElementById("gcRefundNote").value.trim();if(!e||e<=0){i.toast("Veuillez saisir un montant valide","error");return}try{await p.post(`/api/gift-cards/${o}/refund`,{amount_cents:Math.round(e*100),note:t||void 0}),u("gcRefundModal"),i.toast("Remboursement effectué","success"),f()}catch(r){i.toast(r.message||"Erreur lors du remboursement","error")}}async function V(o){const e=b.find(r=>r.id===o);if(!e){i.toast("Carte introuvable","error");return}if(await I("Annuler la carte",`Annuler la carte ${e.code} ? Cette action est irréversible.`,"Annuler la carte","danger"))try{await p.patch(`/api/gift-cards/${o}`,{status:"cancelled"}),i.toast("Carte annulée","success"),f()}catch(r){i.toast(r.message||"Erreur lors de l'annulation","error")}}Object.defineProperty(window,"gcFilter",{get(){return g},set(o){g=o},configurable:!0});Object.defineProperty(window,"gcSearch",{get(){return x},set(o){x=o},configurable:!0});_({loadGiftCards:f,openCreateGiftCardModal:S,selectGcAmount:j,submitCreateGiftCard:N,openDebitGiftCard:F,submitDebitGiftCard:P,refundGiftCard:H,submitRefundGiftCard:U,cancelGiftCard:V,gcClientLiveSearch:L,gcPickClient:T});export{f as loadGiftCards,R as renderGiftCards};
