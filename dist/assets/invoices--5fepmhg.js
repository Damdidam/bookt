import{a as g,G as p,n as B,d as z}from"./dashboard-n81oMnD0.js";let c="all",v="all",b=[];function m(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function y(o){return((o||0)/100).toFixed(2).replace(".",",")+" €"}async function f(){const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;c!=="all"&&t.set("status",c),v!=="all"&&t.set("type",v);const e=await g.get(`/api/invoices?${t}`),i=e.invoices||[],s=e.stats||{};let r="";r+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
      ...
      <select onchange="invoiceFilter=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        <option value="all" ${c==="all"?"selected":""}>Tous statuts</option>
        <option value="draft" ${c==="draft"?"selected":""}>Brouillons</option>
        <option value="sent" ${c==="sent"?"selected":""}>Envoyées</option>
        <option value="paid" ${c==="paid"?"selected":""}>Payées</option>
        <option value="overdue" ${c==="overdue"?"selected":""}>En retard</option>
        <option value="cancelled" ${c==="cancelled"?"selected":""}>Annulées</option>
      </select>
        ...
        <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
          ...
          <td style="padding:10px"><span style="font-size:.7rem;padding:2px 8px;border-radius:10px;background:${_[n.type]}15;color:${_[n.type]};font-weight:600">${d[n.type]||n.type}</span></td>
    ...
    <div style="font-size:1rem;font-weight:700;color:var(--text);margin-top:4px">Total TTC : ${y(r)}</div>`}async function T(o){const t=document.getElementById("invClient")?.value;if(!t){p.toast("Sélectionnez un client","error");return}const e=document.getElementById("invLines"),i=[];if(e.querySelectorAll(":scope > div").forEach(s=>{const r=s.querySelector(".inv-desc")?.value?.trim(),a=parseFloat(s.querySelector(".inv-qty")?.value||1),n=parseFloat(s.querySelector(".inv-price")?.value||0),l=s.getAttribute("data-booking-id")||void 0;r&&n!==0&&i.push({description:r,quantity:a,unit_price_cents:Math.round(n*100),booking_id:l})}),i.length===0){p.toast("Ajoutez au moins une ligne","error");return}try{const s={client_id:t,type:o||"invoice",items:i,vat_rate:parseFloat(document.getElementById("invVat")?.value||21),due_days:parseInt(document.getElementById("invDueDays")?.value||30),client_bce:document.getElementById("invClientBce")?.value?.trim()||void 0,notes:document.getElementById("invNotes")?.value?.trim()||void 0},r=await g.post("/api/invoices",s);document.getElementById("invModal")._dirtyGuard?.markClean(),closeModal("invModal"),p.toast(o==="quote"?"Devis créé !":"Facture créée !","success"),f()}catch(s){p.toast(s.message||"Erreur","error")}}async function q(o,t){if(confirm({sent:"Marquer comme envoyée ?",paid:"Marquer comme payée ?"}[t]||`Changer le statut en "${t}" ?`))try{await g.patch(`/api/invoices/${o}/status`,{status:t}),p.toast("Statut mis à jour","success"),f()}catch(i){p.toast(i.message||"Erreur","error")}}async function M(o){if(confirm("Supprimer ce brouillon ?"))try{await g.delete(`/api/invoices/${o}`),p.toast("Brouillon supprimé","success"),f()}catch(t){p.toast(t.message||"Erreur","error")}}function S(o){const t=localStorage.getItem("genda_token");window.open(`/api/invoices/${o}/pdf?token=${t}`,"_blank")}async function L(o){if(confirm("Créer une facture pour ce rendez-vous ?"))try{const t=await g.post("/api/invoices",{booking_id:o,type:"invoice"});p.toast("Facture créée ! Retrouvez-la dans Facturation.","success"),document.querySelectorAll(".ni").forEach(e=>e.classList.remove("active")),document.querySelector('[data-section="invoices"]')?.classList.add("active"),document.getElementById("pageTitle").textContent="Facturation",f()}catch(t){p.toast(t.message||"Erreur","error")}}async function w(){const o=document.getElementById("invClient")?.value,t=document.getElementById("invUnbilledSection"),e=document.getElementById("invUnbilledList");if(!o||!t||!e){t&&(t.style.display="none");return}try{b=(await g.get(`/api/invoices/unbilled?client_id=${o}`)).bookings||[]}catch{b=[]}if(b.length===0){t.style.display="none";return}t.style.display="",e.innerHTML=b.map((i,s)=>{const r=new Date(i.start_at).toLocaleDateString("fr-BE"),a=i.variant_price_cents??i.service_price_cents??0,n=i.service_name+(i.variant_name?" — "+i.variant_name:"")+" ("+(i.practitioner_name||"?")+") — "+r,l=i.pass_covered?' <span style="font-size:.68rem;color:var(--green);font-weight:600;padding:1px 6px;border-radius:4px;background:var(--green-bg)">Pass</span>':"",u=i.promotion_discount_cents>0?' <span style="font-size:.68rem;color:var(--green);font-weight:600">promo</span>':"",d=i.pass_covered?'<span style="color:var(--green)">inclus</span>':y(a);return`<label style="display:flex;align-items:center;gap:8px;padding:8px 12px;font-size:.82rem;cursor:pointer;border-bottom:1px solid var(--border-light);background:${s%2===0?"var(--white)":"var(--surface)"}">
      <input type="checkbox" data-unbilled-idx="${s}" onchange="invToggleUnbilled(${s},this.checked)">
      <span style="flex:1">${m(n)}${l}</span>
      <span style="font-weight:600;color:var(--text-2)">${d}${u}</span>
    </label>`}).join("")}function I(o,t){const e=b[o];if(!e)return;const i=document.getElementById("invLines");if(i){if(t){const s=new Date(e.start_at).toLocaleDateString("fr-BE"),r=e.service_name+(e.variant_name?" — "+e.variant_name:"")+" ("+(e.practitioner_name||"")+") — "+s,a=e.variant_price_cents??e.service_price_cents??0;if(e.pass_covered&&e.pass_info){x(e.id,r+" (inclus abonnement)",1,0);const n=e.pass_info,l="1 crédit consommé — Pass "+n.name+" ("+n.sessions_remaining+"/"+n.sessions_total+" restants)";[...i.querySelectorAll(".inv-desc")].some(d=>d.value===l)||x(e.id,l,1,0)}else if(x(e.id,r,1,a/100),e.promotion_discount_cents>0&&e.promotion_label){const n="Réduction : "+e.promotion_label+(e.promotion_discount_pct?" (-"+e.promotion_discount_pct+"%)":"");x(e.id,n,1,-(e.promotion_discount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id&&(e.deposit_payment_intent_id.startsWith("pi_")||e.deposit_payment_intent_id.startsWith("cs_"))){const n="Acompte payé (Stripe)";[...i.querySelectorAll(".inv-desc")].some(u=>u.value===n)||x(e.id,n,1,-(e.deposit_amount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id&&e.deposit_payment_intent_id.startsWith("gc_")){const l="Acompte payé (Carte cadeau "+e.deposit_payment_intent_id.replace("gc_","")+")";[...i.querySelectorAll(".inv-desc")].some(d=>d.value===l)||x(e.id,l,1,-(e.deposit_amount_cents/100))}}else i.querySelectorAll(`[data-booking-id="${e.id}"]`).forEach(s=>s.remove());h()}}function x(o,t,e,i){const s=document.getElementById("invLines");if(!s)return;const r=document.createElement("div");r.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",r.setAttribute("data-booking-id",o),r.innerHTML=`
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,s.appendChild(r)}Object.defineProperty(window,"invoiceFilter",{get(){return c},set(o){c=o}});Object.defineProperty(window,"invoiceType",{get(){return v},set(o){v=o}});z({loadInvoices:f,openInvoiceModal:C,addInvoiceLine:$,updateInvTotals:h,saveInvoice:T,changeInvoiceStatus:q,deleteInvoice:M,downloadInvoicePDF:S,createInvoiceFromBooking:L,invClientChanged:w,invToggleUnbilled:I});export{$ as addInvoiceLine,q as changeInvoiceStatus,L as createInvoiceFromBooking,M as deleteInvoice,S as downloadInvoicePDF,f as loadInvoices,C as openInvoiceModal,T as saveInvoice,h as updateInvTotals};
