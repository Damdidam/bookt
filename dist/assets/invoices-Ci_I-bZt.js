import{a as v,G as c,o as B,d as C}from"./dashboard-CHYuFKOq.js";let d="all",u="all",x=[];function g(n){return String(n||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function m(n){return((n||0)/100).toFixed(2).replace(".",",")+" €"}async function b(){const n=document.getElementById("contentArea");n.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;d!=="all"&&t.set("status",d),u!=="all"&&t.set("type",u);const e=await v.get(`/api/invoices?${t}`),o=e.invoices||[],s=e.stats||{};let a="";a+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
      <div class="stat-card"><div class="label">Brouillons</div><div class="val">${s.drafts||0}</div></div>
      <div class="stat-card"><div class="label">En attente</div><div class="val" style="color:var(--gold)">${m(parseInt(s.total_pending||0))}</div><div class="sub">${parseInt(s.sent||0)+parseInt(s.overdue||0)} factures</div></div>
      <div class="stat-card"><div class="label">Payées</div><div class="val" style="color:var(--green)">${m(parseInt(s.total_paid||0))}</div><div class="sub">${s.paid||0} factures</div></div>
      ...
      <select onchange="invoiceFilter=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        ...
        <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        </tr></thead><tbody>`,o.forEach(i=>{const l={draft:"var(--text-4)",sent:"var(--gold)",paid:"var(--green)",overdue:"var(--red)",cancelled:"var(--text-4)"},y={draft:"Brouillon",sent:"Envoyée",paid:"Payée",overdue:"En retard",cancelled:"Annulée"},p={invoice:"Facture",quote:"Devis",credit_note:"Note crédit"},_={invoice:"var(--primary)",quote:"var(--gold)",credit_note:"var(--text-3)"},k=l[i.status]||"var(--text-4)",E=new Date(i.issue_date).toLocaleDateString("fr-BE");a+=`<tr style="border-bottom:1px solid var(--border-light)">
          <td style="padding:10px 14px;font-weight:600">${g(i.invoice_number)}</td>
          ...
          <td style="padding:10px">${g(i.client_name)}</td>
          <td style="padding:10px;color:var(--text-3)">${E}</td>
          <td style="padding:10px;text-align:right;font-weight:600">${m(i.total_cents)}</td>
          <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${k}12;color:${k};font-weight:600">${y[i.status]||i.status}</span></td>
        ...
        </tr>`}),a+="</tbody></table></div>"),v.getBusiness()?.settings?.iban||(a+=`<div class="card" style="background:var(--gold-bg);border:1px solid #E0D4A8;margin-top:14px;padding:14px 18px">
      ...
      </div>`),n.innerHTML=a}catch(t){n.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${g(t.message)}</div>`}}async function T(n="invoice",t={}){let e=[];try{const r=await v.get("/api/clients");e=r.clients||r||[]}catch(r){console.warn("Impossible de charger les clients pour la facture:",r.message)}const o=n==="quote",s=o?"Nouveau devis":"Nouvelle facture",a=document.createElement("div");if(a.className="m-overlay open",a.id="invModal",a.innerHTML=`<div class="m-dialog m-md">
    ...
    <div class="m-body" style="display:grid;gap:12px">
      <div>
        ...
        <select class="m-input" id="invClient" onchange="invClientChanged()">
          ...
          ${e.map(r=>`<option value="${r.id}">${g(r.full_name)}${r.email?" ("+g(r.email)+")":""}</option>`).join("")}
        </select>
      </div>
    ...
    <div style="font-size:.82rem;color:var(--text-3)">Sous-total HT : <strong>${m(o)}</strong></div>
    <div style="font-size:.82rem;color:var(--text-3)">TVA (${e}%) : <strong>${m(s)}</strong></div>
    <div style="font-size:1rem;font-weight:700;color:var(--text);margin-top:4px">Total TTC : ${m(a)}</div>`}async function z(n){const t=document.getElementById("invClient")?.value;if(!t){c.toast("Sélectionnez un client","error");return}const e=document.getElementById("invLines"),o=[];if(e.querySelectorAll(":scope > div").forEach(s=>{const a=s.querySelector(".inv-desc")?.value?.trim(),r=parseFloat(s.querySelector(".inv-qty")?.value||1),i=parseFloat(s.querySelector(".inv-price")?.value||0),l=s.getAttribute("data-booking-id")||void 0;a&&i!==0&&o.push({description:a,quantity:r,unit_price_cents:Math.round(i*100),booking_id:l})}),o.length===0){c.toast("Ajoutez au moins une ligne","error");return}try{const s={client_id:t,type:n||"invoice",items:o,vat_rate:parseFloat(document.getElementById("invVat")?.value||21),due_days:parseInt(document.getElementById("invDueDays")?.value||30),client_bce:document.getElementById("invClientBce")?.value?.trim()||void 0,notes:document.getElementById("invNotes")?.value?.trim()||void 0},a=await v.post("/api/invoices",s);document.getElementById("invModal")._dirtyGuard?.markClean(),closeModal("invModal"),c.toast(n==="quote"?"Devis créé !":"Facture créée !","success"),b()}catch(s){c.toast(s.message||"Erreur","error")}}async function q(n,t){if(confirm({sent:"Marquer comme envoyée ?",paid:"Marquer comme payée ?"}[t]||`Changer le statut en "${t}" ?`))try{await v.patch(`/api/invoices/${n}/status`,{status:t}),c.toast("Statut mis à jour","success"),b()}catch(o){c.toast(o.message||"Erreur","error")}}async function M(n){if(confirm("Supprimer ce brouillon ?"))try{await v.delete(`/api/invoices/${n}`),c.toast("Brouillon supprimé","success"),b()}catch(t){c.toast(t.message||"Erreur","error")}}function S(n){const t=localStorage.getItem("genda_token");window.open(`/api/invoices/${n}/pdf?token=${t}`,"_blank")}async function L(n){if(confirm("Créer une facture pour ce rendez-vous ?"))try{const t=await v.post("/api/invoices",{booking_id:n,type:"invoice"});c.toast("Facture créée ! Retrouvez-la dans Facturation.","success"),document.querySelectorAll(".ni").forEach(e=>e.classList.remove("active")),document.querySelector('[data-section="invoices"]')?.classList.add("active"),document.getElementById("pageTitle").textContent="Facturation",b()}catch(t){c.toast(t.message||"Erreur","error")}}async function I(){const n=document.getElementById("invClient")?.value,t=document.getElementById("invUnbilledSection"),e=document.getElementById("invUnbilledList");if(!n||!t||!e){t&&(t.style.display="none");return}try{x=(await v.get(`/api/invoices/unbilled?client_id=${n}`)).bookings||[]}catch{x=[]}if(x.length===0){t.style.display="none";return}t.style.display="",e.innerHTML=x.map((o,s)=>{const a=new Date(o.start_at).toLocaleDateString("fr-BE"),r=o.variant_price_cents??o.service_price_cents??0,i=o.service_name+(o.variant_name?" — "+o.variant_name:"")+" ("+(o.practitioner_name||"?")+") — "+a;return`<label style="display:flex;align-items:center;gap:8px;padding:8px 12px;font-size:.82rem;cursor:pointer;border-bottom:1px solid var(--border-light);background:${s%2===0?"var(--white)":"var(--surface)"}">
      <input type="checkbox" data-unbilled-idx="${s}" onchange="invToggleUnbilled(${s},this.checked)">
      <span style="flex:1">${g(i)}</span>
      <span style="font-weight:600;color:var(--text-2)">${m(r)}</span>
    </label>`}).join("")}function w(n,t){const e=x[n];if(!e)return;const o=document.getElementById("invLines");if(o){if(t){const s=new Date(e.start_at).toLocaleDateString("fr-BE"),a=e.service_name+(e.variant_name?" — "+e.variant_name:"")+" ("+(e.practitioner_name||"")+") — "+s,r=e.variant_price_cents??e.service_price_cents??0;if(f(e.id,a,1,r/100),e.deposit_payment_intent_id&&e.deposit_payment_intent_id.startsWith("pass_")&&e.pass_info){const i=e.pass_info,l="1 crédit consommé — Pass "+i.name+" ("+i.sessions_remaining+"/"+i.sessions_total+" restants)";[...o.querySelectorAll(".inv-desc")].some(p=>p.value===l)||f(e.id,l,1,0)}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id&&(e.deposit_payment_intent_id.startsWith("pi_")||e.deposit_payment_intent_id.startsWith("cs_"))){const i="Acompte payé (Stripe)";[...o.querySelectorAll(".inv-desc")].some(y=>y.value===i)||f(e.id,i,1,-(e.deposit_amount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id&&e.deposit_payment_intent_id.startsWith("gc_")){const l="Acompte payé (Carte cadeau "+e.deposit_payment_intent_id.replace("gc_","")+")";[...o.querySelectorAll(".inv-desc")].some(p=>p.value===l)||f(e.id,l,1,-(e.deposit_amount_cents/100))}}else o.querySelectorAll(`[data-booking-id="${e.id}"]`).forEach(s=>s.remove());h()}}function f(n,t,e,o){const s=document.getElementById("invLines");if(!s)return;const a=document.createElement("div");a.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",a.setAttribute("data-booking-id",n),a.innerHTML=`
    <input class="inv-desc" value="${g(t)}" style="padding:8px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.82rem">
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,s.appendChild(a)}Object.defineProperty(window,"invoiceFilter",{get(){return d},set(n){d=n}});Object.defineProperty(window,"invoiceType",{get(){return u},set(n){u=n}});C({loadInvoices:b,openInvoiceModal:T,addInvoiceLine:$,updateInvTotals:h,saveInvoice:z,changeInvoiceStatus:q,deleteInvoice:M,downloadInvoicePDF:S,createInvoiceFromBooking:L,invClientChanged:I,invToggleUnbilled:w});export{$ as addInvoiceLine,q as changeInvoiceStatus,L as createInvoiceFromBooking,M as deleteInvoice,S as downloadInvoicePDF,b as loadInvoices,T as openInvoiceModal,z as saveInvoice,h as updateInvTotals};
