import{a as s,G as u,d as b,s as w}from"./dashboard-ChlgHSN4.js";import{i as k,s as $}from"./plan-gate-CWiUKJVs.js";const i=u.toast.bind(u);function d(t){return String(t||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const p={google:{label:"Google Calendar",color:"#4285F4",icon:"M21.35 11.1h-9.18v2.73h5.51c-.54 2.56-2.73 3.97-5.51 3.97a6.13 6.13 0 1 1 0-12.26c1.51 0 2.89.55 3.96 1.45l2.04-2.04A9.27 9.27 0 0 0 12.17 2 10 10 0 1 0 22 12.17c0-.57-.07-1.13-.18-1.66l-.47.59Z"},outlook:{label:"Outlook Calendar",color:"#0078D4",icon:"M21 5H3v14h18V5Zm-2 2v3h-3V7h3ZM5 7h3v3H5V7Zm0 5h3v3H5v-3Zm5 5v-3h4v3h-4Zm4-5h-4v-3h4v3Zm2 5v-3h3v3h-3Zm3-5h-3v-3h3v3Z"},ical:{label:"Lien iCal",color:"#6B6560",icon:"M8 2v4m8-4v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Z"}},C={push:"Genda → Calendrier",pull:"Calendrier → Genda",both:"Bidirectionnel"};async function h(){if(!k()){$(document.getElementById("contentArea"),"Calendrier externe");return}const t=document.getElementById("contentArea");t.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const[o,n]=await Promise.all([fetch("/api/calendar/connections",{headers:{Authorization:"Bearer "+s.getToken()}}),fetch("/api/practitioners",{headers:{Authorization:"Bearer "+s.getToken()}})]),l=(await o.json()).connections||[],a=(await n.json()).practitioners||[];let r='<div style="max-width:640px;margin:0 auto">';if(r+=`<div class="card" style="margin-bottom:16px">
      ...
      <button class="btn-outline" style="display:flex;align-items:center;gap:8px" onclick="calSyncGenIcal()">
    ...
    </div>`,a.length>1&&(r+=`<div style="margin-top:12px;font-size:.78rem;color:var(--text-3)">
      ...
      </div>`),r+="</div>",l.length===0)r+=`<div class="card" style="text-align:center;padding:32px">
        ...
        <p style="font-size:.78rem;color:var(--text-4);margin-top:4px">Connectez un calendrier externe pour synchroniser vos rendez-vous.</p>
          ...
          </div>`,e.error_message&&(r+=`<div style="margin-top:8px;padding:8px 10px;background:var(--red)08;border:1px solid var(--red-bg);border-radius:8px;font-size:.72rem;color:var(--red)">${d(e.error_message)}</div>`),e.provider==="ical"&&(r+=`<div style="margin-top:10px">
        ...
        </div></div>`}r+="</div>"}r+='<div id="icalUrlBox" style="display:none"></div>',r+="</div>",t.innerHTML=r}catch(o){t.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${d(o.message)}</div>`}}function y(){const t=document.getElementById("calSyncPracId");return t?t.value:null}async function z(t){try{const o=y(),n=`/api/calendar/${t}/connect`+(o?`?practitioner_id=${o}`:""),a=await(await fetch(n,{headers:{Authorization:"Bearer "+s.getToken()}})).json();if(a.error)throw new Error(a.error);a.url&&(window.location.href=a.url)}catch(o){i(o.message,"error")}}async function S(){try{const t=y(),o=await fetch("/api/calendar/ical/generate",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+s.getToken()},body:JSON.stringify({practitioner_id:t})}),n=await o.json();if(!o.ok)throw new Error(n.error);const l=document.getElementById("icalUrlBox");l.style.display="block",l.innerHTML=`<div class="card" style="margin-top:16px">
    ...
    </div>`,i("Lien iCal généré","success")}catch(t){i(t.message,"error")}}async function B(t,o){try{const n=await fetch(`/api/calendar/connections/${t}`,{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:"Bearer "+s.getToken()},body:JSON.stringify({sync_direction:o})});if(!n.ok)throw new Error((await n.json()).error);i("Direction mise à jour","success")}catch(n){i(n.message,"error")}}async function M(t){try{i("Synchronisation...","info");const o=await fetch(`/api/calendar/connections/${t}/sync`,{method:"POST",headers:{Authorization:"Bearer "+s.getToken()}}),n=await o.json();if(!o.ok)throw new Error(n.error);i(`Sync OK — ${n.pulled||0} importé(s), ${n.pushed||0} exporté(s)`,"success"),h()}catch(o){i(o.message,"error")}}async function T(t,o){if(await w(`Déconnecter ${o} ?`))try{const n=await fetch(`/api/calendar/connections/${t}`,{method:"DELETE",headers:{Authorization:"Bearer "+s.getToken()}});if(!n.ok)throw new Error((await n.json()).error);i("Calendrier déconnecté","success"),h()}catch(n){i(n.message,"error")}}b({loadCalSync:h,calSyncConnect:z,calSyncGenIcal:S,calSyncUpdateDir:B,calSyncNow:M,calSyncDisconnect:T});export{h as loadCalSync};
