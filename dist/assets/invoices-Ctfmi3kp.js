import{a as p,G as c,o as B,d as C}from"./dashboard-D7M_veCF.js";let d="all",u="all",y=[];function g(t){return String(t||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function m(t){return((t||0)/100).toFixed(2).replace(".",",")+" €"}async function x(){const t=document.getElementById("contentArea");t.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;d!=="all"&&e.set("status",d),u!=="all"&&e.set("type",u);const n=await p.get(`/api/invoices?${e}`),o=n.invoices||[],i=n.stats||{};let a="";a+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
      <div class="stat-card"><div class="label">Brouillons</div><div class="val">${i.drafts||0}</div></div>
      <div class="stat-card"><div class="label">En attente</div><div class="val" style="color:var(--gold)">${m(parseInt(i.total_pending||0))}</div><div class="sub">${parseInt(i.sent||0)+parseInt(i.overdue||0)} factures</div></div>
      <div class="stat-card"><div class="label">Payées</div><div class="val" style="color:var(--green)">${m(parseInt(i.total_paid||0))}</div><div class="sub">${i.paid||0} factures</div></div>
      ...
      <select onchange="invoiceFilter=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        ...
        </tr></thead><tbody>`,o.forEach(r=>{const l={draft:"var(--text-4)",sent:"var(--gold)",paid:"var(--green)",overdue:"var(--red)",cancelled:"var(--text-4)"},b={draft:"Brouillon",sent:"Envoyée",paid:"Payée",overdue:"En retard",cancelled:"Annulée"},v={invoice:"Facture",quote:"Devis",credit_note:"Note crédit"},h={invoice:"var(--primary)",quote:"var(--gold)",credit_note:"var(--text-3)"},k=l[r.status]||"var(--text-4)",E=new Date(r.issue_date).toLocaleDateString("fr-BE");a+=`<tr style="border-bottom:1px solid var(--border-light)">
          <td style="padding:10px 14px;font-weight:600">${g(r.invoice_number)}</td>
          <td style="padding:10px"><span style="font-size:.7rem;padding:2px 8px;border-radius:10px;background:${h[r.type]}15;color:${h[r.type]};font-weight:600">${v[r.type]||r.type}</span></td>
          <td style="padding:10px">${g(r.client_name)}</td>
          <td style="padding:10px;color:var(--text-3)">${E}</td>
          <td style="padding:10px;text-align:right;font-weight:600">${m(r.total_cents)}</td>
          <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${k}12;color:${k};font-weight:600">${b[r.status]||r.status}</span></td>
    ...
    <div class="m-body" style="display:grid;gap:12px">
      <div>
        ...
        <select class="m-input" id="invClient" onchange="invClientChanged()">
          ...
          ${n.map(s=>`<option value="${s.id}">${g(s.full_name)}${s.email?" ("+g(s.email)+")":""}</option>`).join("")}
        </select>
      </div>
  ...
  </div>`,document.body.appendChild(a),B(document.getElementById("invModal"),{noBackdropClose:!0}),$(),f(),e.preselect_client_id){const s=document.getElementById("invClient");s&&(s.value=e.preselect_client_id),await I(),(e.precheck_booking_id||e.precheck_group_id)&&setTimeout(()=>{y.forEach((r,l)=>{if(e.precheck_booking_id&&r.id===e.precheck_booking_id||e.precheck_group_id&&r.group_id===e.precheck_group_id){const v=document.querySelector(`[data-unbilled-idx="${l}"]`);v&&!v.checked&&(v.checked=!0,w(l,!0))}})},100)}}function $(){const t=document.getElementById("invLines");if(!t)return;const e=document.createElement("div");e.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",e.innerHTML=`
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,t.appendChild(e)}function f(){const t=document.getElementById("invLines"),e=document.getElementById("invTotals");if(!t||!e)return;const n=parseFloat(document.getElementById("invVat")?.value||21);let o=0;t.querySelectorAll(":scope > div").forEach(s=>{const r=parseFloat(s.querySelector(".inv-qty")?.value||1),l=parseFloat(s.querySelector(".inv-price")?.value||0);o+=r*l*100});const i=Math.round(o*n/100),a=o+i;e.innerHTML=`
    <div style="font-size:.82rem;color:var(--text-3)">Sous-total HT : <strong>${m(o)}</strong></div>
    <div style="font-size:.82rem;color:var(--text-3)">TVA (${n}%) : <strong>${m(i)}</strong></div>
    <div style="font-size:1rem;font-weight:700;color:var(--text);margin-top:4px">Total TTC : ${m(a)}</div>`}async function z(t){const e=document.getElementById("invClient")?.value;if(!e){c.toast("Sélectionnez un client","error");return}const n=document.getElementById("invLines"),o=[];if(n.querySelectorAll(":scope > div").forEach(i=>{const a=i.querySelector(".inv-desc")?.value?.trim(),s=parseFloat(i.querySelector(".inv-qty")?.value||1),r=parseFloat(i.querySelector(".inv-price")?.value||0),l=i.getAttribute("data-booking-id")||void 0;a&&r!==0&&o.push({description:a,quantity:s,unit_price_cents:Math.round(r*100),booking_id:l})}),o.length===0){c.toast("Ajoutez au moins une ligne","error");return}try{const i={client_id:e,type:t||"invoice",items:o,vat_rate:parseFloat(document.getElementById("invVat")?.value||21),due_days:parseInt(document.getElementById("invDueDays")?.value||30),client_bce:document.getElementById("invClientBce")?.value?.trim()||void 0,notes:document.getElementById("invNotes")?.value?.trim()||void 0},a=await p.post("/api/invoices",i);document.getElementById("invModal")._dirtyGuard?.markClean(),closeModal("invModal"),c.toast(t==="quote"?"Devis créé !":"Facture créée !","success"),x()}catch(i){c.toast(i.message||"Erreur","error")}}async function M(t,e){if(confirm({sent:"Marquer comme envoyée ?",paid:"Marquer comme payée ?"}[e]||`Changer le statut en "${e}" ?`))try{await p.patch(`/api/invoices/${t}/status`,{status:e}),c.toast("Statut mis à jour","success"),x()}catch(o){c.toast(o.message||"Erreur","error")}}async function q(t){if(confirm("Supprimer ce brouillon ?"))try{await p.delete(`/api/invoices/${t}`),c.toast("Brouillon supprimé","success"),x()}catch(e){c.toast(e.message||"Erreur","error")}}function S(t){const e=localStorage.getItem("genda_token");window.open(`/api/invoices/${t}/pdf?token=${e}`,"_blank")}async function L(t){if(confirm("Créer une facture pour ce rendez-vous ?"))try{const e=await p.post("/api/invoices",{booking_id:t,type:"invoice"});c.toast("Facture créée ! Retrouvez-la dans Facturation.","success"),document.querySelectorAll(".ni").forEach(n=>n.classList.remove("active")),document.querySelector('[data-section="invoices"]')?.classList.add("active"),document.getElementById("pageTitle").textContent="Facturation",x()}catch(e){c.toast(e.message||"Erreur","error")}}async function I(){const t=document.getElementById("invClient")?.value,e=document.getElementById("invUnbilledSection"),n=document.getElementById("invUnbilledList");if(!t||!e||!n){e&&(e.style.display="none");return}try{y=(await p.get(`/api/invoices/unbilled?client_id=${t}`)).bookings||[]}catch{y=[]}if(y.length===0){e.style.display="none";return}e.style.display="",n.innerHTML=y.map((o,i)=>{const a=new Date(o.start_at).toLocaleDateString("fr-BE"),s=o.variant_price_cents??o.service_price_cents??0,r=o.service_name+(o.variant_name?" — "+o.variant_name:"")+" ("+(o.practitioner_name||"?")+") — "+a;return`<label style="display:flex;align-items:center;gap:8px;padding:8px 12px;font-size:.82rem;cursor:pointer;border-bottom:1px solid var(--border-light);background:${i%2===0?"var(--white)":"var(--surface)"}">
      <input type="checkbox" data-unbilled-idx="${i}" onchange="invToggleUnbilled(${i},this.checked)">
      <span style="flex:1">${g(r)}</span>
      <span style="font-weight:600;color:var(--text-2)">${m(s)}</span>
    </label>`}).join("")}function w(t,e){const n=y[t];if(!n)return;const o=document.getElementById("invLines");if(o){if(e){const i=new Date(n.start_at).toLocaleDateString("fr-BE"),a=n.service_name+(n.variant_name?" — "+n.variant_name:"")+" ("+(n.practitioner_name||"")+") — "+i,s=n.variant_price_cents??n.service_price_cents??0;if(_(n.id,a,1,s/100),n.deposit_payment_intent_id&&n.deposit_payment_intent_id.startsWith("pass_")&&n.deposit_amount_cents){const l="Pass "+n.deposit_payment_intent_id.replace("pass_","")+" (déduction)";[...o.querySelectorAll(".inv-desc")].some(v=>v.value===l)||_(n.id,l,1,-(n.deposit_amount_cents/100))}}else o.querySelectorAll(`[data-booking-id="${n.id}"]`).forEach(i=>i.remove());f()}}function _(t,e,n,o){const i=document.getElementById("invLines");if(!i)return;const a=document.createElement("div");a.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",a.setAttribute("data-booking-id",t),a.innerHTML=`
    <input class="inv-desc" value="${g(e)}" style="padding:8px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.82rem">
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,i.appendChild(a)}Object.defineProperty(window,"invoiceFilter",{get(){return d},set(t){d=t}});Object.defineProperty(window,"invoiceType",{get(){return u},set(t){u=t}});C({loadInvoices:x,openInvoiceModal:T,addInvoiceLine:$,updateInvTotals:f,saveInvoice:z,changeInvoiceStatus:M,deleteInvoice:q,downloadInvoicePDF:S,createInvoiceFromBooking:L,invClientChanged:I,invToggleUnbilled:w});export{$ as addInvoiceLine,M as changeInvoiceStatus,L as createInvoiceFromBooking,q as deleteInvoice,S as downloadInvoicePDF,x as loadInvoices,T as openInvoiceModal,z as saveInvoice,f as updateInvTotals};
