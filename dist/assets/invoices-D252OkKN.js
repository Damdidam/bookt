import{a as g,G as u,n as B,d as z}from"./dashboard-B3DX9ZTY.js";let p="all",v="all",b=[];function m(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function y(o){return((o||0)/100).toFixed(2).replace(".",",")+" €"}async function f(){const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;p!=="all"&&t.set("status",p),v!=="all"&&t.set("type",v);const e=await g.get(`/api/invoices?${t}`),i=e.invoices||[],s=e.stats||{};let r="";r+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
      ...
      <select onchange="invoiceFilter=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        ...
        <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
          ...
          <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${k}12;color:${k};font-weight:600">${c[n.status]||n.status}</span></td>
    ...
    </label>`}).join("")}function I(o,t){const e=b[o];if(!e)return;const i=document.getElementById("invLines");if(i){if(t){const s=new Date(e.start_at).toLocaleDateString("fr-BE"),r=e.service_name+(e.variant_name?" — "+e.variant_name:"")+" ("+(e.practitioner_name||"")+") — "+s,a=e.variant_price_cents??e.service_price_cents??0;if(e.pass_covered&&e.pass_info){x(e.id,r+" (inclus abonnement)",1,0);const n=e.pass_info,l="1 crédit consommé — Pass "+n.name+" ("+n.sessions_remaining+"/"+n.sessions_total+" restants)";[...i.querySelectorAll(".inv-desc")].some(d=>d.value===l)||x(e.id,l,1,0)}else if(x(e.id,r,1,a/100),e.promotion_discount_cents>0&&e.promotion_label){const n="Réduction : "+e.promotion_label+(e.promotion_discount_pct?" (-"+e.promotion_discount_pct+"%)":"");x(e.id,n,1,-(e.promotion_discount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id&&(e.deposit_payment_intent_id.startsWith("pi_")||e.deposit_payment_intent_id.startsWith("cs_"))){const n="Acompte payé (Stripe)";[...i.querySelectorAll(".inv-desc")].some(c=>c.value===n)||x(e.id,n,1,-(e.deposit_amount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id&&e.deposit_payment_intent_id.startsWith("gc_")){const l="Acompte payé (Carte cadeau "+e.deposit_payment_intent_id.replace("gc_","")+")";[...i.querySelectorAll(".inv-desc")].some(d=>d.value===l)||x(e.id,l,1,-(e.deposit_amount_cents/100))}}else i.querySelectorAll(`[data-booking-id="${e.id}"]`).forEach(s=>s.remove());h()}}function x(o,t,e,i){const s=document.getElementById("invLines");if(!s)return;const r=document.createElement("div");r.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",r.setAttribute("data-booking-id",o),r.innerHTML=`
