import{a as u,e as s,d as _,s as w,G as l,k as v,l as $}from"./dashboard-DhPwRMVe.js";import{i as E}from"./plan-gate-CWiUKJVs.js";import{I as b}from"./icons-DATqSsmT.js";let x=[],c=null;const k={min_amount:t=>`Panier min ${t||"?"} €`,specific_service:t=>`Service : ${t||"?"}`,first_visit:()=>"Nouveau client",date_range:(t,e)=>`Du ${t||"?"} au ${e||"?"}`,none:()=>"Sans condition"},z={free_service:t=>`Service offert : ${t||"?"}`,discount_pct:t=>`Réduction ${t||"?"}%`,discount_fixed:t=>`Réduction ${t||"?"} €`,info_only:()=>"Info seulement"};function C(t){const e=k[t.condition_type];return e?t.condition_type==="min_amount"?e((t.condition_min_cents/100).toFixed(2).replace(".",",")):t.condition_type==="specific_service"?e(t.condition_service_name||"?"):t.condition_type==="date_range"?e(t.condition_start_date,t.condition_end_date):e():t.condition_type||"—"}function M(t){const e=z[t.reward_type];return e?t.reward_type==="free_service"?e(t.reward_service_name||"?"):t.reward_type==="discount_fixed"?e((t.reward_value/100).toFixed(2).replace(".",",")):e(t.reward_value):t.reward_type||"—"}async function m(){const t=document.getElementById("contentArea");t.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=await u.get("/api/promotions"),i=e.promotions||e||[];x=i,I(t,i)}catch(e){t.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${s(e.message)}</div>`}}function I(t,e){const i=e.length,n=e.filter(r=>r.is_active).length;let o="";o+=`<div class="stats" style="grid-template-columns:repeat(2,1fr)">
  ...
  </div>`;const d=E()||n<1;o+=`<div class="card" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 16px">
    ...
    ${d?'<button onclick="openPromoModal()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nouvelle promotion</button>':'<button disabled style="opacity:.5;cursor:not-allowed" title="Le plan gratuit est limité à 1 promotion active" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nouvelle promotion</button>'}
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px;color:var(--text-2);font-size:.78rem">${s(C(r))}${r.max_uses?`<div style="font-size:.7rem;color:var(--text-4);margin-top:2px">${r.current_uses||0}/${r.max_uses} utilisées</div>`:""}</td>
        <td style="padding:10px;color:var(--text-2);font-size:.78rem">${s(M(r))}</td>
        ...
        <td style="padding:10px;text-align:center;white-space:nowrap">
          ...
          <button onclick="deletePromo('${r.id}','${s(r.title)}')" title="Supprimer" style="background:none;border:none;cursor:pointer;color:var(--red);padding:4px 6px"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:15px;height:15px"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
        </td>
      </tr>`}),o+="</tbody></table></div>"),t.innerHTML=o}async function B(){if(c)return c;try{const t=await u.get("/api/services");c=(t.services||t||[]).filter(e=>e.is_active!==!1)}catch(t){console.error("[promotions] fetch services failed",t),c=[]}return c}async function P(t){v("promoModal");let e=null;if(t&&(e=x.find(p=>p.id===t),!e)){l.toast("Promotion introuvable","error");return}const n=(await B()).map(p=>`<option value="${s(p.id)}">${s(p.name)}</option>`).join(""),o=e?.condition_type||"none",d=e?.reward_type||"info_only",r=e?.display_style||"banner",a=document.createElement("div");a.className="m-overlay open",a.id="promoModal",a.innerHTML=`<div class="m-dialog m-md">
    ...
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="savePromo('${t||""}')">${e?"Enregistrer":"Créer"}</button>
  ...
  </div>`,document.body.appendChild(a),$(document.getElementById("promoModal"),{noBackdropClose:!0})}function y(t,e,i){switch(t){case"min_amount":var n=e?.condition_min_cents?(e.condition_min_cents/100).toFixed(2):"";return`<label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Montant minimum (€)</label>
        <input type="number" id="promoConditionValue" min="0" step="1" value="${s(n)}" placeholder="50" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box">`;case"specific_service":var o=e?.condition_service_id||"",d=i.replace('value="'+o+'"','value="'+o+'" selected');return`<label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Service concerné</label>
        ...
        <input type="number" id="promoRewardValue" min="0" step="1" value="${s(r)}" placeholder="10" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box">`;default:return""}}function T(){const t=document.getElementById("promoConditionType").value,e=document.getElementById("promoConditionFields"),n=(c||[]).map(o=>`<option value="${s(o.id)}">${s(o.name)}</option>`).join("");e.innerHTML=y(t,null,n)}function D(){const t=document.getElementById("promoRewardType").value,e=document.getElementById("promoRewardFields"),n=(c||[]).map(o=>`<option value="${s(o.id)}">${s(o.name)}</option>`).join("");e.innerHTML=g(t,null,n)}function S(){document.querySelectorAll('input[name="promoDisplay"]').forEach(e=>{const i=e.closest("label");e.checked?(i.style.borderColor="var(--primary)",i.style.background="var(--primary-light)"):(i.style.borderColor="var(--border)",i.style.background="var(--surface)")})}async function R(t){const e=document.getElementById("promoTitle").value.trim();if(!e){l.toast("Le titre est requis","error");return}const i=document.getElementById("promoDescription").value.trim(),n=document.getElementById("promoConditionType").value,o=document.getElementById("promoRewardType").value,d=document.querySelector('input[name="promoDisplay"]:checked')?.value||"banner",r=document.getElementById("promoMaxUses")?.value,a={title:e,description:i,condition_type:n,reward_type:o,display_style:d,max_uses:r?parseInt(r):null};if(n==="min_amount"){var p=parseFloat(document.getElementById("promoConditionValue")?.value||0);a.condition_min_cents=Math.round(p*100)}else n==="specific_service"?a.condition_service_id=document.getElementById("promoConditionValue")?.value||null:n==="date_range"&&(a.condition_start_date=document.getElementById("promoConditionDateStart")?.value||null,a.condition_end_date=document.getElementById("promoConditionDateEnd")?.value||null);if(o==="free_service")a.reward_service_id=document.getElementById("promoRewardValue")?.value||null;else if(o==="discount_pct")a.reward_value=parseInt(document.getElementById("promoRewardValue")?.value||0);else if(o==="discount_fixed"){var f=parseFloat(document.getElementById("promoRewardValue")?.value||0);a.reward_value=Math.round(f*100)}try{t?(await u.patch(`/api/promotions/${t}`,a),l.toast("Promotion mise à jour","success")):(await u.post("/api/promotions",a),l.toast("Promotion créée","success")),v("promoModal"),m()}catch(h){l.toast(h.message||"Erreur lors de l'enregistrement","error")}}async function A(t,e){try{await u.patch(`/api/promotions/${t}`,{is_active:e}),l.toast(e?"Promotion activée":"Promotion désactivée","success"),m()}catch(i){l.toast(i.message||"Erreur","error")}}async function F(t,e){if(await w(`Supprimer définitivement la promotion "${e}" ? Cette action est irréversible.`))try{await u.delete(`/api/promotions/${t}`),l.toast("Promotion supprimée","success"),m()}catch(i){l.toast(i.message||"Erreur lors de la suppression","error")}}_({loadPromotions:m,openPromoModal:P,savePromo:R,togglePromo:A,deletePromo:F,promoConditionChanged:T,promoRewardChanged:D,promoDisplayChanged:S});export{m as loadPromotions};
