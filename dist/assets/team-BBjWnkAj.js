import{l as k,s as T,o as b,a as u,G as d,S as I,m as C,f as O,p as G,d as ie}from"./dashboard-rfa-74At.js";import{c as ne}from"./color-swatches-CpFwB_nN.js";import{releaseFocus as j,trapFocus as B}from"./focus-trap-C-UMhpsq.js";import{e as se}from"./swipe-close-BqJgOGc2.js";import{I as h}from"./icons-C4WsiP-A.js";let A=null,x=new Date().getFullYear(),g={},f=new Set,$=[];const v=e=>e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):"",M=e=>e==null?"":String(e).replace(/\\/g,"\\\\").replace(/'/g,"\\'").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/\u2028/g,"\\u2028").replace(/\u2029/g,"\\u2029");function L(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const w={close:h.x,edit:h.edit,tasks:h.clipboard,key:h.key,role:h.role,mail:h.mail,phone:h.phone,calendar:h.calendar,star:h.star,sun:h.sun,hourglass:h.hourglass,plus:h.plus,trash:h.trash,link:h.link,shield:h.shield},P={cdi:"CDI",cdd:"CDD",independant:"Indépendant",stagiaire:"Stagiaire",interim:"Intérim"},J={conge:"Congé",maladie:"Maladie",formation:"Formation",recuperation:"Récup."};function U(e){if(!e||e.length===0)return{label:"—",detail:""};const t=e.length;if(t===7)return{label:"7/7",detail:""};const a=["lun","mar","mer","jeu","ven","sam","dim"],i=[];for(let s=0;s<7;s++)e.includes(s)||i.push(a[s]);if(t===5&&!e.includes(5)&&!e.includes(6))return{label:"Temps plein",detail:""};const o=i.length<=3?i.join(", ")+" off":"";return{label:`${t}/7`,detail:o}}function oe(e){return e<=0?"danger":e<=5?"warn":"ok"}async function E(){const e=document.getElementById("contentArea");e.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const[t,a]=await Promise.all([fetch("/api/practitioners",{headers:{Authorization:"Bearer "+u.getToken()}}),fetch("/api/calendar/connections",{headers:{Authorization:"Bearer "+u.getToken()}}).catch(()=>({ok:!1}))]),i=await t.json(),s=(a.ok?await a.json():{connections:[]}).connections||[],r=i.practitioners||[],c=b.practitioner.toLowerCase();let l=`<div class="tm-list-header">
    ...
    <div class="m-body">
      ...
      <div class="m-panel" id="team-panel-schedule">
        <div class="m-sec">
          ...
          <div id="tm_schedule_editor">${t?'<div style="font-size:.78rem;color:var(--text-4)">Chargement...</div>':z()}</div>
        </div>
      ...
      <div class="m-panel" id="team-panel-settings">
        ...
        ${t?`<div class="m-sec" style="margin-top:16px">
          <div class="m-sec-head"><span class="m-sec-title"><svg class="gi" style="width:12px;height:12px" ${w.calendar.slice(4)}> Synchronisation calendrier</span><span class="m-sec-line"></span></div>
  ...
  </div></div>`;document.body.insertAdjacentHTML("beforeend",r);const c=document.getElementById("teamModalOverlay");C(c,{noBackdropClose:!0}),B(c,()=>S()),se(c.querySelector(".m-dialog"),()=>S()),O(c),document.getElementById("p_color_wrap").innerHTML=ne("p_color",e?.color||"#1E3A8A",!1);const l=document.getElementById("p_color");if(l&&l.addEventListener("change",()=>{const n=l.value||"#0D7377",p=document.getElementById("tmHeaderBg"),y=document.getElementById("tmAvatar");p&&(p.style.background=`linear-gradient(135deg,${n} 0%,${n}AA 60%,${n}55 100%)`),y&&(y.style.background=`linear-gradient(135deg,${n},${n}CC)`)}),e?.id,t)window.loadPracCalSync&&window.loadPracCalSync(e.id),K(e.id,x),Z(e.id);else{g={};for(let n=0;n<7;n++)g[n]=[]}}async function S(){j(),await k("teamModalOverlay")}function le(e){document.querySelectorAll("#teamModalOverlay .m-tab").forEach(t=>{t.classList.toggle("active",t.dataset.tab===e)}),document.querySelectorAll("#teamModalOverlay .m-panel").forEach(t=>t.classList.remove("active")),document.getElementById("team-panel-"+e)?.classList.add("active")}function D(){if($.length===0)return`<div style="font-size:.78rem;color:var(--text-4);padding:12px 0">Aucune prestation créée. <a href="#" onclick="event.preventDefault();window.loadSection&&window.loadSection('services')" style="color:var(--primary)">Créer des prestations</a></div>`;const e={},t=[];$.forEach(s=>{const r=s.category||"Sans catégorie";e[r]||(e[r]=[],t.push(r)),e[r].push(s)});const a=$.length,i=f.size;let o=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
    ...
    <button class="m-btn m-btn-ghost" style="font-size:.7rem;padding:4px 10px" onclick="teamToggleAllServices()">${i===a?"Tout désélectionner":"Tout sélectionner"}</button>
      ...
      <label class="svc-assign-cat" onclick="event.preventDefault();teamToggleCatServices('${M(s)}')">
        ...
        <span class="svc-assign-cat-name">${L(s)}</span>
    ...
    <thead><tr><th>Type</th><th>Quota</th><th>Pris</th><th>Solde</th></tr></thead><tbody>`;return t.forEach(i=>{const o=e?.[i]||{total:0,used:0},s=(o.total||0)-(o.used||0),r=oe(s);a+=`<tr>
      ...
      <div class="m-header" style="flex-shrink:0">
        ...
        <div class="m-header-content">
          <div class="m-client-hero">
            <div class="m-avatar" style="background:var(--primary)"><svg class="gi" style="width:20px;height:20px;stroke:#fff;fill:none;stroke-width:2" ${w.tasks.slice(4)}></div>
      ...
      <div class="m-body" style="overflow-y:auto;flex:1">`;p+=`<div class="m-sec"><div class="m-sec-head"><span class="m-sec-title">Tâches en cours (${r.length})</span><span class="m-sec-line"></span></div>`,r.length===0?p+='<div style="font-size:.8rem;color:var(--text-4)">Aucune tâche en cours</div>':r.forEach(m=>{const _=m.booking_start?new Date(m.booking_start).toLocaleDateString("fr-BE",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit",timeZone:"Europe/Brussels"}):"";p+=`<div style="padding:8px 0;border-bottom:1px solid var(--border-light);display:flex;gap:10px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            <div style="font-size:.82rem">${L(m.content)}</div>
          ...
          <div style="font-size:.8rem;text-decoration:line-through">${L(m.content)}</div>
          ...
          <div style="font-size:.7rem;color:var(--text-4)">${m.client_name||""} ${m.service_name?"· "+m.service_name:""} ${m.message?"· "+L(m.message):""}</div>
        </div>`}),n.length>0&&(p+=`<div style="font-size:.72rem;color:var(--text-4);margin-top:8px">${n.length} rappel${n.length>1?"s":""} déjà envoyé${n.length>1?"s":""}</div>`),p+="</div>",p+="</div></div></div>",document.body.insertAdjacentHTML("beforeend",p);const y=document.getElementById("tasksModalOverlay");C(y,{noBackdropClose:!0}),B(y,()=>H())}catch(a){d.toast("Erreur: "+a.message,"error")}}function H(){j(),k("tasksModalOverlay"),document.querySelector(".m-overlay.open")||document.body.classList.remove("has-modal")}async function ke(e,t,a,i,o){try{await fetch(`/api/bookings/${t}/todos/${e}`,{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:"Bearer "+u.getToken()},body:JSON.stringify({is_done:a})}),H(),X(i,o)}catch{d.toast("Erreur","error")}}function we(e,t){const a=I[G]||I.autre;let i=`<div class="m-overlay open" id="inviteModalOverlay"><div class="m-dialog m-sm">
    <div class="m-header-simple">
      <h3>Créer un accès — ${t}</h3>
      <button class="m-close" onclick="closeInviteModal()" aria-label="Fermer">${w.close}</button>
    </div>
    <div class="m-body">
    ...
    <div class="m-bottom">
      ...
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div>
          ...
          ${r?`<div style="font-size:.68rem;color:var(--green)">${h.check} ${N(r.email||"Connecté")}${r.last_sync_at?" · "+new Date(r.last_sync_at).toLocaleDateString("fr-BE",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit",timeZone:"Europe/Brussels"}):""}</div>`:'<div style="font-size:.68rem;color:var(--text-4)">Non connecté</div>'}
        </div>
      </div>
      <div style="display:flex;gap:4px">
        ${r?`
