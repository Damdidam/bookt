import{a as c,d as E,G as i}from"./dashboard-B3DX9ZTY.js";let u="all",m="",b=[];function l(t){return String(t||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function P(t){return((t||0)/100).toFixed(2).replace(".",",")+" €"}async function v(){const t=document.getElementById("contentArea");t.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;u!=="all"&&e.set("status",u),m.trim()&&e.set("search",m.trim());const s=await c.get(`/api/passes?${e}`),r=s.passes||[];b=r;const a=s.stats||{};B(t,r,a)}catch(e){t.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function B(t,e,s){e=e||b,s=s||{};let r="";r+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    ...
    <button onclick="openCreatePass()" class="btn-primary"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>
      ...
      </tr>`}),r+="</tbody></table></div>"}t.innerHTML=r}async function I(){const t=document.getElementById("passCreateModal");t&&t.remove();let e=[];try{const a=await c.get("/api/services");e=(a.services||a||[]).filter(n=>n.is_active!==!1)}catch(a){console.error("[passes] fetch services failed",a),i.toast("Impossible de charger les prestations: "+a.message,"error")}const s=e.map(a=>`<option value="${l(a.id)}">${l(a.name)}${a.price_cents!=null?" — "+P(a.price_cents):""}</option>`).join(""),r=document.createElement("div");r.className="m-overlay open",r.id="passCreateModal",r.innerHTML=`<div class="m-dialog m-md">
  ...
  </div>`,document.body.appendChild(r)}function C(){const t=document.getElementById("passServiceSelect"),e=document.getElementById("passManualNameRow");t.value?e.style.display="none":e.style.display=""}async function S(){const t=document.getElementById("passServiceSelect").value.trim(),e=document.getElementById("passNameManual").value.trim(),s=parseInt(document.getElementById("passSessionsTotal").value),r=parseFloat(document.getElementById("passAmountEur").value),a=document.getElementById("passBuyerName").value.trim(),n=document.getElementById("passBuyerEmail").value.trim(),d=document.getElementById("passExpiresAt").value,p=document.getElementById("passServiceSelect"),o=t?p.options[p.selectedIndex].text.split(" — ")[0]:e;if(!o){i.toast("Veuillez choisir une prestation ou saisir un nom","error");return}if(!s||s<1){i.toast("Veuillez saisir un nombre de séances valide","error");return}if(!r||r<=0){i.toast("Veuillez saisir un prix valide","error");return}if(!a){i.toast("Veuillez saisir le nom du client","error");return}try{await c.post("/api/passes",{service_id:t||void 0,name:o,sessions_total:s,price_cents:Math.round(r*100),buyer_name:a,buyer_email:n||void 0,expires_at:d||void 0}),document.getElementById("passCreateModal").remove(),i.toast("Pass créé avec succès","success"),v()}catch(g){i.toast(g.message||"Erreur lors de la création","error")}}function M(t){const e=b.find(n=>n.id===t);if(!e){i.toast("Pass introuvable","error");return}const s=document.getElementById("passDebitModal");s&&s.remove();const r=parseInt(e.sessions_remaining||0),a=document.createElement("div");a.className="m-overlay open",a.id="passDebitModal",a.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(a)}async function _(t){try{await c.post(`/api/passes/${t}/debit`),document.getElementById("passDebitModal").remove(),i.toast("Séance débitée avec succès","success"),v()}catch(e){i.toast(e.message||"Erreur lors du débit","error")}}async function A(t){const e=b.find(d=>d.id===t);if(!e){i.toast("Pass introuvable","error");return}const s=document.getElementById("passRefundModal");s&&s.remove();const r=parseInt(e.sessions_remaining||0),a=parseInt(e.sessions_total||0),n=document.createElement("div");n.className="m-overlay open",n.id="passRefundModal",n.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(n)}async function R(t){try{await c.post(`/api/passes/${t}/refund`),document.getElementById("passRefundModal").remove(),i.toast("Séance remboursée avec succès","success"),v()}catch(e){i.toast(e.message||"Erreur lors du remboursement","error")}}async function N(t){const e=b.find(s=>s.id===t);if(!e){i.toast("Pass introuvable","error");return}if(confirm(`Annuler le pass ${e.code} ? Cette action est irréversible.`))try{await c.patch(`/api/passes/${t}`,{status:"cancelled"}),i.toast("Pass annulé","success"),v()}catch(s){i.toast(s.message||"Erreur lors de l'annulation","error")}}async function j(t,e){if(confirm(`Supprimer définitivement le pass ${e} ? Cette action est irréversible.`))try{await c.delete(`/api/passes/${t}`),i.toast("Pass supprimé","success"),v()}catch(s){i.toast(s.message||"Erreur lors de la suppression","error")}}function D(t){u=t}function T(t){m=t}Object.defineProperty(window,"passFilter",{get(){return u},set(t){u=t},configurable:!0});Object.defineProperty(window,"passSearch",{get(){return m},set(t){m=t},configurable:!0});E({loadPasses:v,openCreatePass:I,submitCreatePass:S,passServiceChanged:C,openDebitPass:M,debitPass:_,refundPass:A,submitRefundPass:R,cancelPass:N,deletePass:j,setPassFilter:D,passSearchInput:T});export{v as loadPasses,B as renderPasses};
