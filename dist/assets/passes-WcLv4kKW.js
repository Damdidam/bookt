import{a as c,d as _,s as z,G as a,k as u,l as f}from"./dashboard-Bo9qCwvA.js";import{i as S,s as R}from"./plan-gate-Badg7tzn.js";let x="all",b="",g=[];function i(s){return String(s||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function B(s){return((s||0)/100).toFixed(2).replace(".",",")+" €"}let h=!0;async function m(){if(!S()){R(document.getElementById("contentArea"),"Abonnements");return}const s=document.getElementById("contentArea");s.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;x!=="all"&&e.set("status",x),b.trim()&&e.set("search",b.trim());const o=await c.get(`/api/passes?${e}`),t=o.passes||[];g=t,h=o.feature_enabled!==!1;const n=o.stats||{};A(s,t,n)}catch(e){s.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${i(e.message)}</div>`}}function A(s,e,o){e=e||g,o=o||{};let t="";h||(t+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
  ...
  </div>`;const v=[{v:"all",l:"Tous"},{v:"active",l:"Actifs"},{v:"used",l:"Utilisés"},{v:"expired",l:"Expirés"},{v:"cancelled",l:"Annulés"}].map(d=>{const l=x===d.v;return`<button onclick="passFilter='${d.v}';loadPasses()" class="btn-sm${l?" active":""}">${d.l}</button>`}).join("");if(t+=`<div class="card" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 16px">
    ${v}
    <div style="flex:1"></div>
    <input type="text" placeholder="Rechercher par code ou nom..." value="${i(b)}" onkeydown="if(event.key==='Enter'){passSearch=this.value;loadPasses()}" onblur="passSearchInput(this.value)" style="padding:6px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem;min-width:200px">
    ${h?'<button onclick="openCreatePass()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>':'<button disabled title="Activez la fonctionnalité dans les Paramètres" class="btn-primary btn-sm" style="opacity:.5;cursor:not-allowed"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>'}
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${y}12;color:${y};font-weight:600">${l[r.status]||r.status}</span></td>
        <td style="padding:10px;text-align:center;white-space:nowrap">${p}</td>
      </tr>`}),t+="</tbody></table></div>"}s.innerHTML=t}async function I(){u("passCreateModal");let s=[];try{const t=await c.get("/api/services");s=(t.services||t||[]).filter(n=>n.is_active!==!1)}catch(t){console.error("[passes] fetch services failed",t),a.toast("Impossible de charger les prestations: "+t.message,"error")}const e=s.map(t=>`<option value="${i(t.id)}">${i(t.name)}${t.price_cents!=null?" — "+B(t.price_cents):""}</option>`).join(""),o=document.createElement("div");o.className="m-overlay open",o.id="passCreateModal",o.innerHTML=`<div class="m-dialog m-md">
  ...
  </div>`,document.body.appendChild(o),f(o,{noBackdropClose:!0})}function F(){const s=document.getElementById("passServiceSelect"),e=document.getElementById("passManualNameRow");s.value?e.style.display="none":e.style.display=""}async function j(){const s=document.getElementById("passServiceSelect").value.trim(),e=document.getElementById("passNameManual").value.trim(),o=parseInt(document.getElementById("passSessionsTotal").value),t=parseFloat(document.getElementById("passAmountEur").value),n=document.getElementById("passBuyerName").value.trim(),v=document.getElementById("passBuyerEmail").value.trim(),d=document.getElementById("passExpiresAt").value,l=document.getElementById("passServiceSelect"),r=s?l.options[l.selectedIndex].text.split(" — ")[0]:e;if(!r){a.toast("Veuillez choisir une prestation ou saisir un nom","error");return}if(!o||o<1){a.toast("Veuillez saisir un nombre de séances valide","error");return}if(!t||t<=0){a.toast("Veuillez saisir un prix valide","error");return}if(!n){a.toast("Veuillez saisir le nom du client","error");return}try{await c.post("/api/passes",{service_id:s||void 0,name:r,sessions_total:o,price_cents:Math.round(t*100),buyer_name:n,buyer_email:v||void 0,expires_at:d||void 0}),u("passCreateModal"),a.toast("Pass créé avec succès","success"),m()}catch(y){a.toast(y.message||"Erreur lors de la création","error")}}function D(s){const e=g.find(n=>n.id===s);if(!e){a.toast("Pass introuvable","error");return}u("passDebitModal");const o=parseInt(e.sessions_remaining||0),t=document.createElement("div");t.className="m-overlay open",t.id="passDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
      <h3>Débiter — ${i(e.code)}</h3>
    ...
    <div class="m-body">
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
        ...
        <div style="font-size:.75rem;color:var(--text-3);margin-top:2px">${i(e.service_name||"—")}</div>
      </div>
  ...
  </div>`,document.body.appendChild(t),f(t,{noBackdropClose:!0})}async function N(s){try{await c.post(`/api/passes/${s}/debit`),u("passDebitModal"),a.toast("Séance débitée avec succès","success"),m()}catch(e){a.toast(e.message||"Erreur lors du débit","error")}}async function L(s){const e=g.find(v=>v.id===s);if(!e){a.toast("Pass introuvable","error");return}u("passRefundModal");const o=parseInt(e.sessions_remaining||0),t=parseInt(e.sessions_total||0),n=document.createElement("div");n.className="m-overlay open",n.id="passRefundModal",n.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
      <h3>Rembourser — ${i(e.code)}</h3>
  ...
  </div>`,document.body.appendChild(n),f(n,{noBackdropClose:!0})}async function T(s){try{await c.post(`/api/passes/${s}/refund`),u("passRefundModal"),a.toast("Séance remboursée avec succès","success"),m()}catch(e){a.toast(e.message||"Erreur lors du remboursement","error")}}async function H(s){const e=g.find(r=>r.id===s);if(!e){a.toast("Pass introuvable","error");return}const o=parseInt(e.sessions_remaining||0),t=parseInt(e.sessions_total||0),n=t>0?Math.round((e.price_cents||0)/t):0,d=(o*n/100).toFixed(2).replace(".",",")+" €",l=document.createElement("div");l.className="m-overlay open",l.id="passFullRefundModal",l.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
      <h3>Remboursement total — ${i(e.code)}</h3>
  ...
  </div>`,document.body.appendChild(l),f(l,{noBackdropClose:!0})}async function q(s){try{const e=document.getElementById("fullRefundReason")?.value?.trim()||null,o=await c.post(`/api/passes/${s}/refund-full`,{reason:e});if(u("passFullRefundModal"),o.stripe_refund_cents!=null){const t=(o.stripe_refund_cents/100).toFixed(2).replace(".",",")+" €";a.toast(`Pass remboursé (${t} Stripe${o.stripe_fees_cents>0?" — frais "+(o.stripe_fees_cents/100).toFixed(2).replace(".",",")+" €":""})`,"success")}else a.toast("Pass annulé (pas de paiement Stripe à rembourser)","success");m()}catch(e){a.toast(e.message||"Erreur lors du remboursement total","error")}}async function V(s){const e=g.find(t=>t.id===s);if(!e){a.toast("Pass introuvable","error");return}if(await z("Annuler le pass",`Annuler le pass ${e.code} ? Cette action est irréversible.`,"Annuler le pass","danger"))try{await c.patch(`/api/passes/${s}`,{status:"cancelled"}),a.toast("Pass annulé","success"),m()}catch(t){a.toast(t.message||"Erreur lors de l'annulation","error")}}async function U(s,e){if(await z("Supprimer le pass",`Supprimer définitivement le pass ${e} ? Cette action est irréversible.`,"Supprimer","danger"))try{await c.delete(`/api/passes/${s}`),a.toast("Pass supprimé","success"),m()}catch(t){a.toast(t.message||"Erreur lors de la suppression","error")}}function G(s){x=s}function O(s){b=s}Object.defineProperty(window,"passFilter",{get(){return x},set(s){x=s},configurable:!0});Object.defineProperty(window,"passSearch",{get(){return b},set(s){b=s},configurable:!0});_({loadPasses:m,openCreatePass:I,submitCreatePass:j,passServiceChanged:F,openDebitPass:D,debitPass:N,refundPass:L,submitRefundPass:T,fullRefundPass:H,submitFullRefundPass:q,cancelPass:V,deletePass:U,setPassFilter:G,passSearchInput:O});export{m as loadPasses,A as renderPasses};
