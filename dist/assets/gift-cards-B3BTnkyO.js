import{a as v,d as E,G as n,s as A,k as c,l as f}from"./dashboard-_Nf_16Vk.js";import{i as B,s as G}from"./plan-gate-CWiUKJVs.js";let u="all",m="",x=[];function s(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function i(o){return((o||0)/100).toFixed(2).replace(".",",")+" €"}async function g(){if(!B()){G(document.getElementById("contentArea"),"Cartes cadeau");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;u!=="all"&&e.set("status",u),m.trim()&&e.set("search",m.trim());const t=await v.get(`/api/gift-cards?${e}`),r=t.gift_cards||[];x=r;const l=t.stats||{};_(o,r,l)}catch(e){o.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${s(e.message)}</div>`}}function _(o,e,t){e=e||x,t=t||{};let r="";r+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    <div class="stat-card"><div class="label">Total vendu</div><div class="val" style="color:var(--primary)">${i(parseInt(t.total_sold_cents||0))}</div><div class="sub">${t.total||0} cartes</div></div>
    <div class="stat-card"><div class="label">Solde restant</div><div class="val" style="color:var(--gold)">${i(parseInt(t.total_balance_cents||0))}</div><div class="sub">non utilisé</div></div>
    ...
    <button onclick="openCreateGiftCardModal()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer une carte</button>
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px;text-align:right;font-weight:600">${i(a.amount_cents)}</td>
        <td style="padding:10px;text-align:right;font-weight:600;color:${w>0?"var(--green)":"var(--text-4)"}">${i(w)}</td>
  ...
  </div>`,document.body.appendChild(t),f(t,{noBackdropClose:!0})}function I(o){document.getElementById("gcSelectedAmount").value=o,document.querySelectorAll(".gc-amount-pill").forEach(t=>{parseInt(t.dataset.cents)===o?(t.style.background="var(--primary)",t.style.color="#fff",t.style.borderColor="var(--primary)"):(t.style.background="var(--surface)",t.style.color="var(--text-1)",t.style.borderColor="var(--border)")});const e=document.getElementById("gcCustomAmount");e&&!e.matches(":focus")&&(e.value="")}async function R(){const o=parseInt(document.getElementById("gcSelectedAmount").value),e=document.getElementById("gcRecipientName").value.trim(),t=document.getElementById("gcRecipientEmail").value.trim(),r=document.getElementById("gcMessage").value.trim();if(!o||o<=0){n.toast("Veuillez sélectionner un montant","error");return}if(!e){n.toast("Veuillez saisir le nom du destinataire","error");return}try{await v.post("/api/gift-cards",{amount_cents:o,recipient_name:e,recipient_email:t||void 0,message:r||void 0}),c("gcCreateModal"),n.toast("Carte cadeau créée avec succès","success"),g()}catch(l){n.toast(l.message||"Erreur lors de la création","error")}}function S(o){const e=x.find(r=>r.id===o);if(!e){n.toast("Carte introuvable","error");return}c("gcDebitModal");const t=document.createElement("div");t.className="m-overlay open",t.id="gcDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-body">
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
        ...
        <div style="font-size:1.2rem;font-weight:700;color:var(--green)">${i(e.balance_cents)}</div>
      </div>
      <div style="margin-bottom:14px">
  ...
  </div>`,document.body.appendChild(t),f(t,{noBackdropClose:!0})}async function j(o){const e=parseFloat(document.getElementById("gcDebitAmount").value),t=document.getElementById("gcDebitNote").value.trim();if(!e||e<=0){n.toast("Veuillez saisir un montant valide","error");return}try{await v.post(`/api/gift-cards/${o}/debit`,{amount_cents:Math.round(e*100),note:t||void 0}),c("gcDebitModal"),n.toast("Carte débitée avec succès","success"),g()}catch(r){n.toast(r.message||"Erreur lors du débit","error")}}async function N(o){const e=x.find(l=>l.id===o);if(!e){n.toast("Carte introuvable","error");return}c("gcRefundModal");const t=parseInt(e.amount_cents||0)-parseInt(e.balance_cents||0);if(t<=0){n.toast("Aucun montant à rembourser","error");return}const r=document.createElement("div");r.className="m-overlay open",r.id="gcRefundModal",r.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
    ...
    <div class="m-body">
      <div style="margin-bottom:14px;padding:12px;background:var(--surface);border-radius:var(--radius-xs)">
        <div style="display:flex;justify-content:space-between;font-size:.82rem">
          <span style="color:var(--text-3)">Montant initial</span><span style="font-weight:600">${i(e.amount_cents)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-top:4px">
          <span style="color:var(--text-3)">Solde actuel</span><span style="font-weight:600;color:var(--green)">${i(e.balance_cents)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-top:4px">
          <span style="color:var(--text-3)">Montant utilisé</span><span style="font-weight:600">${i(t)}</span>
        </div>
      </div>
      <div style="margin-bottom:14px">
  ...
  </div>`,document.body.appendChild(r),f(r,{noBackdropClose:!0})}async function F(o){const e=parseFloat(document.getElementById("gcRefundAmount").value),t=document.getElementById("gcRefundNote").value.trim();if(!e||e<=0){n.toast("Veuillez saisir un montant valide","error");return}try{await v.post(`/api/gift-cards/${o}/refund`,{amount_cents:Math.round(e*100),note:t||void 0}),c("gcRefundModal"),n.toast("Remboursement effectué","success"),g()}catch(r){n.toast(r.message||"Erreur lors du remboursement","error")}}async function L(o){const e=x.find(r=>r.id===o);if(!e){n.toast("Carte introuvable","error");return}if(await A("Annuler la carte",`Annuler la carte ${e.code} ? Cette action est irréversible.`,"Annuler la carte","danger"))try{await v.patch(`/api/gift-cards/${o}`,{status:"cancelled"}),n.toast("Carte annulée","success"),g()}catch(r){n.toast(r.message||"Erreur lors de l'annulation","error")}}Object.defineProperty(window,"gcFilter",{get(){return u},set(o){u=o},configurable:!0});Object.defineProperty(window,"gcSearch",{get(){return m},set(o){m=o},configurable:!0});E({loadGiftCards:g,openCreateGiftCardModal:D,selectGcAmount:I,submitCreateGiftCard:R,openDebitGiftCard:S,submitDebitGiftCard:j,refundGiftCard:N,submitRefundGiftCard:F,cancelGiftCard:L});export{g as loadGiftCards,_ as renderGiftCards};
