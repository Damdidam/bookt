import{a as c,G as l,o as w,d as I}from"./dashboard-uXvqmNyj.js";let s="all",d="all";function v(e){return String(e||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function u(e){return((e||0)/100).toFixed(2).replace(".",",")+" €"}async function g(){const e=document.getElementById("contentArea");e.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;s!=="all"&&t.set("status",s),d!=="all"&&t.set("type",d);const a=await c.get(`/api/invoices?${t}`),r=a.invoices||[],i=a.stats||{};let o="";o+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
    ...
    </div>`,r.length===0?o+='<div class="card"><div class="empty">Aucune facture. Créez votre première facture ou devis !</div></div>':(o+=`<div class="card" style="padding:0;overflow:auto"><table style="width:100%;border-collapse:collapse;font-size:.82rem">
      ...
      </div>`),e.innerHTML=o}catch(t){e.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${v(t.message)}</div>`}}async function E(e="invoice"){let t=[];try{const o=await c.get("/api/clients");t=o.clients||o||[]}catch(o){console.warn("Impossible de charger les clients pour la facture:",o.message)}const a=e==="quote",r=a?"Nouveau devis":"Nouvelle facture",i=document.createElement("div");i.className="m-overlay open",i.id="invModal",i.innerHTML=`<div class="m-dialog m-md">
    ...
    <div class="m-body" style="display:grid;gap:12px">
      <div>
        ...
        <select class="m-input" id="invClient">
          ...
          ${t.map(o=>`<option value="${o.id}">${v(o.full_name)}${o.email?" ("+v(o.email)+")":""}</option>`).join("")}
        </select>
      </div>
      ...
      <div class="m-row m-row-2">
        ...
        <div>
          <label class="m-field-label">${a?"Validité (jours)":"Échéance (jours)"}</label>
    ...
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="saveInvoice('${e}')">${a?"Créer le devis":"Créer la facture"}</button>
    </div>
  </div>`,document.body.appendChild(i),w(document.getElementById("invModal"),{noBackdropClose:!0}),b(),f()}function b(){const e=document.getElementById("invLines");if(!e)return;const t=document.createElement("div");t.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",t.innerHTML=`
