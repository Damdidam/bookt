import{a as m,e as a,d as f,G as l,n as h}from"./dashboard-CLeyimyY.js";import{I as b}from"./icons-DATqSsmT.js";let x=[],c=null;const _={min_amount:e=>`Panier min ${e||"?"} €`,specific_service:e=>`Service : ${e||"?"}`,first_visit:()=>"Nouveau client",date_range:(e,t)=>`Du ${e||"?"} au ${t||"?"}`,none:()=>"Sans condition"},w={free_service:e=>`Service offert : ${e||"?"}`,discount_pct:e=>`Réduction ${e||"?"}%`,discount_fixed:e=>`Réduction ${e||"?"} €`,info_only:()=>"Info seulement"};function $(e){const t=_[e.condition_type];return t?e.condition_type==="date_range"?t(e.condition_date_start,e.condition_date_end):t(e.condition_value):e.condition_type||"—"}function E(e){const t=w[e.reward_type];return t?t(e.reward_value):e.reward_type||"—"}async function v(){const e=document.getElementById("contentArea");e.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=await m.get("/api/promotions"),o=t.promotions||t||[];x=o,k(e,o)}catch(t){e.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${a(t.message)}</div>`}}function k(e,t){const o=t.length,s=t.filter(i=>i.is_active).length;let r="";r+=`<div class="stats" style="grid-template-columns:repeat(2,1fr)">
    ...
    <div class="m-body">
      ...
      <div style="margin-bottom:14px">
        ...
        <div style="display:flex;gap:10px">
          <label style="flex:1;cursor:pointer;padding:10px;border:2px solid ${d==="cards"?"var(--primary)":"var(--border)"};border-radius:var(--radius-xs);text-align:center;font-size:.82rem;background:${d==="cards"?"var(--primary-light)":"var(--surface)"}">
            <input type="radio" name="promoDisplay" value="cards" ${d==="cards"?"checked":""} style="display:none" onchange="promoDisplayChanged()"> Carte
          </label>
          <label style="flex:1;cursor:pointer;padding:10px;border:2px solid ${d==="banner"?"var(--primary)":"var(--border)"};border-radius:var(--radius-xs);text-align:center;font-size:.82rem;background:${d==="banner"?"var(--primary-light)":"var(--surface)"}">
            <input type="radio" name="promoDisplay" value="banner" ${d==="banner"?"checked":""} style="display:none" onchange="promoDisplayChanged()"> Bannière
          </label>
        </div>
      </div>
    </div>
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="savePromo('${e||""}')">${o?"Enregistrer":"Créer"}</button>
        ...
        </select>`;case"date_range":var n=t?.condition_start_date?t.condition_start_date.substring(0,10):"",d=t?.condition_end_date?t.condition_end_date.substring(0,10):"";return`<div style="display:flex;gap:12px">
      ...
      </div>`;default:return""}}function g(e,t,o){switch(e){case"free_service":var s=t?.reward_service_id||"",r=o.replace('value="'+s+'"','value="'+s+'" selected');return`<label style="font-size:.78rem;font-weight:600;color:var(--text-2);display:block;margin-bottom:6px">Service offert</label>
