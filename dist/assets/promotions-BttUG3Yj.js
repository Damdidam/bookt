import{a as u,e as s,d as _,s as w,G as l,l as b,m as $}from"./dashboard-BaXwXSBA.js";import{i as E}from"./plan-gate-Badg7tzn.js";import{I as x}from"./icons-C4WsiP-A.js";let v=[],c=null;const k={min_amount:t=>`Panier min ${t||"?"} €`,specific_service:t=>`Service : ${t||"?"}`,first_visit:()=>"Nouveau client",date_range:(t,e)=>`Du ${t||"?"} au ${e||"?"}`,none:()=>"Sans condition"},z={free_service:t=>`Service offert : ${t||"?"}`,discount_pct:t=>`Réduction ${t||"?"}%`,discount_fixed:t=>`Réduction ${t||"?"} €`,info_only:()=>"Info seulement"};function C(t){const e=k[t.condition_type];return e?t.condition_type==="min_amount"?e((t.condition_min_cents/100).toFixed(2).replace(".",",")):t.condition_type==="specific_service"?e(t.condition_service_name||"?"):t.condition_type==="date_range"?e(t.condition_start_date,t.condition_end_date):e():t.condition_type||"—"}function M(t){const e=z[t.reward_type];return e?t.reward_type==="free_service"?e(t.reward_service_name||"?"):t.reward_type==="discount_fixed"?e((t.reward_value/100).toFixed(2).replace(".",",")):e(t.reward_value):t.reward_type||"—"}async function m(){const t=document.getElementById("contentArea");t.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=await u.get("/api/promotions"),n=e.promotions||e||[];v=n,I(t,n)}catch(e){t.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${s(e.message)}</div>`}}function I(t,e){const n=e.length,i=e.filter(r=>r.is_active).length;let o="";o+=`<div class="stats" style="grid-template-columns:repeat(2,1fr)">
  ...
  </div>`;const d=E()||i<1;o+=`<div class="card" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 16px">
    ...
    ${d?'<button onclick="openPromoModal()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nouvelle promotion</button>':'<button disabled style="opacity:.5;cursor:not-allowed" title="Le plan gratuit est limité à 1 promotion active" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nouvelle promotion</button>'}
      ...
      </tr>`}),o+="</tbody></table></div>"),t.innerHTML=o}async function B(){if(c)return c;try{const t=await u.get("/api/services");c=(t.services||t||[]).filter(e=>e.is_active!==!1)}catch(t){console.error("[promotions] fetch services failed",t),c=[]}return c}async function P(t){b("promoModal");let e=null;if(t&&(e=v.find(p=>p.id===t),!e)){l.toast("Promotion introuvable","error");return}const i=(await B()).map(p=>`<option value="${s(p.id)}">${s(p.name)}</option>`).join(""),o=e?.condition_type||"none",d=e?.reward_type||"info_only",r=e?.display_style||"banner",a=document.createElement("div");a.className="m-overlay open",a.id="promoModal",a.innerHTML=`<div class="m-dialog m-md">
    <div class="m-header-simple">
      <h3>${e?"Modifier la promotion":"Nouvelle promotion"}</h3>
      <button class="m-close" onclick="closeModal('promoModal')">${x.x}</button>
    </div>
    <div class="m-body">
    ...
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="savePromo('${t||""}')">${e?"Enregistrer":"Créer"}</button>
        ...
        <input type="number" id="promoRewardValue" min="0" step="1" value="${s(r)}" placeholder="10" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box">`;default:return""}}function S(){const t=document.getElementById("promoConditionType").value,e=document.getElementById("promoConditionFields"),i=(c||[]).map(o=>`<option value="${s(o.id)}">${s(o.name)}</option>`).join("");e.innerHTML=y(t,null,i)}function T(){const t=document.getElementById("promoRewardType").value,e=document.getElementById("promoRewardFields"),i=(c||[]).map(o=>`<option value="${s(o.id)}">${s(o.name)}</option>`).join("");e.innerHTML=g(t,null,i)}function D(){document.querySelectorAll('input[name="promoDisplay"]').forEach(e=>{const n=e.closest("label");e.checked?(n.style.borderColor="var(--primary)",n.style.background="var(--primary-light)"):(n.style.borderColor="var(--border)",n.style.background="var(--surface)")})}async function R(t){const e=document.getElementById("promoTitle").value.trim();if(!e){l.toast("Le titre est requis","error");return}const n=document.getElementById("promoDescription").value.trim(),i=document.getElementById("promoConditionType").value,o=document.getElementById("promoRewardType").value,d=document.querySelector('input[name="promoDisplay"]:checked')?.value||"banner",r=document.getElementById("promoMaxUses")?.value,a={title:e,description:n,condition_type:i,reward_type:o,display_style:d,max_uses:r?parseInt(r):null};if(i==="min_amount"){var p=parseFloat(document.getElementById("promoConditionValue")?.value||0);a.condition_min_cents=Math.round(p*100)}else i==="specific_service"?a.condition_service_id=document.getElementById("promoConditionValue")?.value||null:i==="date_range"&&(a.condition_start_date=document.getElementById("promoConditionDateStart")?.value||null,a.condition_end_date=document.getElementById("promoConditionDateEnd")?.value||null);if(o==="free_service")a.reward_service_id=document.getElementById("promoRewardValue")?.value||null;else if(o==="discount_pct")a.reward_value=parseInt(document.getElementById("promoRewardValue")?.value||0);else if(o==="discount_fixed"){var f=parseFloat(document.getElementById("promoRewardValue")?.value||0);a.reward_value=Math.round(f*100)}try{t?(await u.patch(`/api/promotions/${t}`,a),l.toast("Promotion mise à jour","success")):(await u.post("/api/promotions",a),l.toast("Promotion créée","success")),document.getElementById("promoModal")?._dirtyGuard?.markClean(),b("promoModal"),m()}catch(h){l.toast(h.message||"Erreur lors de l'enregistrement","error")}}async function A(t,e){try{await u.patch(`/api/promotions/${t}`,{is_active:e}),l.toast(e?"Promotion activée":"Promotion désactivée","success"),m()}catch(n){l.toast(n.message||"Erreur","error")}}async function F(t){const n=v.find(i=>i.id===t)?.title||"cette promotion";if(await w(`Supprimer définitivement la promotion "${n}" ? Cette action est irréversible.`))try{await u.delete(`/api/promotions/${t}`),l.toast("Promotion supprimée","success"),m()}catch(i){l.toast(i.message||"Erreur lors de la suppression","error")}}_({loadPromotions:m,openPromoModal:P,savePromo:R,togglePromo:A,deletePromo:F,promoConditionChanged:S,promoRewardChanged:T,promoDisplayChanged:D});export{m as loadPromotions};
