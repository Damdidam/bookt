import{a as c,d as R,s as z,G as o,l as p,m as y}from"./dashboard-DXKU8dyQ.js";import{i as E,s as _}from"./plan-gate-Badg7tzn.js";import{r as B,t as F}from"./focus-trap-Vw_KJRtZ.js";import{f as A}from"./format-DoZVPAsZ.js";let b="all",x="",g=[];function l(r){return String(r||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}let h=!0;async function m(){if(!E()){_(document.getElementById("contentArea"),"Abonnements");return}const r=document.getElementById("contentArea");r.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;b!=="all"&&e.set("status",b),x.trim()&&e.set("search",x.trim());const s=await c.get(`/api/passes?${e}`),t=s.passes||[];g=t,h=s.feature_enabled!==!1;const n=s.stats||{};I(r,t,n)}catch(e){r.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function I(r,e,s){e=e||g,s=s||{};let t="";h||(t+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
    ...
    ${h?'<button onclick="openCreatePass()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>':'<button disabled title="Activez la fonctionnalité dans les Paramètres" aria-label="Activez la fonctionnalité dans les Paramètres" class="btn-primary btn-sm" style="opacity:.5;cursor:not-allowed"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>'}
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px;text-align:center;font-weight:600"><span style="color:${k>0?"var(--green)":"var(--text-4)"}">${k}</span><span style="color:var(--text-4);font-weight:400">/${S}</span></td>
  ...
  </div>`,document.body.appendChild(s),y(s,{noBackdropClose:!0})}function D(){const r=document.getElementById("passServiceSelect"),e=document.getElementById("passManualNameRow");r.value?e.style.display="none":e.style.display=""}async function N(){const r=document.getElementById("passServiceSelect").value.trim(),e=document.getElementById("passNameManual").value.trim(),s=parseInt(document.getElementById("passSessionsTotal").value),t=parseFloat(document.getElementById("passAmountEur").value),n=document.getElementById("passBuyerName").value.trim(),v=document.getElementById("passBuyerEmail").value.trim(),d=document.getElementById("passExpiresAt").value,i=document.getElementById("passServiceSelect"),a=r?i.options[i.selectedIndex].text.split(" — ")[0]:e;if(!a){o.toast("Veuillez choisir une prestation ou saisir un nom","error");return}if(!s||s<1){o.toast("Veuillez saisir un nombre de séances valide","error");return}if(!t||t<=0){o.toast("Veuillez saisir un prix valide","error");return}if(!n){o.toast("Veuillez saisir le nom du client","error");return}try{await c.post("/api/passes",{service_id:r||void 0,name:a,sessions_total:s,price_cents:Math.round(t*100),buyer_name:n,buyer_email:v||void 0,expires_at:d||void 0}),p("passCreateModal"),o.toast("Pass créé avec succès","success"),m()}catch(f){o.toast(f.message||"Erreur lors de la création","error")}}function T(r){const e=g.find(n=>n.id===r);if(!e){o.toast("Pass introuvable","error");return}p("passDebitModal");const s=parseInt(e.sessions_remaining||0),t=document.createElement("div");t.className="m-overlay open",t.id="passDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(i),y(i,{noBackdropClose:!0}),F(i,()=>p("passFullRefundModal"))}async function U(r){const e=document.querySelector("#passFullRefundModal .m-btn-danger");if(!e?.disabled){e&&(e.disabled=!0,e.textContent="Traitement...");try{const s=document.getElementById("fullRefundReason")?.value?.trim()||null,t=await c.post(`/api/passes/${r}/refund-full`,{reason:s});if(B(),p("passFullRefundModal"),t.stripe_refund_cents!=null){const n=(t.stripe_refund_cents/100).toFixed(2).replace(".",",")+" €";o.toast(`Pass remboursé (${n} Stripe${t.stripe_fees_cents>0?" — frais "+(t.stripe_fees_cents/100).toFixed(2).replace(".",",")+" €":""})`,"success")}else o.toast("Pass annulé (pas de paiement Stripe à rembourser)","success");m()}catch(s){e&&(e.disabled=!1,e.textContent="Rembourser et annuler"),o.toast(s.message||"Erreur lors du remboursement total","error")}}}async function G(r){const e=g.find(t=>t.id===r);if(!e){o.toast("Pass introuvable","error");return}if(await z("Annuler le pass",`Annuler le pass ${e.code} ? Cette action est irréversible.`,"Annuler le pass","danger"))try{await c.patch(`/api/passes/${r}`,{status:"cancelled"}),o.toast("Pass annulé","success"),m()}catch(t){o.toast(t.message||"Erreur lors de l'annulation","error")}}async function O(r,e){if(await z("Supprimer le pass",`Supprimer définitivement le pass ${e} ? Cette action est irréversible.`,"Supprimer","danger"))try{await c.delete(`/api/passes/${r}`),o.toast("Pass supprimé","success"),m()}catch(t){o.toast(t.message||"Erreur lors de la suppression","error")}}function Z(r){b=r}function J(r){x=r}Object.defineProperty(window,"passFilter",{get(){return b},set(r){b=r},configurable:!0});Object.defineProperty(window,"passSearch",{get(){return x},set(r){x=r},configurable:!0});R({loadPasses:m,openCreatePass:j,submitCreatePass:N,passServiceChanged:D,openDebitPass:T,debitPass:L,refundPass:q,submitRefundPass:H,fullRefundPass:V,submitFullRefundPass:U,cancelPass:G,deletePass:O,setPassFilter:Z,passSearchInput:J});export{m as loadPasses,I as renderPasses};
