import{m as w,s as S,o as b,a as u,G as d,S as I,n as M,f as O,p as G,d as ie}from"./dashboard-N04C3dFD.js";import{c as ne}from"./color-swatches-BdaD7HCX.js";import{releaseFocus as j,trapFocus as A}from"./focus-trap-C-UMhpsq.js";import{e as se}from"./swipe-close-BqJgOGc2.js";import{I as h}from"./icons-C4WsiP-A.js";let z=null,k=new Date().getFullYear(),g={},f=new Set,x=[];const v=e=>e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):"",B=e=>e==null?"":String(e).replace(/\\/g,"\\\\").replace(/'/g,"\\'").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/\u2028/g,"\\u2028").replace(/\u2029/g,"\\u2029");function _(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const E={close:h.x,edit:h.edit,tasks:h.clipboard,key:h.key,role:h.role,mail:h.mail,phone:h.phone,calendar:h.calendar,star:h.star,sun:h.sun,hourglass:h.hourglass,plus:h.plus,trash:h.trash,link:h.link,shield:h.shield},P={cdi:"CDI",cdd:"CDD",independant:"Indépendant",stagiaire:"Stagiaire",interim:"Intérim"},J={conge:"Congé",maladie:"Maladie",formation:"Formation",recuperation:"Récup."};function U(e){if(!e||e.length===0)return{label:"—",detail:""};const t=e.length;if(t===7)return{label:"7/7",detail:""};const a=["lun","mar","mer","jeu","ven","sam","dim"],i=[];for(let s=0;s<7;s++)e.includes(s)||i.push(a[s]);if(t===5&&!e.includes(5)&&!e.includes(6))return{label:"Temps plein",detail:""};const o=i.length<=3?i.join(", ")+" off":"";return{label:`${t}/7`,detail:o}}function oe(e){return e<=0?"danger":e<=5?"warn":"ok"}async function T(){const e=document.getElementById("contentArea");e.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const[t,a]=await Promise.all([fetch("/api/practitioners",{headers:{Authorization:"Bearer "+u.getToken()}}),fetch("/api/calendar/connections",{headers:{Authorization:"Bearer "+u.getToken()}}).catch(()=>({ok:!1}))]),i=await t.json(),s=(a.ok?await a.json():{connections:[]}).connections||[],l=i.practitioners||[],c=b.practitioner.toLowerCase();let r=`<div class="tm-list-header">
      <h3>${l.length} membre${l.length>1?"s":""} de l'équipe</h3>
      <button class="btn-primary btn-sm" onclick="openPractModal()">+ Ajouter</button>
    </div>`;l.length===0?r+=`<div class="card"><div class="empty">Aucun ${c}. Ajoutez votre premier membre !</div></div>`:(r+='<div class="team-grid2">',l.forEach(n=>{const m=n.display_name?.split(" ").map(ae=>ae[0]).join("").toUpperCase().slice(0,2)||"??",y=U(n.work_days),p=!n.is_active;r+=`<div class="tm-card${p?" inactive":""}" onclick="openPractModal('${n.id}')">`,n.photo_url?r+=`<div class="tm-avatar"><img src="${v(n.photo_url)}" alt="${v(n.display_name)}" loading="lazy"></div>`:r+=`<div class="tm-avatar" style="background:linear-gradient(135deg,${v(n.color||"#0D7377")},${v(n.color||"#0D7377")}CC)">${m}</div>`,r+='<div class="tm-info">',r+=`<p class="tm-name">${v(n.display_name)}${p?' <span class="tm-badge-inactive">Inactif</span>':""}</p>`,r+=`<p class="tm-title">${v(n.title||"")}</p>`,y.label!=="—"&&(r+=`<p class="tm-regime">${y.label}${y.detail?" · "+y.detail:""}</p>`);const $=[];n.bookings_30d!=null&&$.push(n.bookings_30d+" RDV/mois"),n.contract_type&&P[n.contract_type]&&$.push(P[n.contract_type]),$.length&&(r+=`<p class="tm-summary">${$.join(" · ")}</p>`),r+="</div>",r+="</div>"}),r+=`<div class="tm-card tm-add" onclick="openPractModal()">
        <div class="tm-avatar tm-add-icon">${E.plus}</div>
        <div class="tm-info"><p class="tm-name">Ajouter un ${v(c)}</p></div>
      </div>`,r+="</div>"),e.innerHTML=r}catch(t){e.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${v(t.message)}</div>`}}function re(e){if(!e&&window._businessPlan==="free"&&document.querySelectorAll(".tm-card:not(.tm-add)").length>=1){d.toast("Passez au Pro pour ajouter des praticiens","error");return}const t={Authorization:"Bearer "+u.getToken()};e?Promise.all([fetch("/api/practitioners",{headers:t}).then(a=>a.json()),fetch("/api/services",{headers:t}).then(a=>a.json())]).then(([a,i])=>{x=(i.services||[]).filter(o=>o.is_active!==!1),F(a.practitioners.find(o=>o.id===e))}):fetch("/api/services",{headers:t}).then(a=>a.json()).then(a=>{x=(a.services||[]).filter(i=>i.is_active!==!1),F(null)})}function F(e){z=null,k=new Date().getFullYear(),f=new Set,e&&x.forEach(n=>{n.practitioner_ids&&n.practitioner_ids.includes(e.id)&&f.add(n.id)});const t=!!e;b.practitioner.toLowerCase();const a=e?.color||"#0D7377",i=e?.display_name?.split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2)||"??",o=e?.photo_url?`<img src="${v(e.photo_url)}" alt="${v(e.display_name)}" style="width:100%;height:100%;object-fit:cover">`:i,s=e?v(e.display_name):"Nouveau "+b.practitioner;let l=`<div id="teamModalOverlay" class="m-overlay open">
        ...
        <div class="m-sec">
          <div class="m-sec-head">
            ...
            <select class="m-input" id="tm_leave_year" style="width:auto;padding:4px 8px;font-size:.72rem" onchange="teamLoadLeave('${e.id}',this.value)">
              ${[k-1,k,k+1].map(n=>`<option value="${n}"${n===k?" selected":""}>${n}</option>`).join("")}
            </select>
          </div>
  ...
  </div></div>`;document.body.insertAdjacentHTML("beforeend",l);const c=document.getElementById("teamModalOverlay");M(c,{noBackdropClose:!0}),A(c,()=>C()),se(c.querySelector(".m-dialog"),()=>C()),O(c),document.getElementById("p_color_wrap").innerHTML=ne("p_color",e?.color||"#1E3A8A",!1);const r=document.getElementById("p_color");if(r&&r.addEventListener("change",()=>{const n=r.value||"#0D7377",m=document.getElementById("tmHeaderBg"),y=document.getElementById("tmAvatar");m&&(m.style.background=`linear-gradient(135deg,${n} 0%,${n}AA 60%,${n}55 100%)`),y&&(y.style.background=`linear-gradient(135deg,${n},${n}CC)`)}),e?.id,t)window.loadPracCalSync&&window.loadPracCalSync(e.id),K(e.id,k),Z(e.id);else{g={};for(let n=0;n<7;n++)g[n]=[]}}async function C(){j(),await w("teamModalOverlay")}function le(e){document.querySelectorAll("#teamModalOverlay .m-tab").forEach(t=>{t.classList.toggle("active",t.dataset.tab===e)}),document.querySelectorAll("#teamModalOverlay .m-panel").forEach(t=>t.classList.remove("active")),document.getElementById("team-panel-"+e)?.classList.add("active")}function D(){if(x.length===0)return`<div style="font-size:.78rem;color:var(--text-4);padding:12px 0">Aucune prestation créée. <a href="#" onclick="event.preventDefault();window.loadSection&&window.loadSection('services')" style="color:var(--primary)">Créer des prestations</a></div>`;const e={},t=[];x.forEach(s=>{const l=s.category||"Sans catégorie";e[l]||(e[l]=[],t.push(l)),e[l].push(s)});const a=x.length,i=f.size;let o=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
    ...
    <button class="m-btn m-btn-ghost" style="font-size:.7rem;padding:4px 10px" onclick="teamToggleAllServices()">${i===a?"Tout désélectionner":"Tout sélectionner"}</button>
  </div>`;return t.forEach(s=>{const l=e[s],c=l.filter(n=>f.has(n.id)).length,r=c===l.length;o+=`<div class="svc-assign-group">
      ...
      </label>`,l.forEach(n=>{const m=f.has(n.id),y=n.price_cents?(n.price_cents/100).toFixed(2).replace(".",",").replace(",00","")+"€":"";o+=`<label class="svc-assign-item${m?" checked":""}" onclick="event.preventDefault();teamToggleService('${n.id}')">
      ...
      </label>`}),o+="</div>"}),o}function ce(e){f.has(e)?f.delete(e):f.add(e),document.getElementById("tm_services_list").innerHTML=D()}function de(e){const t=x.filter(i=>(i.category||"Sans catégorie")===e),a=t.every(i=>f.has(i.id));t.forEach(i=>{a?f.delete(i.id):f.add(i.id)}),document.getElementById("tm_services_list").innerHTML=D()}function me(){f.size===x.length?f.clear():x.forEach(t=>f.add(t.id)),document.getElementById("tm_services_list").innerHTML=D()}const R=["Lundi","Mardi","Mercredi","Jeudi","Vendredi","Samedi","Dimanche"];async function Z(e){try{const o=((await(await fetch(`/api/availabilities?practitioner_id=${e}`,{headers:{Authorization:"Bearer "+u.getToken()}})).json()).availabilities||{})[e];g={};for(let s=0;s<7;s++)g[s]=(o?.schedule?.[s]||[]).map(l=>({start_time:l.start_time,end_time:l.end_time}));document.getElementById("tm_schedule_editor").innerHTML=L()}catch(t){document.getElementById("tm_schedule_editor").innerHTML=`<div style="color:var(--red);font-size:.82rem">Erreur: ${v(t.message)}</div>`}}function L(){const e=[];for(let i=0;i<7;i++)g[i]&&g[i].length>0&&e.push(i);const t=U(e);let a=`<div style="font-size:.82rem;font-weight:600;margin-bottom:10px;display:flex;align-items:center;gap:8px">
    ...
    ${t.detail?`<span style="color:var(--text-4);font-weight:400;font-size:.75rem">(${t.detail})</span>`:""}
      ...
      <div class="slots">`,o.length===0?a+='<span class="day-closed">Fermé</span>':o.forEach((s,l)=>{a+=`<span class="slot-chip"><span class="slot-chip-text" onclick="teamEditSlot(${i},${l})" title="Cliquer pour modifier">${(s.start_time||"").slice(0,5)} – ${(s.end_time||"").slice(0,5)}</span><button class="remove-slot" onclick="teamRemoveSlot(${i},${l})">${E.close}</button></span>`}),a+=`<button class="add-slot-btn" onclick="teamAddSlot(${i})">+ Ajouter</button>
      </div>
  ...
  </div><div class="m-bottom"><div style="flex:1"></div><button class="m-btn m-btn-ghost" onclick="closeModal('teamSlotModal')">Annuler</button><button class="m-btn m-btn-danger" onclick="teamRemoveSlot(${e},${t});closeModal('teamSlotModal')" style="margin-right:auto">Supprimer</button><button class="m-btn m-btn-primary" onclick="teamConfirmEditSlot(${e},${t})">Enregistrer</button></div></div></div>`;document.body.insertAdjacentHTML("beforeend",s),M(document.getElementById("teamSlotModal"),{noBackdropClose:!0}),A(document.getElementById("teamSlotModal"),()=>w("teamSlotModal")),O(document.getElementById("teamSlotModal"))}function ge(e,t){const a=document.getElementById("tm_slot_start").value,i=document.getElementById("tm_slot_end").value;if(!a||!i){d.toast("Heures requises","error");return}if(a>=i){d.toast("L'heure de fin doit être après le début","error");return}if((g[e]||[]).some((r,n)=>n===t?!1:a<(r.end_time||"").slice(0,5)&&i>(r.start_time||"").slice(0,5))){d.toast("Ce créneau chevauche un autre","error");return}const l=a+":00",c=i+":00";g[e]&&g[e][t]&&(g[e][t]={start_time:l,end_time:c},g[e].sort((r,n)=>r.start_time.localeCompare(n.start_time))),w("teamSlotModal"),document.getElementById("tm_schedule_editor").innerHTML=L()}function he(e,t){g[e].splice(t,1),document.getElementById("tm_schedule_editor").innerHTML=L()}function Y(e){const t=["conge","maladie","formation","recuperation"];let a=`<table class="leave-table">
    <thead><tr><th>Type</th><th>Quota</th><th>Pris</th><th>Solde</th></tr></thead><tbody>`;return t.forEach(i=>{const o=e?.[i]||{total:0,used:0},s=(o.total||0)-(o.used||0),l=oe(s);a+=`<tr>
    ...
    </tr>`}),a+="</tbody></table>",a}async function K(e,t){k=parseInt(t);try{const i=await(await fetch(`/api/practitioners/${e}/leave-balance?year=${t}`,{headers:{Authorization:"Bearer "+u.getToken()}})).json();document.getElementById("tm_leave_table").innerHTML=Y(i.balances);const o=document.getElementById("tm_recent_abs");i.recent_absences&&i.recent_absences.length>0?o.innerHTML=i.recent_absences.map(s=>{const l=new Date(s.date_from).toLocaleDateString("fr-BE",{day:"numeric",month:"short",timeZone:"Europe/Brussels"}),c=new Date(s.date_to).toLocaleDateString("fr-BE",{day:"numeric",month:"short",timeZone:"Europe/Brussels"});return`<div style="padding:6px 0;border-bottom:1px solid var(--border-light);display:flex;justify-content:space-between;align-items:center">
  ...
  </div></div>`;document.body.insertAdjacentHTML("beforeend",o);const s=document.getElementById("inviteModalOverlay");M(s,{noBackdropClose:!0}),A(s,()=>q())}async function q(){j(),await w("inviteModalOverlay")}function ee(){const e="ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789",t=window.crypto||window.msCrypto;let a="";if(t&&typeof t.getRandomValues=="function"){const i=new Uint8Array(10);t.getRandomValues(i);for(let o=0;o<10;o++)a+=e[i[o]%e.length]}else{console.warn("[TEAM] window.crypto unavailable — falling back to Math.random for temp pwd");for(let i=0;i<10;i++)a+=e[Math.floor(Math.random()*e.length)]}return a}async function Ee(e){const t=document.getElementById("inv_email").value,a=document.getElementById("inv_pwd").value,i=document.getElementById("inv_role").value;if(!t||!a)return d.toast("Email et mot de passe requis","error");try{const o=await fetch(`/api/practitioners/${e}/invite`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+u.getToken()},body:JSON.stringify({email:t,password:a,role:i})});if(!o.ok)throw new Error((await o.json()).error);document.getElementById("inviteModalOverlay")?._dirtyGuard?.markClean(),q(),d.toast("Compte créé ! Communiquez les identifiants.","success"),T()}catch(o){d.toast("Erreur: "+o.message,"error")}}function Te(e,t,a){const i=_(t),o=I[G]||I.autre,s=[{value:"owner",label:"Propriétaire / Manager",desc:"Accès complet au dashboard"},{value:"practitioner",label:o.practitioner,desc:"Voit uniquement son propre agenda et ses clients"}];let l=`<div class="m-overlay open" id="roleModalOverlay"><div class="m-dialog m-sm">
    <div class="m-header-simple">
      <h3>Modifier le rôle — ${i}</h3>
      <button class="m-close" onclick="closeRoleModal()" aria-label="Fermer">${E.close}</button>
    </div>
    <div class="m-body">
      <div style="display:flex;flex-direction:column;gap:8px">`;s.forEach(r=>{const n=r.value===a?"checked":"",m=r.value===a?"var(--primary)":"var(--border-light)";l+=`<label style="display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border:1.5px solid ${m};border-radius:10px;cursor:pointer;transition:all .15s" onmouseover="this.style.borderColor='var(--primary)'" onmouseout="this.style.borderColor='${r.value===a?"var(--primary)":"var(--border-light)"}'" onclick="this.parentElement.querySelectorAll('label').forEach(l=>l.style.borderColor='var(--border-light)');this.style.borderColor='var(--primary)'">
      <input type="radio" name="role_pick" value="${r.value}" ${n} style="margin-top:2px">
    ...
    </div>`,r+="</div>",r+='<div id="p_ical_url" style="display:none;margin-top:8px"></div>',s||l){const n=(s||l).sync_direction||"both";r+=`<div style="margin-top:10px;display:flex;align-items:center;gap:8px;font-size:.78rem">
