import{a as m,e as a,d as f,s as h,G as l,n as _}from"./dashboard-CAdIVIet.js";import{I as b}from"./icons-DATqSsmT.js";let x=[],c=null;const w={min_amount:e=>`Panier min ${e||"?"} €`,specific_service:e=>`Service : ${e||"?"}`,first_visit:()=>"Nouveau client",date_range:(e,t)=>`Du ${e||"?"} au ${t||"?"}`,none:()=>"Sans condition"},$={free_service:e=>`Service offert : ${e||"?"}`,discount_pct:e=>`Réduction ${e||"?"}%`,discount_fixed:e=>`Réduction ${e||"?"} €`,info_only:()=>"Info seulement"};function E(e){const t=w[e.condition_type];return t?e.condition_type==="min_amount"?t((e.condition_min_cents/100).toFixed(2).replace(".",",")):e.condition_type==="specific_service"?t(e.condition_service_name||"?"):e.condition_type==="date_range"?t(e.condition_start_date,e.condition_end_date):t():e.condition_type||"—"}function k(e){const t=$[e.reward_type];return t?e.reward_type==="free_service"?t(e.reward_service_name||"?"):e.reward_type==="discount_fixed"?t((e.reward_value/100).toFixed(2).replace(".",",")):t(e.reward_value):e.reward_type||"—"}async function v(){const e=document.getElementById("contentArea");e.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=await m.get("/api/promotions"),o=t.promotions||t||[];x=o,z(e,o)}catch(t){e.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${a(t.message)}</div>`}}function z(e,t){const o=t.length,d=t.filter(i=>i.is_active).length;let r="";r+=`<div class="stats" style="grid-template-columns:repeat(2,1fr)">
    ...
    <button onclick="openPromoModal()" class="btn-primary"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nouvelle promotion</button>
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px;color:var(--text-2);font-size:.78rem">${a(k(i))}</td>
    ...
    <div class="m-bottom">
      ...
      <button class="m-btn m-btn-primary" onclick="savePromo('${e||""}')">${o?"Enregistrer":"Créer"}</button>
        ...
        <input type="number" id="promoRewardValue" min="0" step="1" value="${a(n)}" placeholder="10" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.85rem;box-sizing:border-box">`;default:return""}}function I(){const e=document.getElementById("promoConditionType").value,t=document.getElementById("promoConditionFields"),d=(c||[]).map(r=>`<option value="${a(r.id)}">${a(r.name)}</option>`).join("");t.innerHTML=y(e,null,d)}function M(){const e=document.getElementById("promoRewardType").value,t=document.getElementById("promoRewardFields"),d=(c||[]).map(r=>`<option value="${a(r.id)}">${a(r.name)}</option>`).join("");t.innerHTML=g(e,null,d)}function P(){document.querySelectorAll('input[name="promoDisplay"]').forEach(t=>{const o=t.closest("label");t.checked?(o.style.borderColor="var(--primary)",o.style.background="var(--primary-light)"):(o.style.borderColor="var(--border)",o.style.background="var(--surface)")})}async function T(e){const t=document.getElementById("promoTitle").value.trim();if(!t){l.toast("Le titre est requis","error");return}const o=document.getElementById("promoDescription").value.trim(),d=document.getElementById("promoConditionType").value,r=document.getElementById("promoRewardType").value,i=document.querySelector('input[name="promoDisplay"]:checked')?.value||"banner",n={title:t,description:o,condition_type:d,reward_type:r,display_style:i};if(d==="min_amount"){var s=parseFloat(document.getElementById("promoConditionValue")?.value||0);n.condition_min_cents=Math.round(s*100)}else d==="specific_service"?n.condition_service_id=document.getElementById("promoConditionValue")?.value||null:d==="date_range"&&(n.condition_start_date=document.getElementById("promoConditionDateStart")?.value||null,n.condition_end_date=document.getElementById("promoConditionDateEnd")?.value||null);if(r==="free_service")n.reward_service_id=document.getElementById("promoRewardValue")?.value||null;else if(r==="discount_pct")n.reward_value=parseInt(document.getElementById("promoRewardValue")?.value||0);else if(r==="discount_fixed"){var p=parseFloat(document.getElementById("promoRewardValue")?.value||0);n.reward_value=Math.round(p*100)}try{e?(await m.patch(`/api/promotions/${e}`,n),l.toast("Promotion mise à jour","success")):(await m.post("/api/promotions",n),l.toast("Promotion créée","success")),document.getElementById("promoModal").remove(),v()}catch(u){l.toast(u.message||"Erreur lors de l'enregistrement","error")}}async function D(e,t){try{await m.patch(`/api/promotions/${e}`,{is_active:t}),l.toast(t?"Promotion activée":"Promotion désactivée","success"),v()}catch(o){l.toast(o.message||"Erreur","error")}}async function S(e,t){if(await h(`Supprimer définitivement la promotion "${t}" ? Cette action est irréversible.`))try{await m.delete(`/api/promotions/${e}`),l.toast("Promotion supprimée","success"),v()}catch(o){l.toast(o.message||"Erreur lors de la suppression","error")}}f({loadPromotions:v,openPromoModal:B,savePromo:T,togglePromo:D,deletePromo:S,promoConditionChanged:I,promoRewardChanged:M,promoDisplayChanged:P});export{v as loadPromotions};
