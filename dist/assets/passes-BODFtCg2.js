import{a as p,d as E,s as $,G as o,l as d,m as y}from"./dashboard-C_JJSNJO.js";import{i as _,s as B}from"./plan-gate-Badg7tzn.js";import{releaseFocus as F,trapFocus as h}from"./focus-trap-C-UMhpsq.js";import{f as A}from"./format-DoZVPAsZ.js";let b="all",x="",g=[];function l(r){return String(r||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}let w=!0;async function m(){if(!_()){B(document.getElementById("contentArea"),"Abonnements");return}const r=document.getElementById("contentArea");r.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const e=new URLSearchParams;b!=="all"&&e.set("status",b),x.trim()&&e.set("search",x.trim());const s=await p.get(`/api/passes?${e}`),t=s.passes||[];g=t,w=s.feature_enabled!==!1;const n=s.stats||{};I(r,t,n)}catch(e){r.innerHTML=`<div class="empty" style="color:var(--red)">Erreur: ${l(e.message)}</div>`}}function I(r,e,s){e=e||g,s=s||{};let t="";w||(t+=`<div class="card" style="padding:14px 18px;margin-bottom:14px;background:#FFF8E1;border-left:4px solid #F9A825">
  ...
  </div>`;const v=[{v:"all",l:"Tous"},{v:"active",l:"Actifs"},{v:"used",l:"Utilisés"},{v:"expired",l:"Expirés"},{v:"cancelled",l:"Annulés"}].map(c=>{const i=b===c.v;return`<button onclick="passFilter='${c.v}';loadPasses()" class="btn-sm${i?" active":""}">${c.l}</button>`}).join("");if(t+=`<div class="card" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 16px">
    ${v}
    ...
    ${w?'<button onclick="openCreatePass()" class="btn-primary btn-sm"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>':'<button disabled title="Activez la fonctionnalité dans les Paramètres" aria-label="Activez la fonctionnalité dans les Paramètres" class="btn-primary btn-sm" style="opacity:.5;cursor:not-allowed"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Créer un pass</button>'}
      ...
      <thead><tr style="background:var(--surface);border-bottom:1px solid var(--border)">
        ...
        <td style="padding:10px"><div style="font-weight:500">${l(C)}</div>${k?`<div style="font-size:.7rem;color:var(--text-4)">${l(k)}</div>`:""}</td>
        <td style="padding:10px;color:var(--text-2)">${l(M)}</td>
        <td style="padding:10px;text-align:center;font-weight:600"><span style="color:${z>0?"var(--green)":"var(--text-4)"}">${z}</span><span style="color:var(--text-4);font-weight:400">/${R}</span></td>
        <td style="padding:10px;font-size:.78rem;color:var(--text-3)">${P}</td>
        <td style="padding:10px;text-align:center"><span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:${f}12;color:${f};font-weight:600">${i[a.status]||a.status}</span></td>
        <td style="padding:10px;text-align:center;white-space:nowrap">${u}</td>
      </tr>`}),t+="</tbody></table></div>"}r.innerHTML=t}async function j(){d("passCreateModal");let r=[];try{const t=await p.get("/api/services");r=(t.services||t||[]).filter(n=>n.is_active!==!1)}catch(t){console.error("[passes] fetch services failed",t),o.toast("Impossible de charger les prestations: "+t.message,"error")}const e=r.map(t=>`<option value="${l(t.id)}">${l(t.name)}${t.price_cents!=null?" — "+A(t.price_cents):""}</option>`).join(""),s=document.createElement("div");s.className="m-overlay open",s.id="passCreateModal",s.innerHTML=`<div class="m-dialog m-md">
  ...
  </div>`,document.body.appendChild(s),y(s,{noBackdropClose:!0}),h(s,()=>d(s.id))}function D(){const r=document.getElementById("passServiceSelect"),e=document.getElementById("passManualNameRow");r.value?e.style.display="none":e.style.display=""}async function N(){const r=document.getElementById("passServiceSelect").value.trim(),e=document.getElementById("passNameManual").value.trim(),s=parseInt(document.getElementById("passSessionsTotal").value),t=parseFloat(document.getElementById("passAmountEur").value),n=document.getElementById("passBuyerName").value.trim(),v=document.getElementById("passBuyerEmail").value.trim(),c=document.getElementById("passExpiresAt").value,i=document.getElementById("passServiceSelect"),a=r?i.options[i.selectedIndex].text.split(" — ")[0]:e;if(!a){o.toast("Veuillez choisir une prestation ou saisir un nom","error");return}if(!s||s<1){o.toast("Veuillez saisir un nombre de séances valide","error");return}if(!t||t<=0){o.toast("Veuillez saisir un prix valide","error");return}if(!n){o.toast("Veuillez saisir le nom du client","error");return}try{await p.post("/api/passes",{service_id:r||void 0,name:a,sessions_total:s,price_cents:Math.round(t*100),buyer_name:n,buyer_email:v||void 0,expires_at:c||void 0}),d("passCreateModal"),o.toast("Pass créé avec succès","success"),m()}catch(f){o.toast(f.message||"Erreur lors de la création","error")}}function T(r){const e=g.find(n=>n.id===r);if(!e){o.toast("Pass introuvable","error");return}d("passDebitModal");const s=parseInt(e.sessions_remaining||0),t=document.createElement("div");t.className="m-overlay open",t.id="passDebitModal",t.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(t),y(t,{noBackdropClose:!0}),h(t,()=>d(t.id))}async function L(r){const e=document.querySelector("#passDebitModal .m-btn-primary");if(!e?.disabled){e&&(e.disabled=!0,e.textContent="Traitement...");try{await p.post(`/api/passes/${r}/debit`),d("passDebitModal"),o.toast("Séance débitée avec succès","success"),m()}catch(s){e&&(e.disabled=!1,e.textContent="Débiter 1 séance"),o.toast(s.message||"Erreur lors du débit","error")}}}async function q(r){const e=g.find(v=>v.id===r);if(!e){o.toast("Pass introuvable","error");return}d("passRefundModal");const s=parseInt(e.sessions_remaining||0),t=parseInt(e.sessions_total||0),n=document.createElement("div");n.className="m-overlay open",n.id="passRefundModal",n.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(n),y(n,{noBackdropClose:!0}),h(n,()=>d(n.id))}async function H(r){const e=document.querySelector("#passRefundModal .m-btn-primary");if(!e?.disabled){e&&(e.disabled=!0,e.textContent="Traitement...");try{await p.post(`/api/passes/${r}/refund`),d("passRefundModal"),o.toast("Séance remboursée avec succès","success"),m()}catch(s){e&&(e.disabled=!1,e.textContent="Rembourser 1 séance"),o.toast(s.message||"Erreur lors du remboursement","error")}}}async function V(r){const e=g.find(a=>a.id===r);if(!e){o.toast("Pass introuvable","error");return}const s=parseInt(e.sessions_remaining||0),t=parseInt(e.sessions_total||0),n=t>0?Math.round((e.price_cents||0)/t):0,c=(s*n/100).toFixed(2).replace(".",",")+" €",i=document.createElement("div");i.className="m-overlay open",i.id="passFullRefundModal",i.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(i),y(i,{noBackdropClose:!0}),h(i,()=>d("passFullRefundModal"))}async function U(r){const e=document.querySelector("#passFullRefundModal .m-btn-danger");if(!e?.disabled){e&&(e.disabled=!0,e.textContent="Traitement...");try{const s=document.getElementById("fullRefundReason")?.value?.trim()||null,t=await p.post(`/api/passes/${r}/refund-full`,{reason:s});if(F(),d("passFullRefundModal"),t.stripe_refund_cents!=null){const n=(t.stripe_refund_cents/100).toFixed(2).replace(".",",")+" €";o.toast(`Pass remboursé (${n} Stripe${t.stripe_fees_cents>0?" — frais "+(t.stripe_fees_cents/100).toFixed(2).replace(".",",")+" €":""})`,"success")}else o.toast("Pass annulé (pas de paiement Stripe à rembourser)","success");m()}catch(s){e&&(e.disabled=!1,e.textContent="Rembourser et annuler"),o.toast(s.message||"Erreur lors du remboursement total","error")}}}async function G(r){const e=g.find(t=>t.id===r);if(!e){o.toast("Pass introuvable","error");return}if(await $("Annuler le pass",`Annuler le pass ${e.code} ? Cette action est irréversible.`,"Annuler le pass","danger"))try{await p.patch(`/api/passes/${r}`,{status:"cancelled"}),o.toast("Pass annulé","success"),m()}catch(t){o.toast(t.message||"Erreur lors de l'annulation","error")}}async function O(r,e){if(await $("Supprimer le pass",`Supprimer définitivement le pass ${e} ? Cette action est irréversible.`,"Supprimer","danger"))try{await p.delete(`/api/passes/${r}`),o.toast("Pass supprimé","success"),m()}catch(t){o.toast(t.message||"Erreur lors de la suppression","error")}}function Z(r){b=r}function J(r){x=r}Object.defineProperty(window,"passFilter",{get(){return b},set(r){b=r},configurable:!0});Object.defineProperty(window,"passSearch",{get(){return x},set(r){x=r},configurable:!0});E({loadPasses:m,openCreatePass:j,submitCreatePass:N,passServiceChanged:D,openDebitPass:T,debitPass:L,refundPass:q,submitRefundPass:H,fullRefundPass:V,submitFullRefundPass:U,cancelPass:G,deletePass:O,setPassFilter:Z,passSearchInput:J});export{m as loadPasses,I as renderPasses};
