import{s as k,a as g,G as u,l as I,d as M}from"./dashboard-Bo9qCwvA.js";import{i as T,s as S}from"./plan-gate-Badg7tzn.js";let p="all",b="all",h=[];function v(o){return String(o||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function f(o){return((o||0)/100).toFixed(2).replace(".",",")+" €"}async function _(){if(!T()){S(document.getElementById("contentArea"),"Facturation");return}const o=document.getElementById("contentArea");o.innerHTML='<div class="loading"><div class="spinner"></div></div>';try{const t=new URLSearchParams;p!=="all"&&t.set("status",p),b!=="all"&&t.set("type",b);const e=await g.get(`/api/invoices?${t}`),i=e.invoices||[],r=e.stats||{};let s="";s+=`<div class="stats" style="grid-template-columns:repeat(4,1fr)">
      ...
      <select onchange="invoiceType=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        <option value="all" ${b==="all"?"selected":""}>Tous types</option>
        <option value="invoice" ${b==="invoice"?"selected":""}>Factures</option>
        <option value="quote" ${b==="quote"?"selected":""}>Devis</option>
        <option value="credit_note" ${b==="credit_note"?"selected":""}>Notes de crédit</option>
      </select>
      <select onchange="invoiceFilter=this.value;loadInvoices()" style="padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-xs);font-size:.78rem">
        ...
        </tr></thead><tbody>`,i.forEach(n=>{const l={draft:"var(--text-4)",sent:"var(--gold)",paid:"var(--green)",overdue:"var(--red)",cancelled:"var(--text-4)"},d={draft:"Brouillon",sent:"Envoyée",paid:"Payée",overdue:"En retard",cancelled:"Annulée"},c={invoice:"Facture",quote:"Devis",credit_note:"Note crédit"},y={invoice:"var(--primary)",quote:"var(--gold)",credit_note:"var(--text-3)"},m=l[n.status]||"var(--text-4)",w=new Date(n.issue_date).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"});s+=`<tr style="border-bottom:1px solid var(--border-light)">
  ...
  </div>`,document.body.appendChild(s),I(document.getElementById("invModal"),{noBackdropClose:!0}),E(),$(),t.preselect_client_id){const a=document.getElementById("invClient");a&&(a.value=t.preselect_client_id),await C(),(t.precheck_booking_id||t.precheck_group_id)&&setTimeout(()=>{h.forEach((n,l)=>{if(t.precheck_booking_id&&n.id===t.precheck_booking_id||t.precheck_group_id&&n.group_id===t.precheck_group_id){const c=document.querySelector(`[data-unbilled-idx="${l}"]`);c&&!c.checked&&(c.checked=!0,B(l,!0))}})},100)}}function E(){const o=document.getElementById("invLines");if(!o)return;const t=document.createElement("div");t.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",t.innerHTML=`
    ...
    <div style="font-size:1rem;font-weight:700;color:var(--text);margin-top:4px">Total TTC : ${f(s)}</div>`}async function A(o){const t=document.getElementById("invClient")?.value;if(!t){u.toast("Sélectionnez un client","error");return}const e=document.getElementById("invLines"),i=[];if(e.querySelectorAll(":scope > div").forEach(r=>{const s=r.querySelector(".inv-desc")?.value?.trim(),a=parseFloat(r.querySelector(".inv-qty")?.value||1),n=parseFloat(r.querySelector(".inv-price")?.value||0),l=r.getAttribute("data-booking-id")||void 0;s&&n!==0&&i.push({description:s,quantity:a,unit_price_cents:Math.round(n*100),booking_id:l})}),i.length===0){u.toast("Ajoutez au moins une ligne","error");return}try{const r={client_id:t,type:o||"invoice",items:i,vat_rate:parseFloat(document.getElementById("invVat")?.value||21),due_days:parseInt(document.getElementById("invDueDays")?.value||30),client_bce:document.getElementById("invClientBce")?.value?.trim()||void 0,notes:document.getElementById("invNotes")?.value?.trim()||void 0},s=await g.post("/api/invoices",r);document.getElementById("invModal")._dirtyGuard?.markClean(),closeModal("invModal"),u.toast(o==="quote"?"Devis créé !":"Facture créée !","success"),_()}catch(r){u.toast(r.message||"Erreur","error")}}async function D(o,t){if(await k("Changer le statut",{sent:"Marquer comme envoyée ?",paid:"Marquer comme payée ?"}[t]||`Changer le statut en "${t}" ?`,"Confirmer"))try{await g.patch(`/api/invoices/${o}/status`,{status:t}),u.toast("Statut mis à jour","success"),_()}catch(r){u.toast(r.message||"Erreur","error")}}async function L(o){if(await k("Supprimer le brouillon","Supprimer ce brouillon ?","Supprimer","danger"))try{await g.delete(`/api/invoices/${o}`),u.toast("Brouillon supprimé","success"),_()}catch(e){u.toast(e.message||"Erreur","error")}}function F(o){const t=localStorage.getItem("genda_token");window.open(`/api/invoices/${o}/pdf?token=${t}`,"_blank","noopener,noreferrer")}async function N(o,t){const e=document.createElement("div");e.className="m-overlay open",e.id="creditNoteModal",e.innerHTML=`<div class="m-dialog m-sm">
    <div class="m-header-simple">
  ...
  </div>`,document.body.appendChild(e),I(e,{noBackdropClose:!0})}async function j(o){try{const t=document.getElementById("cnReason")?.value?.trim()||null,e=document.getElementById("cnMarkCancelled")?.checked||!1,i=await g.post(`/api/invoices/${o}/credit-note`,{reason:t,mark_original_cancelled:e});closeModal("creditNoteModal"),u.toast(`Note de crédit ${i.credit_note?.invoice_number||""} émise`,"success"),_()}catch(t){u.toast(t.message||"Erreur lors de l'émission","error")}}async function P(o){if(await k("Créer une facture pour ce rendez-vous ?"))try{const t=await g.post("/api/invoices",{booking_id:o,type:"invoice"});u.toast("Facture créée ! Retrouvez-la dans Facturation.","success"),document.querySelectorAll(".ni").forEach(e=>e.classList.remove("active")),document.querySelector('[data-section="invoices"]')?.classList.add("active"),document.getElementById("pageTitle").textContent="Facturation",_()}catch(t){u.toast(t.message||"Erreur","error")}}async function C(){const o=document.getElementById("invClient")?.value,t=document.getElementById("invUnbilledSection"),e=document.getElementById("invUnbilledList");if(!o||!t||!e){t&&(t.style.display="none");return}try{h=(await g.get(`/api/invoices/unbilled?client_id=${o}`)).bookings||[]}catch{h=[]}if(h.length===0){t.style.display="none";return}t.style.display="",e.innerHTML=h.map((i,r)=>{const s=new Date(i.start_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}),a=i.booked_price_cents??i.variant_price_cents??i.service_price_cents??0,n=i.service_name+(i.variant_name?" — "+i.variant_name:"")+" ("+(i.practitioner_name||"?")+") — "+s,l=i.pass_covered?' <span style="font-size:.68rem;color:var(--green);font-weight:600;padding:1px 6px;border-radius:4px;background:var(--green-bg)">Pass</span>':"",d=i.promotion_discount_cents>0?' <span style="font-size:.68rem;color:var(--green);font-weight:600">promo</span>':"",c=i.pass_covered?'<span style="color:var(--green)">inclus</span>':f(a);return`<label style="display:flex;align-items:center;gap:8px;padding:8px 12px;font-size:.82rem;cursor:pointer;border-bottom:1px solid var(--border-light);background:${r%2===0?"var(--white)":"var(--surface)"}">
    ...
    </label>`}).join("")}function B(o,t){const e=h[o];if(!e)return;const i=document.getElementById("invLines");if(i){if(t){const r=new Date(e.start_at).toLocaleDateString("fr-BE",{timeZone:"Europe/Brussels"}),s=e.service_name+(e.variant_name?" — "+e.variant_name:"")+" ("+(e.practitioner_name||"")+") — "+r,a=e.booked_price_cents??e.variant_price_cents??e.service_price_cents??0;if(e.pass_covered&&e.pass_info){x(e.id,s+" (inclus abonnement)",1,0);const n=e.pass_info,l="1 crédit consommé — Pass "+n.name+" ("+n.sessions_remaining+"/"+n.sessions_total+" restants)";[...i.querySelectorAll(".inv-desc")].some(c=>c.value===l)||x(e.id,l,1,0)}else if(x(e.id,s,1,a/100),e.promotion_discount_cents>0&&e.promotion_label){const n="Réduction : "+e.promotion_label+(e.promotion_discount_pct?" (-"+e.promotion_discount_pct+"%)":"");x(e.id,n,1,-(e.promotion_discount_cents/100))}if(e.deposit_status==="paid"&&e.deposit_amount_cents&&e.deposit_payment_intent_id){const n=parseInt(e.gc_paid_cents)||0,l=parseInt(e.deposit_amount_cents)||0;if(e.deposit_payment_intent_id.startsWith("gc_")&&n<=0){const c="Acompte payé (Carte cadeau "+e.deposit_payment_intent_id.replace("gc_","")+")";[...i.querySelectorAll(".inv-desc")].some(m=>m.value===c)||x(e.id,c,1,-(l/100))}else if(n>0){const d="Acompte payé (Carte cadeau)";[...i.querySelectorAll(".inv-desc")].some(m=>m.value===d)||x(e.id,d,1,-(n/100));const y=l-n;if(y>0){const m="Acompte payé (Stripe)";[...i.querySelectorAll(".inv-desc")].some(z=>z.value===m)||x(e.id,m,1,-(y/100))}}else if(e.deposit_payment_intent_id.startsWith("pi_")||e.deposit_payment_intent_id.startsWith("cs_")){const d="Acompte payé (Stripe)";[...i.querySelectorAll(".inv-desc")].some(y=>y.value===d)||x(e.id,d,1,-(l/100))}}}else i.querySelectorAll(`[data-booking-id="${e.id}"]`).forEach(r=>r.remove());$()}}function x(o,t,e,i){const r=document.getElementById("invLines");if(!r)return;const s=document.createElement("div");s.style.cssText="display:grid;grid-template-columns:1fr 60px 100px 30px;gap:8px;align-items:center;margin-bottom:6px",s.setAttribute("data-booking-id",o),s.innerHTML=`
    ...
    <button onclick="this.parentElement.remove();updateInvTotals()" style="background:none;border:none;cursor:pointer;color:var(--red);font-size:1rem"><svg class="gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`,r.appendChild(s)}Object.defineProperty(window,"invoiceFilter",{get(){return p},set(o){p=o}});Object.defineProperty(window,"invoiceType",{get(){return b},set(o){b=o}});M({loadInvoices:_,openInvoiceModal:q,addInvoiceLine:E,updateInvTotals:$,saveInvoice:A,changeInvoiceStatus:D,deleteInvoice:L,downloadInvoicePDF:F,createInvoiceFromBooking:P,invClientChanged:C,invToggleUnbilled:B,issueCreditNote:N,submitCreditNote:j});export{E as addInvoiceLine,D as changeInvoiceStatus,P as createInvoiceFromBooking,L as deleteInvoice,F as downloadInvoicePDF,_ as loadInvoices,q as openInvoiceModal,A as saveInvoice,$ as updateInvTotals};
